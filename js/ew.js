...
(function (ew$1, $$1, luxon) {
  ...
  function MultiPage(formid) {
    ...
    this.pagerTemplate =
      '<nav><ul class="pagination"><li class="page-item previous ew-prev"><a href="#" class="page-link"><i class="fa-solid fa-angle-left"></i> {Prev}</a></li><li class="page-item next ew-next"><a href="#" class="page-link">{Next} <i class="fa-solid fa-angle-right"></i></a></li></ul></nav>'; // Pager template
    ...
    // DOM loaded
    this.init = function () {
      var tpl = this.pagerTemplate
        .replace(/\{prev\}/i, ew.language.phrase("Prev"))
        .replace(/\{next\}/i, ew.language.phrase("Next"));
      if (this.isTab) {
        ...
        this.$form.find(".ew-prev").click(function (e) {
          self.$pages
            .off("show.bs.tab", _show)
            .filter(".active")
            .parent()
            .prev(
              ":has([data-bs-toggle=tab]:not(.ew-hidden):not(.ew-disabled))"
            )
        ...
        if (this.hideTabs)
          this.$form.find(".ew-multi-page > .ew-nav > .nav").hide();
        ...
        this.$form.find(".ew-prev").click(function (e) {
          self.$pages
            ...
            .filter(":has(.collapse.show)")
            .prev(
              ":has([data-bs-toggle=collapse]:not(.ew-hidden):not(.ew-disabled))"
            )
            .toggleClass("disabled d-none", false)
    ...
    this.pageShow = function () {
      ...
      var $btn = this.$form
        .closest(".content, .modal-content")
        .find("#btn-action, button.ew-submit")
        .prop("disabled", disabled)
        .toggle(!this.hideDisabledButton || !disabled);
    ...
    this.gotoPage = function (i) {
      ...
      if (this.pageIndex != i) {
        ...
        if (this.isTab) {
          if ($page.is(":not(.d-none):not(.disabled)")) $page.click();
          else
            $page
              .parent()
              .next(":has([data-bs-toggle=tab]):not(.d-none):not(.disabled)")
              .find("[data-bs-toggle=tab]")
              .toggleClass("disabled", false)
              .click();
        } else if (this.isCollapse) {
          var $p = $page.closest(".accordion-item");
          if ($p.is(":not(.d-none)")) $page.click();
          else
            $p.next(":has([data-bs-toggle=collapse]):not(.d-none)")
              .find("[data-bs-toggle=collapse]")
              .click();
        }
        this.pageIndex = i;
    ...
    return function (el) {
      ...
      if (elId && el) {
        ...
        if (level === 0 && !ew.sameText(name, "Default")) {
          ...
          level > 0 &&
          ["anonymous", "administrator", "default"].includes(name.toLowerCase())
        ) {
          return {
  ...
  function required(fieldName) {
    return function (el) {
      ...
      let $el = $$1(el),
        $p = $el.closest(
          "#r_" +
            ((_$el$data = $el.data("field")) == null
              ? void 0
              : _$el$data.substring(2))
        ); // Find the row
      ...
      if (el && !ew.hasValue(el)) {
        return {
          required: ew.language
            .phrase("EnterRequiredField")
            .replace("%s", fieldName),
        };
      }
      return false;
  ...
  function passwordStrength(el) {
    let $el = $$1(el);
    if (
      !ew.isMaskedPassword(el) &&
      $el.hasClass("ew-password-strength") &&
      !$el.data("validated")
    ) {
      return {
  ...
  function username(raw) {
    return function (el) {
      if (
        ...
        el.value.match(
          new RegExp(
            "[" + ew.escapeRegExChars(ew.INVALID_USERNAME_CHARACTERS) + "]"
          )
        )
      )
        return {
  ...
  function password(raw) {
    return function (el) {
      if (
        !raw &&
        !ew.ENCRYPTED_PASSWORD &&
        el.value.match(
          new RegExp(
            "[" + ew.escapeRegExChars(ew.INVALID_PASSWORD_CHARACTERS) + "]"
          )
        )
      )
        return {
  ...
  function recaptcha(el) {
    ...
    if (
      el &&
      !ew.hasValue(el) &&
      ((_grecaptcha = grecaptcha) == null
        ? void 0
        : _grecaptcha.getResponse(el.dataset.id)) === ""
    ) {
      return {
  ...
  function minDate(min) {
    return function (el) {
      ...
      if (min instanceof HTMLElement) {
        ...
        if (min.value && min.dataset.formatPattern)
          minDateTime = ew.parseDate(minValue, minFormat);
      ...
      if (value && format && minDateTime) {
        ...
        if (dateTime < minDateTime)
          return {
            minDate: ew.language
              .phrase("IncorrectMinDate")
              .replace(
                "%s",
                ew.formatDateTime(
                  minDateTime,
                  ew.DATE_FORMAT + " " + ew.TIME_FORMAT
                )
              ),
          };
      }
  ...
  function maxDate(max) {
    return function (el) {
      ...
      if (max instanceof HTMLElement) {
        ...
        if (max.value && max.dataset.formatPattern)
          maxDateTime = ew.parseDate(maxValue, maxFormat);
  ...
  $$1.extend(Batch.prototype, {
    ...
    _deliver: function (data, statusText) {
      ...
      if (
        responses != null &&
        (_responses$error = responses.error) != null &&
        _responses$error.description
      ) {
        var _responses$error2;
        ew$1.alert(
          responses == null || (_responses$error2 = responses.error) == null
            ? void 0
            : _responses$error2.description
        );
        return;
      }
      ...
      $$1.each(responses, function (i, response) {
        ...
        if (statusText === "success") {
          ...
          (_request$settings$suc = request.settings.success) == null ||
            _request$settings$suc.call(request.xhr, response, statusText);
        }

        // Call complete
        (_request$settings$com = request.settings.complete) == null ||
          _request$settings$com.call(request.xhr, statusText);
      });
    },
  });
  ...
  function FormBase(id, pageId) {
    ...
    this.disableForm = function () {
      ...
      $$1(form.elements)
        .filter(":submit:not(.dropdown-toggle), .ew-submit")
        .prop("disabled", true)
        .addClass("disabled");
    ...
    this.enableForm = function () {
      ...
      $elements
        .filter(".ew-disabled-element")
        .removeClass("ew-disabled-element")
        .prop("disabled", false);
      $elements
        .filter(".ew-enabled-element")
        .removeClass("ew-enabled-element")
        .prop("disabled", true);
      $elements
        .filter(":submit:not(.dropdown-toggle), .ew-submit")
        .not(".ew-disabled") // Exclude submit buttons with .ew-disabled (disabled purposely)
        .prop("disabled", false)
    ...
    this.canSubmit = async function (e) {
      ...
      this.updateTextArea();
      if (
        ((_await$this$validate = await ((_this$validate = this.validate) == null
          ? void 0
          : _this$validate.call(this, e))) != null
          ? _await$this$validate
          : true) &&
        !$form.find(".is-invalid")[0]
      ) {
        $form
          .find(
            "input[name^=sv_], input[name^=p_], [name*=_query_builder_rule_], .ew-template input, .ew-custom-option"
          ) // Do not submit these values
          .prop("disabled", true)
          .addClass("ew-disabled-element");
        $form
          .find("[data-readonly=1][disabled]")
          .prop("disabled", false)
        ...
        if (
          ...
          (result === true || ($$1.isObject(result) && result.value))
        )
          // Support Swal.fire()
          return true;
      } else {
    ...
    this.submit = async function (e) {
      ...
      let form = this.getForm(),
        formAction =
          e == null ||
          (_e$originalEvent = e.originalEvent) == null ||
          (_e$originalEvent = _e$originalEvent.submitter) == null
            ? void 0
            : _e$originalEvent.formAction;
      if (formAction) {
    ...
    this.getList = function (name) {
      ...
      let field =
        (_this$getElement = this.getElement(name)) == null ||
        (_this$getElement = _this$getElement.dataset.field) == null
          ? void 0
          : _this$getElement.replace(/^x_|\[\]$/g, ""); // Remove element name prefix/suffix
    ...
    this.fixId = function (id, multiple, rowindex) {
      ...
      id = id.startsWith("x_")
        ? id.replace(/^x_/, prefix) // Field element name
        : prefix + id; // Field var
    ...
    this.updateList = function (id, list, rowindex, immediate) {
      ...
      (_list = list) != null
        ? _list
        : (list = $$1.isString(id) // Find the list if not provided
            ? (_Object$entries$find = Object.entries(this.lists).find(
                (entry) => entry[0] == id.replace(/^x_/, "")
              )) == null
              ? void 0
              : _Object$entries$find[1] // String
            : this.getList(id)); // HTMLElement
      if (list.template && $$1.isString(list.template))
        // Compile template in case updateLists() called before init()
        list.template = $$1.templates(list.template);
      let form = this.element,
        parents = list.parentFields
          .slice()
          .map((parent) => this.fixId(parent, false, rowindex)),
        // Clone and fix index
      ...
      return !ajax || immediate
        ? ew.updateOptions.call(this, id, parents, ajax, false) // Non-Ajax (lookup cache or user values) or update immediately
        : [
            id,
            parents.map((parent) => ew.getOptionValues(parent, form)),
            ajax,
            false,
          ]; // Ajax (async) => to be batch updated
    };
    ...
    this.updateLists = function (rowindex, immediate) {
      ...
      if (
        this.pageId == "grid" &&
        !$$1.isNumber(rowindex) &&
        !$$1.isUndefined(rowindex)
      )
        return;
      let form = this.getForm(); // Set up $element and htmlForm
      if (
        (form == null ||
        (_form$querySelector = form.querySelector("input#confirm")) == null
          ? void 0
          : _form$querySelector.value) == "confirm"
      )
        // Confirm page
        return;
      let selector = Object.entries(this.lists)
        .map((_ref) => {
          let [id, list] = _ref;
      ...
      if (selector && this.element.querySelector(selector))
        ...
        Object.entries(this.lists)
          ...
          .forEach((request) =>
            this.batch.add(ew.updateOptions.bind(this, ...request))
          ); // Batch update async requests
    ...
    this.createAutoSuggest = function (settings) {
      var options = Object.assign(
        ...
        ew.autoSuggestSettings,
        settings
      ); // Global settings + field specific settings
    ...
    this.setupFilters = function (e, filters) {
      let id = this.id,
        ...
        $sf = $$1(".ew-save-filter[data-form=" + id + "]").toggleClass(
          "disabled",
          !data
        ),
        $df = $$1(".ew-delete-filter[data-form=" + id + "]")
          .toggleClass("disabled", !filters.length)
          .toggleClass("dropdown-toggle", !!filters.length),
        $delete = $df
          .parent("li")
          .toggleClass("dropdown-submenu dropdown-hover", !!filters.length)
          .toggleClass("disabled", !filters.length),
        $save = $sf.parent("li").toggleClass("disabled", !data);
      let saveFilters = function (id, filters) {
        if (ew.CLIENT_SEARCH_FILTER) {
          localStorage.setItem(
            ew.PROJECT_NAME + "_" + id + "_filters",
          ...
          $$1
            ...
            .done(function (result) {
              var _result$;
              if ((_result$ = result[0]) != null && _result$.success)
      ...
      if (filters.length) {
        ...
        filters.forEach((filter, i, ar) => {
          $$1(
            '<li><a class="dropdown-item" data-index="' +
              i +
              '" data-ew-action="none">' +
              filter[0] +
              "</a></li>"
            ...
            .appendTo($submenu);
          $$1(
            '<li><a class="dropdown-item ew-reset-filter-list" data-index="' +
              i +
              '" data-ew-action="none">' +
              filter[0] +
              "</a></li>"
            ...
            .on("click", function (e) {
              if (currentPageID == "calendar") {
                ...
                ew.refresh(
                  ew.fetch(ew.setLayout(ew.currentPage(), false), {
                    method: "POST",
                    body,
                  })
                );
    ...
    this.init = function () {
      ...
      if (/s(ea)?rch$/.test(this.id)) {
        // Search panel
        if (this.initSearchPanel && !ew.hasFormData(form))
          $$1("#" + this.id + "_search_panel").removeClass("show");

        // Hide search operator column
        if (!$$1(".ew-table .ew-search-operator").text().trim())
          $$1(".ew-table .ew-search-operator").parent("td").hide();

        // Search operators
        $form.find("select[id^=z_]").each(function () {
          var $this = $$1(this).trigger("change");
          if ($this.val() != "BETWEEN")
            $form.find("#w_" + this.id.substring(2)).trigger("change");
        });
      }
      ...
      // HTML editors
      loadjs.ready(["editor"], () =>
        setTimeout(this.initEditors.bind(this), 0)
      ); // Delay for custom template to apply first
      ...
      if (this.$element.is("form")) {
        ...
        // Detail pages
        this.$element
          .find(".ew-detail-pages .ew-nav a[data-bs-toggle=tab]")
      ...
      if (
        ...
        Array.from(form.elements).find((el) =>
          el.matches("button.disabled.enable-on-init")
        )
      )
        this.enableForm();
  ...
  let Field = /*#__PURE__*/ (function () {
    ...
    _proto.resetInvalid = function resetInvalid() {
      ...
      this.clearErrors();
      if (
        (_this$_element = this._element) != null &&
        (_this$_element = _this$_element.classList) != null &&
        _this$_element.contains("is-invalid") &&
        !this._error
      )
        this.addError({
          server:
            (_this$_element$closes = this._element.closest(
              ew.fieldContainerSelector
            )) == null ||
            (_this$_element$closes =
              _this$_element$closes.querySelector(".invalid-feedback")) == null
              ? void 0
              : _this$_element$closes.innerHTML,
        }); // Server side error
    };
    ...
    _proto.updateFeedback = function updateFeedback() {
      ...
      if (this._element && err) {
        ...
        let feedback =
          (_this$_element$closes2 = this._element.closest(
            ew.fieldContainerSelector
          )) == null
            ? void 0
            : _this$_element$closes2.querySelector(".invalid-feedback");
    ...
    _proto.canFocus = function canFocus() {
      ...
      return (
        el &&
        !(
          (el.hidden && !el.tagName == "SELECTION-LIST") ||
          el.readonly ||
          el.disabled ||
          el.type == "hidden" ||
          ((_el$style = el.style) == null ? void 0 : _el$style.display) ==
            "none" ||
          ((_el$classList = el.classList) != null &&
            _el$classList.contains("d-none"))
        )
      );
    };
    ...
    _createClass(Field, [
      ...
      },
      {
         ...
         */ set: function (el) {
          ...
          this._element = el;
          this._checkbox =
            (_this$_element2 = this._element) != null &&
            (_this$_element2 = _this$_element2.id) != null &&
            _this$_element2.match(/^[xy]_/)
              ? document.getElementById(
                  this._element.id.replace(/^[xy]_/, "u_").replace(/\[\]$/, "")
                )
              : null; // Find the checkbox for the field in Update page
        },
      },
      {
        key: "value",
  ...
  let Form = /*#__PURE__*/ (function (_FormBase) {
    ...
    _proto.setFocus = function setFocus(el) {
      ...
      if (el && el != document.activeElement && el.focus) {
        let preventScroll =
          ...
          Form.autoFocusPreventScroll &&
          this.autoFocusPreventScroll;
        setTimeout(() => {
          el.focus({
            preventScroll,
          });
    ...
    _proto.focus = function focus() {
      ...
      for (let [fldvar, field] of Object.entries(this.fields)) {
        ...
        if (
          field.invalid ||
          ((_this$_error2 = this._error) != null && _this$_error2[fldvar])
        ) {
    ...
    _proto.getFocusable = function getFocusable(field) {
      var _field$element;
      (_field$element = field.element) != null
        ? _field$element
        : (field.element = this.getFieldElements(field.name));
      if (!field.canFocus())
        field.element = this.getFieldElements(field.name, 0); // Inline-Add
      if (!field.canFocus())
        field.element = this.getFieldElements(field.name, 1); // Inline-Edit or Grid-Add/Edit
    };
    ...
    _proto.tryFocus = function tryFocus(fieldName) {
      if (!this.canFocus()) return;
      if (!fieldName && (!Form.autoFocus || !this.autoFocus || this._focused))
        return;
      ...
      if (["add", "edit"].includes(this.pageId)) {
        ...
        let form = this.getForm(),
          detailpage = Array.from(
            form.querySelectorAll("input[name=detailpage]")
          ).find((dp) => {
            var _ew$forms$get;
            return (_ew$forms$get = ew.forms.get(dp.value)) == null
              ? void 0
              : _ew$forms$get.invalid;
          });
        if (detailpage) {
          detailpage.focus();
          return;
      ...
      if (
        ...
        input.focus({
          preventScroll:
            Form.autoFocusPreventScroll && this.autoFocusPreventScroll,
    ...
    _proto.makeVisible = function makeVisible(el) {
      if (this.multiPage) {
        ...
        if ($pane[0] && !$pane.hasClass("active")) {
          $pane
            .closest(".ew-nav")
            .find("a[data-bs-toggle=tab][href='#" + $pane.attr("id") + "']")
            .trigger("click");
          return true;
        }
      }
    ...
    _proto.validate = async function validate(e) {
      ...
      let form = this.getForm();
      if (
        ((_form$querySelector = form.querySelector("#confirm")) == null
          ? void 0
          : _form$querySelector.value) == "confirm"
      )
        return true;
      ...
      let addcnt = 0,
        ...
        // Inline-Add
        action =
          ((_form$querySelector2 = form.querySelector("#action")) == null
            ? void 0
            : _form$querySelector2.value) ||
          ((_ref =
            (_e$currentTarget = e == null ? void 0 : e.currentTarget) != null
              ? _e$currentTarget
              : inlineAdd) == null
            ? void 0
            : _ref.dataset.action),
        ...
        gridinsert =
          action == "gridinsert" ||
          (["insert", "confirm"].includes(action) && keycnt),
      ...
      for (let i = startcnt; i <= rowcnt; i++) {
        ...
        if (["list", "grid"].includes(this.pageId)) {
          if (gridinsert ? !this.emptyRow(rowIndex) : true) addcnt++;
          else continue;
        }
        ...
        if (
          !((_await$this$customVal = await ((_this$customValidate =
            this.customValidate) == null
            ? void 0
            : _this$customValidate.call(this, form))) != null
            ? _await$this$customVal
            : true)
        ) {
          this.focus();
          return false;
      ...
      if (["add", "edit"].includes(this.pageId)) {
        ...
        for (let dp of detailpages) {
          ...
          if (
            !((_await$frm$validate = await (frm == null || frm.validate == null
              ? void 0
              : frm.validate(e))) != null
              ? _await$frm$validate
              : true)
    ...
    _proto.getCurrentRowIndex = function getCurrentRowIndex() {
      ...
      return (_this$getForm$dataset = this.getForm().dataset.rowindex) != null
        ? _this$getForm$dataset
        : "";
    };
    ...
    _proto.getValue = function getValue(rowIndex) {
      var _rowIndex2;
      (_rowIndex2 = rowIndex) != null
        ? _rowIndex2
        : (rowIndex = this.getCurrentRowIndex());
      let value = {};
      for (let field of Object.values(this.fields)) {
        var _field$element2;
        (_field$element2 = field.element) != null
          ? _field$element2
          : (field.element = this.getFieldElements(field.name, rowIndex));
    ...
    _proto.getFieldValue = function getFieldValue(fldvar, rowIndex) {
      ...
      if (field) {
        ...
        (_rowIndex3 = rowIndex) != null
          ? _rowIndex3
          : (rowIndex = this.getCurrentRowIndex());
        (_field$element3 = field.element) != null
          ? _field$element3
          : (field.element = this.getFieldElements(field.name, rowIndex));
        return field.value;
      }
    ...
    _proto.getFieldElements = function getFieldElements(name, rowIndex) {
      ...
      ); // By name directly (e.g. email form)
    };
    ...
    _proto.setInvalid = function setInvalid(rowIndex) {
      ...
      if (typeof rowIndex === "undefined" && this.formKeyCountName) {
        ...
        // Process detail forms
        if (["add", "edit"].includes(this.pageId))
          form.querySelectorAll("input[name=detailpage]").forEach((dp) => {
            var _ew$forms$get2;
            return (_ew$forms$get2 = ew.forms.get(dp.value)) == null
              ? void 0
              : _ew$forms$get2.setInvalid();
          });
      }
    };
    _createClass(Form, [
      ...
      },
      {
        ...
        get: function () {
          return (
            this._error ||
            Object.values(this.fields).some((field) => field.invalid)
          );
        },
      },
      {
        ...
        get: function () {
          ...
          let keycnt = this.formKeyCountName
            ? (_this$getForm$querySe = this.getForm().querySelector(
                "#" + this.formKeyCountName
              )) == null
              ? void 0
              : _this$getForm$querySe.value
            : undefined; // Get key_count
  ...
  let FormBuilder = /*#__PURE__*/ (function () {
    ...
    _proto.setQueryBuilderLists = function setQueryBuilderLists(value) {
      if (value && typeof value == "object" && !Array.isArray(value)) {
        for (let [id, list] of Object.entries(value))
          value[id] = {
            ...list,
            ...{
    ...
    // Add field
    _proto.addField = function addField(fldvar, validators, invalid) {
      if (!this.fields.find((field) => field[0] == fldvar))
        this.fields.push([fldvar, validators, invalid]);
      return this;
    };
    ...
    _proto.build = function build() {
      ...
      if (typeof this.customValidate == "function")
        form.customValidate = this.customValidate;
      ...
      if (this.submitWithFetch) {
        ...
        form.on("aftersubmit", (e, args) =>
          ew.refresh(
            args == null ? void 0 : args.result,
            args == null ? void 0 : args.context
          )
        );
      }
  ...
  let AjaxLookup = /*#__PURE__*/ (function () {
    ...
    function AjaxLookup(settings) {
      ...
      let m = this.elementId.match(/^[xy](\d*|\$rowindex\$)_/),
        rowindex = m ? m[1] : "";
      this.parentFields =
        (_this$list$parentFiel =
          (_this$list$parentFiel2 = this.list.parentFields) == null
            ? void 0
            : _this$list$parentFiel2
                .slice() // Clone
                .map((pf) =>
                  pf.split(" ").length == 1
                    ? pf.replace(/^x_/, "x" + rowindex + "_")
                    : pf
                )) != null
          ? _this$list$parentFiel
    ...
    _proto.formatResult = function formatResult(opt) {
      ...
      return this.list.template && !this.isAutoSuggest
        ? this.list.template.render(opt, ew.jsRenderHelpers)
        : ew.displayValue(opt, this.element) || opt[0];
    };
    ...
    _proto.generateRequest = function generateRequest() {
      ...
      if (this.parentFields.length > 0)
        this.parentFields.forEach(
          (pf, i) =>
            (data["v" + (i + 1)] = ew.getOptionValue(pf, this.formElement))
        );
      return data;
    };
    ...
    _createClass(AjaxLookup, [
      {
        ...
        get: function () {
          ...
          (_this$_isAutoSuggest = this._isAutoSuggest) != null
            ? _this$_isAutoSuggest
  ...
  let AutoSuggest = /*#__PURE__*/ (function (_AjaxLookup) {
    ...
    function AutoSuggest(settings) {
      ...
      if (!_this.input || _this.elementId.includes("$rowindex$"))
        return _assertThisInitialized(_this);
      ...
      if ($input.val() && $element.val())
        _this.element.add($element.val(), $input.val(), true);

      // Add events
      $input
        .on("typeahead:select", (e, d) => {
      ...
      if (async && !self.templates.footer)
        self.templates.footer =
          '<div class="tt-footer dropdown-item"><div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">' +
          ew.language.phrase("LoadingMore") +
          "</span></div></div>"; // "footer" template
      ...
      let source = (query, syncResults, asyncResults) => {
        if (async) {
          ...
          timer = $$1.later(_this.debounce, null, () => {
            ...
            $$1
              .ajax(_this.prepare(query))
              .done((data) => asyncResults(_this.transform(data)));
          });
        } else {
      ...
      $$1(function () {
        ...
        let menu = self.typeahead.menu,
          ...
          suggestionHeight = () =>
            $menu.find(".tt-suggestion").outerHeight(false);
        ...
        $input.on("typeahead:rendered", (e, suggestions) => {
          ...
          if (rendered > 0)
            $dataset.scrollTop(suggestionHeight() * (count - rendered)); // Scroll to the first suggestion
        ...
        if (async) {
          ...
          $dataset.on("scroll", () => {
            ...
            (_loadingMoreTimer = loadingMoreTimer) == null ||
            ...
            loadingMoreTimer = $$1.later(self.delay, null, () => {
              ...
              if (!$footer.is(":hidden") && !loadingMore) {
                let currentOffset =
                    $dataset.offset().top + $dataset.outerHeight(false),
                  loadingMoreOffset =
                    $footer.offset().top + $footer.outerHeight(false);
                if (currentOffset + 20 > loadingMoreOffset) {
                  ...
                  (_loadingMoreTimer2 = loadingMoreTimer) == null ||
                    _loadingMoreTimer2.cancel();
                }
              }
            });
          });
    ...
    _proto.setValue = function setValue(v) {
      ...
      if (index < 0) {
        ...
        if (this.forceSelection && v) {
          ...
          this.$element
            ...
            .html(ew.language.phrase("ValueNotExist"));
      ...
      if (v !== this.$element.attr("value"))
        this.$element.attr("value", v).trigger("change"); // Set value to the actual field
    };

    // Transform suggestion
    _proto.transform = function transform(data) {
      let results = _AjaxLookup.prototype.transform
        .call(this, data)
        .map((item) =>
          Object.assign({}, item, {
      ...
      this.element.options = results.map(
        (item) => new ew.SelectionListOption(item.text, item.lf || item[0])
      );
      return results;
    };
      ...
      if (this.options.length) {
        let results = this.options.filter((item) => {
          if (this.lookupAllDisplayFields) {
            let v = [item.df, item.df2, item.df3, item.df4]
              .map((df) => String(df).toLowerCase())
              .join(" ");
            return query
              .toLowerCase()
              .split(" ")
              .filter((q) => q !== "")
              .every((q) => v.includes(q));
          } else {
    ...
    _proto.getMore = function getMore() {
      ...
      return $$1
        .ajax(settings)
        .done((data) =>
          menu.datasets[0]._append(menu.query, this.transform(data))
        );
    };
    _createClass(AutoSuggest, [
      {
        ...
        get: function () {
          return (
            this.typeahead.menu.$node.find(".tt-suggestion.tt-selectable")
              .length || 0
          );
        },
      },
  ...
  let Forms = /*#__PURE__*/ (function () {
    ...
    _proto.get = function get(el) {
      var _ew$getForm;
      if (!el) return null;
      let id = $$1.isString(el)
        ? el
        : (_ew$getForm = ew.getForm(el)) == null
        ? void 0
        : _ew$getForm.id;
    ...
    _proto.add = function add(f) {
      if (this._forms[f.id] && this._forms[f.id] !== f)
        delete this._forms[f.id];
      this._forms[f.id] = f;
    };
  ...
  var Select2Language = {
    ...
    loadingMore: function () {
      return (
        '<div class="spinner-border spinner-border-sm text-primary" role="status"><span class="visually-hidden">' +
        lang.loadingMore +
        "</span></div>"
      );
    },
  ...
  let Select2ResultsDecorator = /*#__PURE__*/ (function () {
    ...
    _proto.render = function render(decorated) {
      var $results = $$1(
        '<div class="select2-results__options ' +
          this.options.get("containerClass") +
          '" role="listbox"></div>'
      ); //***
    ...
    _proto.append = function append(decorated, data) {
        ...
        if (this.$results.children().length === 0) {
          if (this.$element.data("updating") && data.pagination.more) {
            this.trigger("results:message", {
              message:
                '<div class="spinner-border spinner-border-sm text-primary ew-select-spinner" role="status"><span class="visually-hidden">' +
                ew.language.phrase("Loading") +
                "</span></div> " +
                ew.language.phrase("Loading"),
            });
            this.$element.one("updated", () =>
              this.$element.select2("close").select2("open")
            );
          } else {
            this.trigger("results:message", {
              message: "noResults",
      ...
      for (var d = 0; d < data.results.length; d++) {
        ...
        if (!$row.length || $row.children().length == cols) {
          // Add new row
          $row = $$1(
            '<div class="' + this.options.get("rowClass") + '"></div>'
          );
        ...
        if (d == len - 1) {
          ...
          for (var i = 0; i < cnt; i++)
            $row.append(
              '<div class="' + this.options.get("cellClass") + '"></div>'
            );
        }
      }
    };
    _proto.option = function option(decorated, data) {
      ...
      this.options
        .get("cellClass")
        .split(" ")
        .forEach((c) => option.classList.add(c)); //***
  ...
  let ModalResults = /*#__PURE__*/ (function () {
    ...
    _proto.bind = function bind(decorated, container, $container) {
      ...
      // Remove handlers
      (_container$listeners$ = container.listeners["results:select"]) == null ||
        _container$listeners$.pop();
  ...
  let ModalSearch = /*#__PURE__*/ (function () {
    ...
    _proto.bind = function bind(decorated, container, $container) {
      ...
      var self = this;
      decorated.call(this, container, $container);
      (_container$listeners$ = container.listeners["close"]) == null ||
        _container$listeners$.pop(); // Remove handler from Search
  ...
  let DropdownSearchDecorator = /*#__PURE__*/ (function () {
    ...
    _proto.handleSearch = function handleSearch(decorated, evt) {
      ...
      if (
        [KEYS.TAB, KEYS.SHIFT, KEYS.CTRL, KEYS.ALT, KEYS.PRINT_SCREEN].includes(
          key
        )
      ) {
        return;
  ...
  let AttachBody$2 =
    $$1.fn.select2.amd.require._defined["select2/dropdown/attachBody"];
  ...
  let Select2AttachBody = /*#__PURE__*/ (function (_AttachBody) {
    ...
    _proto._attachPositioningHandler = function _attachPositioningHandler(
      decorated,
      container
    ) {
      var self = this;
      var events = [
        "scroll.select2." + container.id,
        "resize.select2." + container.id,
      ...
      container.$element
        .closest(".modal")
        .on("scroll.select2." + container.id, handler);
    };
    // Override _detachPositioningHandler
    _proto._detachPositioningHandler = function _detachPositioningHandler(
      decorated,
      container
    ) {
      var events = [
  ...
  let ModalAttachBody = /*#__PURE__*/ (function () {
    ...
    _proto._bindContainerResultHandlers = function _bindContainerResultHandlers(
      decorated,
      container
    ) {
      ...
      container.on("select", function (e) {
        ...
        let target =
          (_e$originalEvent = e.originalEvent) == null
            ? void 0
            : _e$originalEvent.currentTarget;
        target == null ||
          target.classList.add("select2-results__option--selected");
        self._updateDropdown();
      });
      container.on("unselect", function (e) {
        var _e$originalEvent2;
        let target =
          (_e$originalEvent2 = e.originalEvent) == null
            ? void 0
            : _e$originalEvent2.currentTarget;
        target == null ||
          target.classList.remove("select2-results__option--selected");
        self._updateDropdown();
      });
      this._containerResultsHandlersBound = true;
    };
    _proto._updateDropdown = function _updateDropdown() {
      if (!this.$modal.find(this.$dropdownContainer)[0])
        this.$modal
          .find(".modal-body")
          .children()
          .detach()
    ...
    _proto._showDropdown = function _showDropdown(decorated) {
      ...
      (_this$$modal = this.$modal) != null
        ? _this$$modal
        : (this.$modal = $$1("#ew-modal-lookup-dialog"));
      ...
      this.$modal
        ...
        .append(
          ew.language
            .phrase("LookupTitle")
            .replace("%s", this.$element.data("caption"))
        );
      this.$modal
        .find(".modal-footer button[data-value]")
        .off()
        .on("click", function () {
  ...
  let FilterAttachBody = /*#__PURE__*/ (function (_AttachBody) {
    ...
    _proto._showDropdown = function _showDropdown(decorated) {
      ...
      $footer
        .find(".ew-filter-clear")
        .off("click.ew")
        .on("click.ew", (e) =>
          self.$element.data("select2").selection._handleClear(e)
        );
      let $filterDropdown = this.$dropdownContainer.find(".ew-filter-dropdown");
      if (!$filterDropdown.find(".ew-filter-btn")[0])
        $filterDropdown.append($footer);

      // Popper
      var dropdownButton = document.querySelector(
          ".ew-filter-dropdown-btn[data-table=" +
        ...
        reference =
          (_dropdownButton$close = dropdownButton.closest(
            ".ew-table-header-cell"
          )) != null
            ? _dropdownButton$close
            : dropdownButton.closest(".ew-table-header-btn");
      (_this$_popper2 = this._popper) != null
        ? _this$_popper2
        : (this._popper = Popper.createPopper(
            reference,
            this.$dropdownContainer[0],
  ...
  let DropdownAttachBody = /*#__PURE__*/ (function (_AttachBody) {
    ...
    _proto._showDropdown = function _showDropdown(decorated) {
      ...
      (_this$_popper = this._popper) != null
        ? _this$_popper
        : (this._popper = Popper.createPopper(
            this.$element.parent().find(".select2-container")[0],
            this.$dropdownContainer[0],
            {
              placement: ew.IS_RTL ? "bottom-end" : "bottom-start",
              modifiers: [
  ...
  let Select2DataAdapterDecorator = /*#__PURE__*/ (function () {
    ...
    _proto.option = function option(decorated, data) {
      ...
      var text = data.text,
        html =
          (_data$element = data.element) == null
            ? void 0
            : _data$element.innerHTML,
        $option = decorated.call(this, data);

      // Check HTML
      if (text.startsWith("<") && text.endsWith(">")) $option.html(text);
      else if (html && html != text) $option.html(html);
      return $option;
    };
  ...
  let _defined = $.fn.select2.amd.require._defined,
    ...
    MinimumResultsForSearch =
      _defined["select2/dropdown/minimumResultsForSearch"],
    ...
    if (options.resultsAdapter == null) {
      ...
      if (options.columns > 0 && options.customOption) {
        ...
        if (
          options.iconClass &&
          options.multiple &&
          options.templateResult == ew.selectOptions.templateResult
        ) {
          options._templateResult = options.templateResult;
          options.templateResult = (result) =>
            result.loading
              ? result.text
              : '<div class="form-check-input ew-dropdown-check-input"></div><label class="' +
                options.iconClass +
                ' ew-dropdown-check-label">' +
                options._templateResult(result) +
                "</label>";
        }
    ...
    if (options.selectionAdapter == null) {
      if (options.multiple) {
        ...
        options.selectionAdapter = Utils.Decorate(
          ...
          Select2MultipleSelectionDecorator
        );
      } else {
      ...
      if (options.allowClear) {
        options.selectionAdapter = Utils.Decorate(
          options.selectionAdapter,
          options.filter ? FilterAllowClear : AllowClear
        );
      }
      if (options.multiple) {
        options.selectionAdapter = Utils.Decorate(
  ...
  Pace.on("done", () => {
    var _forms$get;
    let form =
      document.querySelector(".modal.show form.ew-form") ||
      document.querySelector("form.ew-form");
    (_forms$get = forms.get(form == null ? void 0 : form.id)) == null ||
      _forms$get.tryFocus();
  });
  ...
  function createSelect(options) {
      ...
      options.ajax = {
        ...
        processResults: function (data) {
          ...
          return {
            ...
            pagination: {
              more:
                self.container.$results.find(
                  ".select2-results__option:not(.select2-results__option--load-more)"
                ).length +
                  ((_data$records$length =
                    (_data$records = data.records) == null
                      ? void 0
                      : _data$records.length) != null
                    ? _data$records$length
                    : 0) <
                lookup.recordCount,
            },
    ...
    let $select = $$1(
      "select[data-select2-id='" + options.selectId + "']"
    ).select2(args.options);
    $select.on("select2:open", function () {
      var _$$data$$dropdown$fin;
      (_$$data$$dropdown$fin = $$1(this)
        .data("select2")
        .$dropdown.find(".select2-search__field")[0]) == null ||
        _$$data$$dropdown$fin.focus();
    });
    if ($$1.isObject(options.ajax)) {
      $select.on("select2:opening", function () {
        $$1(this)
          .data("select2")
          .$results.find(".select2-results__option:not(.loading-results)")
          .remove();
      });
    }
  ...
  function createModalLookup(options) {
            ...
            pagination: {
              more:
                self.container.$results.find(
                  ...
                  ((_data$records$length2 =
                    (_data$records2 = data.records) == null
                      ? void 0
                      : _data$records2.length) != null
                    ? _data$records$length2
                    : 0) <
                lookup.recordCount,
            },
    ...
    $select.on("select2:open", function () {
      ...
      (_$$data$$dropdown$fin2 = $$1(this)
        .data("select2")
        .$dropdown.find(".select2-search__field")
        .addClass("form-control")[0]) == null || _$$data$$dropdown$fin2.focus();
    });
    if ($$1.isObject(options.ajax)) {
      $select.on("select2:opening", function () {
        $$1(this)
  ...
  function createFilter(options) {
            ...
            pagination: {
              more:
                self.container.$results.find(
                  ...
                  ((_data$records$length3 =
                    (_data$records3 = data.records) == null
                      ? void 0
                      : _data$records3.length) != null
                    ? _data$records$length3
                    : 0) <
                lookup.recordCount,
            },
    ...
    $select.on("select2:open", function () {
      ...
      (_$$data$$dropdown$fin3 = $$1(this)
        .data("select2")
        .$dropdown.find(".select2-search__field")
        .addClass("form-control")[0]) == null ||
        _$$data$$dropdown$fin3.focus({
  ...
  function initIcons(e) {
    ...
    $$1(el)
      .find(".ew-icon")
      .closest(".btn, .ew-home, .ew-row-link:not(.dropdown-item)")
      .each(function () {
        let $this = $$1(this);
        if ($this.hasClass("dropdown-toggle")) {
          ...
          if ($p.children(".btn").length == 1) {
            $p.tooltip({
              ...
              return (_bootstrap$Tooltip$ge = bootstrap.Tooltip.getInstance(
                e.currentTarget
              )) == null
                ? void 0
                : _bootstrap$Tooltip$ge.hide();
            });
            $this
              .next(".dropdown-menu")
  ...
  function initPasswordOptions(e) {
    var _e$target2;
    var el =
      (_e$target2 = e == null ? void 0 : e.target) != null
        ? _e$target2
        : document;
    if ($$1.fn.pStrength && typeof ew.MIN_PASSWORD_STRENGTH != "undefined") {
      $$1(el)
        .find(".ew-password-strength")
        .each(function () {
          var $this = $$1(this);
          if (!$this.data("pStrength"))
            $this.pStrength({
              ...
              onPasswordStrengthChanged: function (strength, percentage) {
                var $pst = $$1(
                    "[id='" + this.attr("data-password-strength") + "']"
                  ),
                  // Do not use #
                ...
                if (this.val() && !ew.isMaskedPassword(this)) {
                  ...
                  if (percentage < min * 0.25) {
                    $pb
                      ...
                      .removeClass(
                        "text-bg-warning text-bg-info text-bg-success"
                      );
                  ...
                  $pb.css("width", pct);
                  if (percentage > min * 0.5)
                    pct = ew.language
                      .phrase("PasswordStrength")
                      .replace("%p", pct);
                  $pb.html(pct);
          ...
          if (!$this.data("pGenerator"))
            $this.pGenerator({
              ...
              onPasswordGenerated: function (pwd) {
                ...
                $$1("#" + this.attr("data-password-field"))
                  .val(pwd)
                  .trigger("change")
                  .trigger("focus")
                  .triggerHandler("click"); // Trigger click to remove "is-invalid" class (Do not use $this.data)
              },
            });
        });
    }
  ...
  function sanitizeUrl(url) {
    ...
    searchParams.forEach((value, key) => {
      ...
      if (["<>", "<=", ">=", ">", "<"].includes(value))
        searchParams.set(key, value);
      else searchParams.set(key, ew.sanitize(value));
    });
    return buildUrl(obj);
  }
  ...
  function setSessionTimer() {
      ...
      if (counter > 0) {
        ...
        if (message.includes("%m") && message.includes("%s")) {
          message = message
            .replace(
              "%m",
              '<span class="ew-session-counter-minute">' +
                Math.floor(counter / 60) +
                "</span>"
            )
            .replace(
              "%s",
              '<span class="ew-session-counter-second">' +
                (counter % 60) +
                "</span>"
            );
          ...
          message = message.replace(
            "%s",
            '<span class="ew-session-counter-second">' + counter + "</span>"
          );
        }
        Swal.fire({
    ...
    let setTimer = () => {
      var _ew$vars$login;
      if (
        ew.SESSION_TIMEOUT > 0 &&
        window.location.pathname !=
          ((_ew$vars$login = ew.vars.login) == null ||
          (_ew$vars$login = _ew$vars$login.login) == null
            ? void 0
            : _ew$vars$login.url)
      ) {
    ...
    if (
      ew.SESSION_KEEP_ALIVE_INTERVAL > 0 ||
      (ew.IS_LOGGEDIN && ew.IS_AUTOLOGIN)
    ) {
      // Keep alive
      let keepAliveInterval = Math.max(
        ew.SESSION_KEEP_ALIVE_INTERVAL ||
          ew.SESSION_TIMEOUT - ew.SESSION_TIMEOUT_COUNTDOWN,
        ew.MIN_SESSION_KEEP_ALIVE_INTERVAL
      );
      keepAliveTimer = $$1.later(
  ...
  function initExportLinks(e) {
    var _e$target3;
    let $el = $$1(
      (_e$target3 = e == null ? void 0 : e.target) != null
        ? _e$target3
        : document
    );
    ...
    $el
      .find("a.ew-export-link[href]:not(.ew-email):not(.ew-print):not(.ew-xml)")
      .off("click")
      .on("click", function (e) {
        let href = this.href;
    ...
    $el
      .find("a.ew-export-link[href].ew-print, a.ew-export-link[href].ew-xml")
      .each(function () {
  ...
  function initMultiSelectCheckboxes(e) {
    var _e$target4;
    var el =
        (_e$target4 = e == null ? void 0 : e.target) != null
          ? _e$target4
          : document,
      $el = $$1(el),
    ...
    var _update = function (id) {
      ...
      $$1("input[type=checkbox]#" + id)
        .prop("checked", len == cnt)
        .prop("indeterminate", len != cnt && len != 0);
    };
    $cbs.on("click", (e) => _update(e.target.name.split("_")[0]));
    $el
      .find("input[type=checkbox].ew-priv:not(.ew-multi-select)")
      .each((i, el) => _update(el.id)); // Init
  }
  ...
  function getError(result) {
    ...
    return (
      result &&
      (((_result$error = result.error) == null
        ? void 0
        : _result$error.description) ||
        ((_result$error2 = result.error) == null
          ? void 0
          : _result$error2.message) ||
        ($$1.isString(result.error) && result.error) ||
  ...
  function fileDownload(href, data) {
    let isHtml =
        href.includes("export=html") ||
        href.includes(getApiUrl([ew.API_EXPORT_ACTION, "html"])),
      swal = window.parent.Swal; // Use window.parent.Swal in case in iframe
    data = mergeSearchParams(data, [
          ...
          .then(async (response) => {
            ...
            if (ct != null && ct.includes("json")) {
              ...
              let result = await response.json(),
                error =
                  (_getError = getError(result)) != null
                    ? _getError
                    : ew.language.phrase("FailedToExport");
              if (error) {
                swal.hideLoading();
                swal.update({
  ...
  function lazyLoad(e) {
    ...
    var el =
      (_e$target5 = e == null ? void 0 : e.target) != null
        ? _e$target5
        : document;
    el.querySelectorAll("img.ew-lazy").forEach((img, i) => {
      if (ew.LAZY_LOAD_DELAY > 0)
        setTimeout(() => (img.src = img.dataset.src), i * ew.LAZY_LOAD_DELAY);
  ...
  function updateDropdownPosition() {
    var select = $$1(".select2-container--open")
      .prev(".ew-select")
      .data("select2");
    if (select) {
  ...
  function initLightboxes(e) {
    ...
    var el =
      (_e$target6 = e == null ? void 0 : e.target) != null
        ? _e$target6
        : document;
  ...
  function initPdfObjects(e) {
    ...
    let el =
        (_e$target7 = e == null ? void 0 : e.target) != null
          ? _e$target7
          : document,
  ...
  function initTooltips(e) {
    var _e$target8;
    let el =
        (_e$target8 = e == null ? void 0 : e.target) != null
          ? _e$target8
          : document,
      $el = $$1(el);
    $el
      .find(
        "input[data-bs-toggle=tooltip], textarea[data-bs-toggle=tooltip], select[data-bs-toggle=tooltip]"
      )
      .each(function () {
        let $this = $$1(this);
        $this.tooltip({
  ...
  function searchOperatorChange(el) {
    ...
    $cond
      .toggleClass("d-none", hideOpr2)
      .find(":input")
      .prop("disabled", hideOpr2);
  ...
  function initForms(e) {
    var _e$target9;
    let el =
        (_e$target9 = e == null ? void 0 : e.target) != null
          ? _e$target9
          : document,
      ids = ew.forms.ids();
    for (let id of ids) {
      if (
        (el.matches != null && el.matches("#" + id)) ||
        el.querySelector("#" + id)
      )
        forms.get(id).init();
    }
  ...
  function _alert(msg, cb, type) {
    ...
    if (args.show)
      return Swal.fire(args.config).then((result) =>
        isFunction$2(cb) ? cb(result.isConfirmed) : result
      );
  }
  ...
  function _prompt(cfg, cb) {
    ...
    if (config.input)
      // Prompt
      (_config$inputValidato = (_config = config).inputValidator) != null
        ? _config$inputValidato
        : (_config.inputValidator = (value) => {
            if (!value) return ew.language.phrase("EnterValue");
          });
    return Swal.fire(config).then((result) =>
      isFunction$2(cb) ? cb(result.value) : result
    );
  }
  ...
  function toast(options) {
    ...
    var position = options.position,
      $container = $$1(
        "#toastsContainer" + position[0].toUpperCase() + position.substring(1)
      );
  ...
  function getForm(el) {
    ...
    if (!$f[0])
      ...
      $f = $el
        .closest(".ew-grid, .ew-multi-column-grid, .modal")
        .find(".ew-form");
    return $f[0];
  }
  ...
  function hasFormData(form) {
    var selector =
        "[name^=x_],[name^=y_],[name^=z_],[name^=w_],[name=" +
        ew.TABLE_BASIC_SEARCH +
        "]",
  ...
  function setSearchType(el) {
    var val = el.dataset.searchType,
      phraseId = "Auto";
    if (val == "=") phraseId = "Exact";
    else if (val == "AND") phraseId = "All";
    else if (val == "OR") phraseId = "Any";
    el
      .closest(".ew-basic-search")
      .querySelector("input.ew-basic-search-type").value = val || "";
  ...
  function updateOptions(obj, parentId, async, change) {
    ...
    var f = this.$element
      ? this.$element[0]
      : this.form || ((_obj = obj) == null ? void 0 : _obj.form); // Get form/div element from this/obj
    if (!f) return;
    ...
    if (Array.isArray(obj) && $$1.isString(obj[0])) {
          ...
          $el.each(function () {
            if (!names.includes(this.name)) {
              ...
              var $elf = $el.filter("[name='" + this.name + "']"),
                typ = $elf.attr("type"),
                elf = ["radio", "checkbox"].includes(typ)
                  ? $elf.get()
                  : $elf[0];
              batch.add(
                _updateOptions.bind(self, elf, parentId, async, change)
              );
            }
          });
        } else {
      ...
      if (
        Array.isArray(list == null ? void 0 : list.autoFillTargetFields) &&
        list.autoFillTargetFields[0]
      )
        // AutoFill
  ...
  function _updateOptions(obj, parentId, async, change) {
    ...
    var _updateSibling = function () {
      ...
      if (
        /(srch|search|summary|crosstab)$/.test(fo.id) &&
        prefix == "x" &&
        !rowindex
      ) {
        // Search form
    ...
    if (
      !$$1.isBoolean(async) ||
      (Array.isArray(list.lookupOptions) && list.lookupOptions.length > 0)
    ) {
      ...
      if (isAutoSuggest(obj) && self.htmlForm)
        ...
        data["v0"] = ar[0]
          ? obj.multiple
            ? ar.join(ew.MULTIPLE_OPTION_SEPARATOR)
            : ar[0]
          : random(); // Filter by the current value
  ...
  function getQueryBuilderFilterInput() {
    return function (rule, name) {
      ...
      let tblVar = this.status.id.replace(/_query_builder$/, ""),
        ...
        span =
          (_document$getElementB = document.getElementById(tmplId)) == null ||
          (_document$getElementB = _document$getElementB.content) == null
            ? void 0
            : _document$getElementB
                .cloneNode(true)
                .querySelector(".ew-search-field"),
        html =
          (_span$innerHTML$repla =
            span == null
              ? void 0
              : span.innerHTML
                  .replace(/\binvalid-feedback\b/, ew.queryBuilderErrorClass)
                  .replace(
                    new RegExp(
                      '(data-field=")?(\\b|_)x_' + fldVar + "(\\[\\]|\\b)",
                      "g"
                    ),
                    ($0, $1, $2) => ($1 ? $0 : $2 + name)
                  )) != null
            ? _span$innerHTML$repla
            : ""; // Keep "data-field" attribute
  ...
  function getQueryBuilderValueSetter() {
    return function (rule, value) {
      let input = rule.$el
          ...
          .find(
            "select, selection-list, input[name^=sv_], input[data-field]"
          )[0],
    ...
    return {
      callback: function (value, rule) {
        ...
        for (let i = 0; i < rule.operator.nb_inputs; i++) {
          $value
            .find("[name='" + rule.id + "_value_" + i + "']")
            .each(function () {
              let $this = $$1(this),
                $invalid = $this.next("." + ew.queryBuilderErrorClass).empty();
              validators == null ||
                validators.forEach == null ||
                validators.forEach((validator) => {
                  let err = validator(this);
                  if ($$1.isObject(err)) {
                    errors++;
                    $invalid.append(
                      "<span>" + Object.values(err)[0] + "</span>"
                    );
                  }
                });
              if ($invalid.html())
                $this
                  .addClass("is-invalid")
                  .one("click", () => $this.removeClass("is-invalid"));
            });
        }
  ...
  function applyTemplate(
    ...
    if (args.enabled) {
      ...
      let parent = isModal
          ? document.querySelector("#ew-modal-dialog.show")
          : document.body,
        // Parent (Modal dialog or document)
        template =
          parent == null ||
          (_parent$querySelector = parent.querySelector("#" + tmplId)) == null
            ? void 0
            : _parent$querySelector.content;
      if (!template) return;
        ...
        if (subtmpl != null && subtmpl.content) {
          if (el.dataset.rowspan > 1)
            Array.prototype.slice
              .call(subtmpl.content.childNodes)
              .forEach((node) => (node.rowSpan = el.dataset.rowspan));
      ...
      if ($$1.views) {
        ...
        if (!hasTag) {
          let selector = ew.jsRenderAttributes
            .map((attr) => "[" + attr + "*='{{'][" + attr + "*='}}']")
            .join(",");
        ...
        if (hasTag) {
          ...
          let html = div.innerHTML.replace(/{{([^}]+)}}/g, (m) =>
              htmlDecode(m)
            ),
            // HTML-decode comparison operators
            tmpl = $$1.templates(html);
          parent.querySelector("#" + divId).innerHTML = tmpl.render(
            args.data,
            ew.jsRenderHelpers
          );
  ...
  function exportCustom(divId, exportType, tblVar) {
    ...
    if (
      (_div$children$ = div.children[0]) != null &&
      _div$children$.matches("div[id^=ct_]")
    )
    ...
    if (exportType == "email")
      exportEmail(
        tblVar,
        mergeSearchParams(currentUrl.searchParams, data)
      ); // Merge data with email form data in current URL parameters
    else
      fileDownload(getApiUrl([ew.API_EXPORT_ACTION, exportType, tblVar]), data);
  }
  ...
  function toggleGroup(el) {
    ...
    if (!$tr.data("group")) {
      ...
      $tr
        .closest("table")
        .find("tbody")
        .find(".ew-rpt-grp-field-1 > .ew-group-toggle")
        .each(function () {
          toggleGroup(this);
      ...
      if ($el.hasClass("ew-rpt-grp-hide")) {
        ...
        $$1(selector)
          .slice(1)
          .removeClass("ew-rpt-grp-hide-" + level);
        $el.removeClass("ew-rpt-grp-hide");
      } else {
        // Hide
        $$1(selector)
          .slice(1)
          .addClass("ew-rpt-grp-hide-" + level);
  ...
  function convertToBool(value) {
    return (
      value && ["1", "y", "t", "true"].includes(String(value).toLowerCase())
    );
  }
  ...
  function valueChanged(fobj, infix, fld, bool) {
    ...
    if (oldEl && el) {
      if (bool) {
        if (
          convertToBool(getOptionValue(oldEl)) ===
          convertToBool(getOptionValue(el))
        )
          return false;
      } else {
  ...
  function submitAction(e, args) {
    ...
    let el = e.currentTarget,
      ...
      isMultiple =
        (!args.select && !args.key) ||
        (args.select && sameText(args.select[0], "m"));
    ...
    let _success = function (result) {
      if ($$1.isString(result)) {
        ...
        else if ($$1.isString(result.successMessage))
          _alert(result.successMessage, "success");
    ...
    let _submit = function (value) {
      if (isPostBack && f) {
        ...
        settings.success ||
          (settings.success = isFunction$2(success) ? success : _success);
        $$1.ajax(url, settings);
      }
    };
    msg = $$1.isString(msg)
      ? (_window$msg = window[msg]) != null
        ? _window$msg
        : msg
      : msg; // Get config object if available
    msg
      ? _prompt(msg).then(
          (result) => result.isDismissed || _submit(result.value)
        )
      : _submit();
    return false;
  }
  ...
  async function _export(args) {
    let { evt, url, export: type, custom, exportSelected, emailForm } = args,
      f = evt.currentTarget.form;
    exportSelected &&
      (exportSelected =
        f && !!f.querySelector("input[type=checkbox][name='key_m[]']"));
    ...
    let $f = f
        ? $$1(f)
        : $$1('<form class="ew-export-form" method="post"></form>').appendTo(
            $body
          ),
    ...
    try {
      if (custom) {
        ...
        url = setSearchParams(
          ...
          $$1(emailForm).serialize()
        ); // Make sure parameters include "export=type&custom=1"
        (_document$querySelect = document.querySelector(
          "iframe[name='" + name + "']"
        )) == null || _document$querySelect.remove();
        let $iframe = $$1("<iframe>")
          .attr("name", name)
          .addClass("d-none")
          .appendTo($body);
        ...
        postParams.forEach((_ref2) => {
          ...
          return (
            $f.find('input[name="' + name + '"]').val(value)[0] ||
            $$1("<input>")
              .attr({
                type: "hidden",
                name,
        ...
        $iframe
          .one("load", () => (document.body.style.cursor = "default"))
          .one("load", () =>
            $f.attr({
              action,
              target,
        ...
        if (type == "print")
          $f.attr("action", url).trigger("submit"); // Submit the form directly
        else
          fileDownload(
            url,
            mergeSearchParams($f.serialize(), getchartParams())
          );
      }
    } finally {
      f || $f.remove();
  ...
  function removeSpaces(value) {
    return /^(<(p|br)\/?>(&nbsp;)?(<\/p>)?)?$/i.test(value.replace(/\s/g, ""))
      ? ""
      : value;
  }
  ...
  function isModalLookup(el) {
    var _el$dataset;
    return el == null || (_el$dataset = el.dataset) == null
      ? void 0
      : _el$dataset.modalLookup;
  }
  ...
  function isFilter(el) {
    var _el$dataset2;
    return el == null || (_el$dataset2 = el.dataset) == null
      ? void 0
      : _el$dataset2.filter;
  }
  ...
  function isSelect2(el) {
    return el == null || el.matches == null
      ? void 0
      : el.matches("select.select2-hidden-accessible");
  }
  ...
  function isNativeSelectOne(el) {
    return (
      (el == null || el.matches == null
        ? void 0
        : el.matches("select:not(.select2-hidden-accessible)")) &&
      el.type == "select-one"
    );
  }
  ...
  function isBooleanCheckbox(el) {
    return el == null || el.matches == null
      ? void 0
      : el.matches("input[type=checkbox][data-boolean]");
  }
  ...
  function onError(frm, el, msg, focus) {
    ...
    $$1(el)
      .closest(fieldContainerSelector)
  ...
  function setInvalid(obj) {
    ...
    if (isAutoSuggest(obj)) {
      $p.find(".ew-auto-suggest")
        .removeClass("is-valid")
        .addClass("is-invalid")
        .one("click keydown paste", reset);
      ...
      $obj
        .removeClass("is-valid")
        .addClass("is-invalid")
        .one("select2:open", reset);
    } else {
      if (["checkbox", "radio"].includes(obj.type)) {
        $obj
          .removeClass("is-valid")
          .addClass("is-invalid")
          .one("click keydown", reset);
      } else {
        let events =
          ew.MOBILE_DETECT.version("Safari") &&
          !ew.MOBILE_DETECT.version("Chrome")
            ? "change keydown paste"
            : "click keydown paste"; // Use "change" event for Safari
        $obj.parent().one(events, reset);
        $obj
          .add($obj.closest(".input-group"))
          .removeClass("is-valid")
          .addClass("is-invalid");
      }
    }
  ...
  function sort(e, dataset) {
    let { sortUrl: url, sortType: type, context, ajax } = dataset;
    ...
    if (convertToBool(ajax)) refresh(fetch(setLayout(url, false)), context);
    else window.location = sanitizeUrl(url);
    return false;
  }
  ...
  function filter(e) {
    ...
    $$1(
      "select[data-select2-id='f" + data.table + "srch_" + data.field + "']"
    ).select2("open");
  }
  ...
  async function inlineAction(args) {
    var _args$evt;
    let $el = $$1(
        (_args$evt = args.evt) == null ? void 0 : _args$evt.currentTarget
      ),
      $grid = $el.closest(".ew-grid, .ew-multi-column-grid");
    $el.tooltip("hide");
    if (!$grid[0] && $el.is(".dropdown-item")) {
      let $btn = $$1(
        "#" + $el.closest(".dropdown-menu").attr("aria-labelledby")
      );
      if ($btn[0]) {
        $el = $btn;
        $grid = $el.closest(".ew-grid, .ew-multi-column-grid");
    ...
    let _disableInlineButtons = () =>
      $$1(".ew-list-options, .ew-list-option-body")
        .find(
          "[data-ew-action='inline'][data-action='add'],[data-ew-action='inline'][data-action='copy'],[data-ew-action='inline'][data-action='edit']"
        )
        .addClass("disabled");

    // Enable inline buttons
    let _enableInlineButtons = () =>
      $$1(".ew-list-options, .ew-list-option-body")
        .find(
          "[data-ew-action='inline'][data-action='add'],[data-ew-action='inline'][data-action='copy'],[data-ew-action='inline'][data-action='edit']"
        )
    ...
    let _updateRowIndexes = (tbl) => {
      let j = 0;
      Array.from(tbl.rows).forEach(
        (row) =>
          (row.dataset.rowindex =
            row.dataset.rowindex === "0" ? "0" : String(j++))
      ); // Update row index if not Inline-Add/Copy row
    };
    ...
    let _success = async function (response) {
      ...
      let text = await response.clone().text(),
        ...
        $row = $data
          .find(
            "tr[data-rowindex=0], div[data-rowindex=0]" +
              (key
                ? ", tr[data-key='" + key + "'], div[data-key='" + key + "']"
                : "")
          )
          .filter("[data-rowtype=2], [data-rowtype=3]"),
        // Find Inline-Add first
        inline = $row.data("inline"),
        rowtype = $row.data("rowtype"),
        rowindex =
          inline && rowtype == 2 /* Inline-Add */
            ? 0
            : inline && rowtype == 3 /* Inline-Edit */
            ? 1
            : $row.data("rowindex"),
      ...
      if (["add", "copy"].includes(action)) {
        // Inline-Add/Copy
        if (
          document.querySelector("main.ew-no-record") &&
          $data.find("main:not(.ew-no-record)")[0]
        ) {
          ...
          if ($row.is("TR")) {
            // Table
            let tbl = _getTable();
            if (tbl.matches(".ew-infinite-scroll-table"))
              $$1(tbl.tBodies).last().append($els);
            else $$1(tbl.tBodies).last().prepend($els);
    ...
    let _submitSuccess = async function (response) {
      ...
      if (response.headers.get("Infinite-Scroll")) {
        ...
        if (["insert", "update"].includes(action)) {
          $row = $data.find(
            "tr[data-rowindex][data-rowtype=" +
              ew.ROWTYPE_VIEW +
              "], div[data-rowindex][data-rowtype=" +
              ew.ROWTYPE_VIEW +
              "]"
          ); // Row(s) with ROWTYPE_VIEW
          ...
          if (action == "insert")
            ...
            $grid
              ...
              .replaceWith($row[0]); // Replace add row
          ...
          if (["edit", "multiedit"].includes(action))
            ...
            // Grid-Add
            $grid
              .find(".ew-grid-middle-panel .ew-table > tbody:last")
              .append($row); // Append to bottom
        }
        let tbl = _getTable();
        _updateRowIndexes(tbl); // Update row index
        setupTable(tbl, true);
      ...
      if (
        returnUrl &&
        ["inline", "modal"].includes(args.ewAction) &&
        !samePath(parseUrl(returnUrl).pathname, currentUrl.pathname)
      )
        $grid
          .find(
            "[data-ew-action=modal][data-url='" +
              returnUrl +
              "'], [data-ew-action=inline][data-url='" +
              returnUrl +
              "']"
          )
          .trigger("click");
    ...
    if (ewAction == "inline") {
      if (action == "cancel") {
        if (curindex == "0") {
          ...
          var _$grid$data;
          // Cancel Inline-Edit
          (_$grid$data = $grid.data("detached")) == null ||
            _$grid$data.replaceAll($record);
        }
        ...
        if (!key) {
          ...
          $record = $grid
            .find("input[type=checkbox][name='key_m[]']:checked")
            .closest("tr[data-rowindex], div[data-rowindex]"); // Update $record
          if (!$record[0]) {
            _alert(ew.language.phrase("NoRecordSelected"));
            return false;
          }
      ...
      if (
        ((ewAction == "grid" && action == "edit") ||
          (ewAction == "modal" && ["multiedit", "update"].includes(action))) &&
        !keySelected(f)
      ) {
  ...
  function keySelected(f) {
    return !!(
      f != null &&
      f.querySelector("input[type=checkbox][name='key_m[]']:checked")
    );
  }
    ...
    $$1(tbl.tBodies).each(function () {
      $$1(this.rows).each(function () {
        ...
        if ($r.is(":not(.ew-template):not(.ew-table-preview-row)"))
          $r.toggleClass(
            "table-active ew-table-selected-row",
            cb.checked
    ...
    $$1(cb.form.elements)
      .filter(
        "input[type=checkbox][name^=" +
          cb.name +
          "_], [type=checkbox][name=" +
          cb.name +
          "]"
      )
      .not(cb)
      .not(":disabled")
      .prop("checked", cb.checked);
  }
  ...
  function updateSelected(f) {
    return !!(
      f != null &&
      f.querySelector(
        "input[type=checkbox][name^=u_]:checked,input[type=hidden][name^=u_][value='1']"
      )
    );
  }
  ...
  function clearSelected(tbl) {
    let rowIndexes = $$1(tbl)
      .find("input[type=checkbox][name='key_m[]']:checked")
      .closest("[data-rowindex]")
      .map((i, r) => r.dataset.rowindex)
      .get();
    $$1(tbl == null ? void 0 : tbl.rows)
      .filter(
        (i, r) =>
          r.classList.contains("table-active") &&
          !rowIndexes.includes(r.dataset.rowindex)
      )
      .removeClass("table-active ew-table-selected-row")
      .triggerHandler("change");
  }
  ...
  function clearDelete(el) {
    ...
    $tr
      .siblings("[data-rowindex='" + $tr.data("rowindex") + "']")
      .addBack()
  ...
  function selectKey(e) {
    ...
    $tr
      ...
      .each(function () {
        $$1(this)
          .toggleClass("table-active ew-table-selected-row", chk.checked)
          .triggerHandler("change");
      });
    e.stopPropagation();
  }
  ...
  function setupTable() {
    ...
    if (
      !tbl ||
      !tbl.rows ||
      (!force && tbl.dataset.isset) ||
      tbl.tBodies.length == 0
    )
      return;
    ...
    let click = function (e) {
      ...
      if (
        !tbl ||
        $target.hasClass("btn") ||
        $target.hasClass("ew-preview-btn") ||
        $target.is(":input")
      )
        return;
      clearSelected(tbl); // Clear all other selected rows
      $this
        .siblings("[data-rowindex='" + $this.data("rowindex") + "']")
        .addBack()
        .toggleClass("table-active")
        .triggerHandler("change");
    };
    let n =
        $rows.filter("[data-rowindex=1]").length ||
        $rows.filter("[data-rowindex=0]").length ||
        1,
      ...
      rows = $rows
        ...
        .each(function () {
          $$1(this.cells)
            .removeClass("ew-table-last-row")
            .last()
            .addClass("ew-table-last-col"); // Cell of last column
        })
        .get();
    if (rows.length >= n) {
      let div = $tbl.parentsUntil(
        ".ew-grid",
        "." + ew.RESPONSIVE_TABLE_CLASS
      )[0];
      rows[rows.length - 1].classList.add("border-bottom-0"); // Last row
      rows.splice(n * -1).forEach((row, i) =>
        Array.from(row.cells)
          .filter((cell) => cell.rowSpan == i + 1) // Cell of last row
          .forEach((cell) => {
            cell.classList.add("ew-table-last-row");
            cell.classList.toggle(
              "ew-table-border-bottom",
              (div == null ? void 0 : div.clientHeight) > tbl.offsetHeight
            );
          })
      );
    }
    let form = tbl.closest("form"),
      attach =
        form &&
        $$1(form.elements).filter("input#action:not([value^=grid])").length > 0;
    $$1(tbl.tBodies[tbl.tBodies.length - 1].rows) // Use last TBODY (avoid Opera bug)
      ...
      .each(function () {
        ...
        if (attach && !$r.data("isset"))
          $r.on("click", click).data("isset", true);
      });
  ...
  function setupGrid(grid, force) {
    ...
    let rowcnt = $grid
      .find("table.ew-table > tbody")
      .first()
      .children("tr:not(.ew-table-preview-row, .ew-template)").length;
    if (
      rowcnt == 0 &&
      !grid.querySelector(".ew-grid-upper-panel, .ew-grid-lower-panel")
    )
      $grid.hide();
    if (
      $grid
        .find(".ew-grid-middle-panel:visible")
        .hasClass(ew.RESPONSIVE_TABLE_CLASS) &&
      $grid.width() > $$1(".content").width()
    ) {
    ...
    if (
      $grid.find(".ew-grid-middle-panel:visible") &&
      (!grid.querySelector(".ew-grid-upper-panel") ||
        $grid.find(".ew-grid-upper-panel:hidden")[0])
    ) {
      let $panel = $grid
          .find(".ew-grid-middle-panel:not(.overflow-y-auto):visible")
          .addClass("rounded-top"),
        $cells = $panel
          .children(".ew-table:not(.table-head-fixed):visible")
          .first()
          .children("thead:visible")
          .first()
          .children("tr:visible")
          .first()
          .children("th:visible");
    ...
    if (
      $grid.find(".ew-grid-middle-panel:visible") &&
      (!grid.querySelector(".ew-grid-lower-panel") ||
        $grid.find(".ew-grid-lower-panel:hidden")[0])
    ) {
      let $panel = $grid
          .find(".ew-grid-middle-panel:not(.overflow-y-auto):visible")
  ...
  function addGridRow(el) {
    ...
    let $row = $tpl.clone(true, true).removeClass("ew-template"),
      form =
        grid.querySelector("div.ew-form[id^=f][id$=grid]") ||
        grid.querySelector("form.ew-form[id^=f][id$=list]") ||
        grid.querySelector("form.ew-form[id^=f][id$=grid]"),
      $form = $$1(form),
      suffix = $form.is("div") ? "_" + form.id : "",
      $keycnt = $form.find("#" + ew.FORM_KEY_COUNT_NAME + suffix),
      keycnt = parseInt($keycnt.val(), 10) + 1,
      name =
        ew.FORM_ROW_ACTION_NAME.replace(/^k_/, "k" + keycnt + "_") + suffix,
      // name="k<n>_action"
      $els = $tpl
        .find("script:contains('$rowindex$')")
        .add("tr.ew-template + script[data-rowindex='$rowindex$']"); // Get scripts with rowindex
    $row.attr({
    ...
    (_bootstrap$Tooltip$ge2 = bootstrap.Tooltip.getInstance($btn[0])) == null ||
      _bootstrap$Tooltip$ge2.dispose();
    $btn.tooltip({
      ...ew.tooltipOptions,
      container: "body",
      trigger: "hover",
    });
    $keycnt.val(keycnt).after(
      $$1("<input>").attr({
        type: "hidden",
        id: name,
        name: name,
    ...
    $els
      .get()
      .forEach((el) => addScript(el.text.replace(/\$rowindex\$/g, keycnt)));
  ...
  function deleteGridRow(el, infix) {
    ...
    (_bootstrap$Tooltip$ge3 = bootstrap.Tooltip.getInstance(el)) == null ||
      _bootstrap$Tooltip$ge3.dispose();
    let grid = el.closest(".ew-grid, .ew-multi-column-grid"),
      ...
      form =
        ...
        (grid == null
          ? void 0
          : grid.querySelector(
              "form.ew-form[id^=f][id$=list], form.ew-form[id^=f][id$=grid]"
            )),
      $form = $$1(form),
      frm = $form.data("form");
  ...
  function getElements(el, root) {
    ...
    let $root = root ? $$1(root) : $document$1,
      ...
      getFieldSelector = (field) =>
        "[data-field='" +
        field +
        "']:not(.ew-custom-option):not([name^=y_]):not([data-old])";
    if (el instanceof HTMLElement && el.dataset.table && el.dataset.field) {
      ...
      selectors = [
        "[data-table='" +
          el.dataset.table +
          "']" +
          getFieldSelector(el.dataset.field),
      ];
    } else if ($$1.isString(el)) {
      let [s1, s2] = el.split(" "); // Check if "<table> <name>"
      selectors = s2
        ? ["[data-table='" + s1 + "']" + getFieldSelector(getId(s2))] // "<table> <name>", remove []
        : [
            "[name='" + el + "']:not(.ew-custom-option):not([data-old])",
            getFieldSelector(getId(el)),
          ]; // "<name>" only
    }
    let selector = ["input", "select", "textarea", "button", "selection-list"]
        .flatMap((tag) => selectors.map((sel) => tag + sel))
        .join(),
    ...
    if (
      ($els.length == 1 &&
        ($els.is(":not([type=checkbox]):not([type=radio])") ||
          isBooleanCheckbox($els[0]))) ||
      ($els.length == 2 &&
        $els.eq(0).is("selection-list") &&
        $els.eq(1).is("input[type=hidden]"))
    )
  ...
  function getAncestorBy(node, fn) {
    while ((node = node.parentNode)) {
      var _node;
      if (
        ((_node = node) == null ? void 0 : _node.nodeType) == 1 &&
        (!fn || fn(node))
      )
        return node;
    }
  ...
  function isHidden(el) {
    ...
    return (
      ($el.css("display") == "none" &&
        !$el.is("selection-list") &&
        !$el.closest(".dropdown-menu")[0] &&
        !isModalLookup(el) &&
        !isAutoSuggest(el) &&
        !isHiddenTextArea(el)) ||
      getAncestorBy(
  ...
  function samePath(p1, p2) {
    ...
    if (
      p1 != p2 &&
      p1.startsWith(ew.PATH_BASE) &&
      p2.startsWith(ew.PATH_BASE)
    ) {
      ...
      let ar1 = p1.split("/"),
        ar2 = p2.split("/");
      if (
        (ar1[0] == ar2[0] && ar1[0].match(/(list|view|edit)$/i)) ||
        // *list == *list
        (ar1[0] == ar2[0] &&
          ar1[1] == ar2[1] &&
          [ew.LIST_ACTION, ew.VIEW_ACTION, ew.EDIT_ACTION].includes(ar1[1]))
      )
        // xxx/list == xxx/list
        return true;
    }
  ...
  function getValue(el, form) {
    ...
    if ($$1.isString(el))
      obj =
        el.split(" ").length == 2 // Parent field in master table
          ? getElements(el)
          : getElements(el, form);
  ...
  function getOptionTexts(el, form) {
    ...
    if (isAutoSuggest(obj)) {
      ...
      // Selection list
      return isSelect2(obj)
        ? $$1(obj)
            .find(":selected")
            .map((i, el) => el.innerHTML)
            .get()
        : Array.from(obj.options)
            .filter((option) => option.selected && option.value !== "")
  ...
  function displayValue(opt, obj) {
    return [opt.df, opt.df2, opt.df3, opt.df4].reduce((text, value, i) => {
      ...
      return !$$1.isUndefined(sep) && $$1.isValue(value) && value != ""
        ? text + sep + value
        : text;
    }, "");
  }
  ...
  function newOption(obj, data, f, selected) {
    let frm = forms.get(f.id),
      ...
      text =
        template != null && template.render && !isAutoSuggest(obj)
          ? template.render(data, ew.jsRenderHelpers)
          : displayValue(data, obj) || value;
    if (obj.options) {
      // Selection list
      let option;
      if (obj.list) {
      ...
      if (obj.list) {
        ...
        if (args.group && !isSelect2(obj)) {
          ...
          let optGroup = Array.from(
            (_obj$querySelector = obj.querySelector(":scope > optgroup")) !=
              null
              ? _obj$querySelector
              : []
          ).find((g) => g.label == args.group);
          if (!optGroup) {
  ...
  function selectOption(obj, values, change) {
    ...
    if (obj.options) {
      ...
      if (obj.list) {
        ...
        if (
          obj.type == "select-one" &&
          obj.selectedIndex == -1 &&
          !((_obj$options$ = obj.options[0]) != null && _obj$options$.value)
        )
          obj.selectedIndex = 0; // Make sure an option is selected
      }
    ...
    if (
      ((_obj$dataset = obj.dataset) == null
        ? void 0
        : _obj$dataset.autoselect) === "false"
    )
    ...
    let form = getForm(obj),
      autoSelect =
        form &&
        !(
          form.id.endsWith("search") ||
          form.id.endsWith("srch") ||
          ((_forms$get$getList = forms.get(form.id).getList(obj)) == null ||
          (_forms$get$getList = _forms$get$getList.parentFields) == null
            ? void 0
            : _forms$get$getList.length) === 0
        ); // Not search forms and has parent fields
  ...
  function _fetch(url, init) {
    ...
    if (isApi && ew.API_JWT_TOKEN)
      init.headers.set(
        ew.API_JWT_AUTHORIZATION_HEADER,
        "Bearer " + ew.API_JWT_TOKEN
      );
    if (
      (_init$headers = init.headers) != null &&
      (_init$headers = _init$headers.get("Content-Type")) != null &&
      _init$headers.includes("application/json")
    ) {
      ...
      init.headers.set(
        headerCase(ew.ANTIFORGERY_TOKEN_KEY),
        ew.ANTIFORGERY_TOKEN
      ); // Send antiforgery token in header // PHP
    }
  ...
  $document$1.on("ajaxSend", function (event, jqxhr, settings) {
    let url = settings.url,
      ...
      // Is API request
      allowed =
        isApi || url.startsWith(ew.PATH_BASE) || url.startsWith(currentPage());
    ...
    if (allowed) {
      if (isApi && ew.API_JWT_TOKEN)
        jqxhr.setRequestHeader(
          ew.API_JWT_AUTHORIZATION_HEADER,
          "Bearer " + ew.API_JWT_TOKEN
        ); // Note: settings.headers won't work
      if (
        $$1.isString(settings.contentType) &&
        settings.contentType.includes("application/json")
      ) {
  ...
  function executeScript(html, id, modal) {
    let pattern = modal
        ? /<(head)[^>]*>[\s\S]*?<\/\1\s*>/gi
        : /<(head|template)[^>]*>[\s\S]*?<\/\1\s*>/gi,
      // If modal, scripts in template tags not executed by browser, execute manually
      matches = html
        .replaceAll(pattern, "")
        .matchAll(/<script[^>]*>[\s\S]*?<\/script\s*>/gi); // Do not execute scripts in template tags, use non-greedy [\s\S]*?
    Array.from(
      document
        .createRange()
        .createContextualFragment(
          Array.from(matches)
            .map((m) => m[0])
            .filter((s) => !s.includes("$rowindex$"))
            .join("")
        )
        .querySelectorAll("script:not([type]), script[type='text/javascript']")
    )
      .sort((s1, s2) =>
        s2.classList.contains("ew-apply-template")
          ? 1
          : s1.classList.contains("ew-apply-template")
          ? -1
          : 0
      ) // Execute custom template first
  ...
  function stripScript(html) {
    let matches = html
      .replaceAll(/<(head|template)[^>]*>[\s\S]*?<\/\1\s*>/gi, "")
      .matchAll(/<script([^>]*)>([\s\S]*?)<\/script\s*>/gi); // Do not strip scripts in template tags, use non-greedy [\s\S]*?
    return Array.from(matches)
      .filter((m) =>
        document
          .createRange()
          .createContextualFragment(m[0])
          .querySelector("script:not([type]), script[type='text/javascript']")
      )
      .reduce((html, m) => html.replace(m[0], ""), html);
  }

  // Add SCRIPT tag
  function addScript(text, id) {
    let scr =
      text instanceof HTMLScriptElement
        ? text
  ...
  function getOptions(obj) {
    return obj != null && obj.options
      ? Array.prototype.map.call(obj.options, (opt) => [opt.value, opt.text])
      : [];
  }
  ...
  function enable2FA() {
    ...
    if (sameText(ew.TWO_FACTOR_AUTHENTICATION_TYPE, "google")) {
            ...
            _prompt(
              {
                ...
                preConfirm: (value) => {
                  return $$1
                    .get(
                      getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_VERIFY, value])
                    )
                    .then((result) => {
                      if ((result == null ? void 0 : result.success) !== true)
                        throw new Error(
                          ew.language.phrase("2FAVerificationFailed")
                        );
                      return result;
                    })
      ...
      // Send OTP and Verify
      let url = getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_SHOW]);
      let html = sameText(ew.TWO_FACTOR_AUTHENTICATION_TYPE, "email")
        ? ew.language.phrase("EnterOTPEmailAddress")
        : ew.language.phrase("EnterOTPMobileNumber");
      $$1
        .get(url, (result) => {
          var _result$error7;
          if (result.success) {
            _prompt(
              {
                ...
                preConfirm: (value) => {
                  return $$1
                    .get(
                      getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_SEND_OTP, value])
                    )
                    .then((result) => {
                      if ((result == null ? void 0 : result.success) !== true)
                        throw new Error(
                          ew.language
                            .phrase("SendOTPFailed")
                            .replace("%e", getError(result))
                        );
                      return result;
                    })
                    .catch((error) => Swal.showValidationMessage(error));
              ...
              (result) => {
                ...
                if (
                  result != null &&
                  (_result$error5 = result.error) != null &&
                  _result$error5.description
                ) {
                  showToast(result.error.description);
                } else if (result != null && result.success) {
                  // Verify OTP
                  _prompt(
                    {
                      ...
                      preConfirm: (value) => {
                        return $$1
                          ...
                          .then((result) => {
                            if (
                              (result == null ? void 0 : result.success) !==
                              true
                            )
                              throw new Error(
                                ew.language.phrase("2FAVerificationFailed")
                    ...
                    (result) => {
                      ...
                      if (
                        result != null &&
                        (_result$error6 = result.error) != null &&
                        _result$error6.description
                      ) {
                        showToast(result.error.description);
                      } else if (result != null && result.success) {
                        showToast(ew.language.phrase("2FAEnabled"), "success");
                        $$1("#enable-2fa").addClass("d-none");
                        $$1("#disable-2fa, #backup-codes").removeClass(
                          "d-none"
                        );
                      }
                    }
                  ).catch((err) =>
                    showToast(err == null ? void 0 : err.message)
                  );
                }
              }
            ).catch((err) => showToast(err == null ? void 0 : err.message));
            ...
            result != null &&
            (_result$error7 = result.error) != null &&
            _result$error7.description
          ) {
            _alert(result.error.description);
          }
        })
    ...
    _prompt(
      {
        ...
        preConfirm: (value) => {
          if (value)
            return $$1
              .get(getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_RESET]))
              .then((result) => {
                var _result$error8;
                if (
                  result != null &&
                  (_result$error8 = result.error) != null &&
                  _result$error8.description
                )
                  showToast(result.error.description);
      ...
      (result) => {
        ...
        if (
          result != null &&
          (_result$error9 = result.error) != null &&
          _result$error9.description
        ) {
          showToast(result.error.description);
        } else if (result != null && result.success) {
  ...
  function toggleChat(value) {
    document.body.style.cursor = "wait";
    $$1
      .get(getApiUrl([ew.API_CHAT_ACTION, value]), (result) => {
        var _result$error10;
        // value should be 1 or 0
        if (
          result != null &&
          (_result$error10 = result.error) != null &&
          _result$error10.description
        ) {
          _alert(result.error.description);
        } else if (result != null && result.success) {
      ...
      .fail((jqXHR, textStatus, errorThrown) => _alert(errorThrown))
      .always(() => (document.body.style.cursor = "default"));
  }
    ...
    _prompt(
      {
        ...
        preConfirm: (value) => {
          let url = account
            ? getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_SEND_OTP, account])
            : getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_SEND_OTP]);
          if (value)
            return $$1
              .get(url)
              .then((result) => {
                ...
                if (
                  result != null &&
                  (_result$error11 = result.error) != null &&
                  _result$error11.description
                )
                  showToast(result.error.description);
                else if ((result == null ? void 0 : result.success) !== true)
                  showToast(ew.language.phrase("SendOTPFailed"));
                return result;
              })
              .fail((jqXHR, textStatus, errorThrown) => showToast(errorThrown));
        },
      ...
      (result) => {
        ...
        if (
          result != null &&
          (_result$error12 = result.error) != null &&
          _result$error12.description
        ) {
          showToast(result.error.description);
        } else if (result != null && result.success) {
  ...
  function showBackupCodes() {
    ...
    return _alert({
      ...
      willOpen: () => {
        ...
        Swal.disableButtons();
        $$1
          .get(getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_BACKUP_CODES]))
          .then((result) => {
            if (result.success && Array.isArray(result.codes)) {
              Swal.update({
                html:
                  html +
                  '<textarea class="form-control ew-backup-codes" readonly>' +
                  result.codes.join("\n") +
                  "</textarea>",
              });
              Swal.enableButtons();
      ...
      preConfirm: () => {
        let codes = copyToClipboard(
          Swal.getHtmlContainer().querySelector("textarea")
        );
        if (codes)
          showToast(ew.language.phrase("CopiedToClipboard"), "success");
        return false; // Keep the alert open
      },
      preDeny: async () => {
        Swal.showLoading(Swal.getDenyButton());
        await $$1
          .get(getApiUrl([ew.API_2FA_ACTION, ew.API_2FA_NEW_BACKUP_CODES]))
          .then((result) => {
            if (result.success && Array.isArray(result.codes)) {
              Swal.update({
                html:
  ...
  function addOptionDialogShow(args) {
    ...
    args.lnk =
      args.lnk ||
      ((_args$evt2 = args.evt) == null ? void 0 : _args$evt2.currentTarget);
    ...
    var $dlg =
      ew.addOptionDialog ||
      $$1("#ew-add-opt-dialog").on("hidden.bs.modal", _hide);
    if (!$dlg[0]) {
    ...
    var _submitSuccess = function (data) {
      ...
      var results = data,
        ...
        // form object
        objName = $dlg
          .find(".modal-body form input[name='" + ew.API_OBJECT_NAME + "']")
          .val(),
      ...
      if (
        ...
        if ((_results2 = results) != null && _results2.error) {
          ...
          if ($div[0]) {
            ...
            var _results3;
            msg =
              ((_results3 = results) == null
                ? void 0
                : _results3.failureMessage) || data;
      ...
      if (await frm.canSubmit(e)) {
        ...
        $$1
          .post(
            getApiUrl([
              ew.API_ADD_ACTION,
              form.elements[ew.API_OBJECT_NAME].value,
            ]),
            $$1(form).serialize(),
            _submitSuccess
          )
    ...
    var success = function (data) {
      ...
      var list = frm.getList(args.el),
        pfs = list.parentFields
          .slice() // Clone
          .map((pf) =>
            pf.split(" ").length == 1 ? pf.replace(/^x_/, prefix) : pf
          ),
        // Parent field in the same table, add row index
        form = frm.htmlForm,
      ...
      $dlg
        .find(".modal-title")
        .html($$1(args.lnk).closest(".ew-add-opt-btn").data("title"));
  ...
  function modalDialogShow(args) {
    ...
    args.lnk ||
      (args.lnk =
        (_args$evt3 = args.evt) == null ? void 0 : _args$evt3.currentTarget);
    (_bootstrap$Tooltip$ge4 = bootstrap.Tooltip.getInstance(args.lnk)) ==
      null || _bootstrap$Tooltip$ge4.hide();
    let { ewAction, action } = args,
      f =
        args.f ||
        ((_args$evt4 = args.evt) == null ||
        (_args$evt4 = _args$evt4.currentTarget) == null
          ? void 0
          : _args$evt4.form);
    if (
      ((ewAction == "grid" && action == "edit") ||
    ...
    let url =
        args.url ||
        ((_args$evt5 = args.evt) == null ||
        (_args$evt5 = _args$evt5.currentTarget) == null
          ? void 0
          : _args$evt5.dataset.url),
      $dlg =
        ew.modalDialog ||
        $$1("#ew-modal-dialog").on("hidden.bs.modal", modalDialogHide); // div#ew-modal-dialog always exists
    ...
    let _button = function () {
      let args = $dlg.data("args");
      if (args.btn === null) return "";
      else if (args.btn && args.btn != "") return ew.language.phrase(args.btn);
      return _caption();
    };
    ...
    // Close hidden dialog
    let _hide = function () {
      if ($dlg.data("showing") && !$dlg.hasClass("show"))
        $dlg.data("showing", false).trigger("hidden.bs.modal"); // Call modalDialogHide()
    };
    ...
    let handleResult = function (result) {
      ...
      if (error) {
        ...
        (result.success &&
          !isFunction$2(args == null ? void 0 : args.callback)) ||
        $$1.isString(result.successMessage)
      ) {
        ...
        let lnk = args == null ? void 0 : args.lnk,
          btnId =
            lnk == null ||
            (_lnk$closest = lnk.closest(".dropdown-menu[aria-labelledby]")) ==
              null
              ? void 0
              : _lnk$closest.getAttribute("aria-labelledby"),
          btn = btnId ? document.getElementById(btnId) : null,
          tabPane =
            (_lnk$closest2 = lnk == null ? void 0 : lnk.closest(".tab-pane")) !=
            null
              ? _lnk$closest2
              : btn == null
              ? void 0
          ...
          navItem =
            tabPane == null ||
            (_tabPane$parentNode = tabPane.parentNode) == null ||
            (_tabPane$parentNode = _tabPane$parentNode.parentNode) == null
              ? void 0
              : _tabPane$parentNode.querySelector(
                  "[data-bs-toggle='tab'][data-bs-target='#" +
                    tabPane.id +
                    "']:not([tabindex])"
                ); // The active button does not have tabindex="-1
    ...
    let _createButtons = function () {
      let $footer = $dlg.find(".modal-footer").last().empty(),
        ...
        $btn = $dlg
          .find(".card-body button[type=submit]")
          .first()
          .addClass("ew-submit")
          .on("click", _submit); // Find submit button found in card body
      if (!$btn[0]) {
        // No submit button found in card body
        $dlg
          .find(".modal-body .ew-modal-buttons")
          .contents()
          .clone()
          .find("button")
          .each(function () {
              ...
              if (this.dataset.href) {
                $$1(this).on("click", function (e) {
                  ...
                  if (
                    frm != null &&
                    frm.modified &&
                    ew.hasFormData(this.form)
                  ) {
                    ew.prompt(ew.language.phrase("ConfirmCancel"), (result) => {
                      if (result) {
                        if (samePath(this.dataset.href, currentUrl.pathname))
                          $dlg.modal("hide");
                        else redirect(this.dataset.href);
                      }
                    });
                  } else {
                    if (samePath(this.dataset.href, currentUrl.pathname))
        ...
        if (
          !$dlg.find(".card-body button[type=submit], .modal-footer .btn")[0]
        ) {
          // No buttons found
          let btn = _button();
          if (btn) {
            $$1(
              ...
              .appendTo($footer);
            $footer.append(
              '<button type="button" class="btn btn-default ew-btn" data-bs-dismiss="modal">' +
                ew.language.phrase("CancelBtn") +
                "</button>"
            );
          } else {
            $footer.append(
              '<button type="button" class="btn btn-default ew-btn" data-bs-dismiss="modal">' +
                ew.language.phrase("CloseBtn") +
                "</button>"
            );
          }
        }
      ...
      if (ct != null && ct.includes("json")) {
        ...
        if (body.startsWith("<") && body.endsWith(">")) {
          ...
          if (
            ...
            args.action == "add" &&
            document.querySelector("main.ew-no-record") &&
            $$1(body).find("main:not(.ew-no-record)")[0]
          ) {
          ...
          $dlg
            .find(".modal-body .ew-infinite-scroll-grid .ew-grid-middle-panel")
            .css("height", "auto"); // Do not use infinite scrolling height for Grid-Add/Edit
          // If callback is function and no modal buttons, call the callback function then return
          if (
            !$dlg.find(".modal-body .ew-modal-buttons").length &&
            isFunction$2(args == null ? void 0 : args.callback)
          ) {
            $dlg.modal("hide");
        ...
        if (frm.submitWithFetch) {
          p.then(async (response) => {
            ...
            let resp = response.clone(),
              isJson =
                (_resp$headers$get = resp.headers.get("Content-Type")) == null
                  ? void 0
                  : _resp$headers$get.includes("json"),
              result = isJson ? await resp.json() : null,
              hasErrors =
                (result == null ? void 0 : result.validation) ||
                getError(result);
            if (!isJson || !hasErrors)
    ...
    let success = function (data) {
      ...
      if ($$1.isObject(result)) {
        ...
        if (lnkData.table)
          $dlg.find(".modal-dialog").addClass("table-" + lnkData.table);
        ...
        if (lnkData.ajax && lnkData.table && lnkData.action == "view") {
          ...
          [".ew-add", ".ew-copy", ".ew-edit", ".ew-delete"].forEach(
            (className) => {
              let $dlglnk = $dlg.find(
                  ".modal-body " + className + "[data-ew-action=modal]"
                ),
                // Link inside modal dialog
                $lnk = $$1(
                  "[data-ew-action=modal][data-table='" +
                    lnkData.table +
                    "'][data-url='" +
                    $dlglnk.data("url") +
                    "']"
                  ...
                  .not(".modal-body *")
                  .first(); // Link outside modal dialog
              $lnk[0]
                ? $dlglnk.on("click", (e) => {
                    e.preventDefault();
        ...
        if (!modal._focustrap.__handleFocusin) {
          ...
          modal._focustrap._handleFocusin = function (e) {
            ...
            if (
              (_e$target10 = e.target) != null &&
              _e$target10.matches("[class^=cke_dialog_]")
            )
    ...
    if (
      args.callback === true &&
      (_$dlg$data = $dlg.data("args")) != null &&
      _$dlg$data.callback
    )
      args.callback = $dlg.data("args").callback;
    if (
      url &&
      (_$dlg$data2 = $dlg.data("args")) != null &&
      _$dlg$data2.reload
    ) {
  ...
  function importDialogShow(args) {
    ...
    args.lnk =
      args.lnk ||
      ((_args$evt6 = args.evt) == null ? void 0 : _args$evt6.currentTarget);
    (_bootstrap$Tooltip$ge5 = bootstrap.Tooltip.getInstance(args.lnk)) ==
      null || _bootstrap$Tooltip$ge5.hide();
    ...
    let addRow = function (result) {
      let { row, success, error, count } = result,
        callback = () => {
          table
            .addRow(row)
            .then((r) => r.getElement().classList.toggle("text-danger", error));
          if (error)
            console.log(
              ew.language
                .phrase("ImportRowError")
                .replace("%i", count + " " + JSON.stringify(row))
                .replace("%e", error)
            );
        };
      row = {
      ...
      if (!table) {
                  ...
                  if (["_index", "_error"].includes(column.field)) {
                    ...
                    column.formatter = (cell) => {
                      ...
                      let row = cell.getRow(),
                        data = row.getData();
                      if (data._error)
                        new bootstrap.Tooltip(row.getElement(), {
                          placement: "bottom",
                          title: data._error,
        ...
        table.on("tableBuilt", () =>
          table.element
            .querySelector(".tabulator-row")
            .classList.toggle("text-danger", error)
        );
      } else {
    ...
    // Enable buttons
    let enableButtons = (enabled) =>
      $dlg.find(".modal-footer .btn").prop("disabled", !enabled);
    ...
    // Show progress
    let showProgress = (pc, classname) =>
      $progress
        .removeClass("d-none")
        .find(".progress-bar")
        .removeClass("text-bg-success text-bg-info")
        .addClass(classname || "text-bg-success")
    ...
    let uploadProgress = function (data) {
      ...
      if (pc === 100)
        showMessage(ew.language.phrase("ImportUploadComplete"), "text-primary");
      else
        showMessage(
          ew.language.phrase("ImportUploadProgress").replace("%p", pc),
          "text-primary"
        );
    };
    ...
    let updateProgress = function (e) {
      ...
      try {
        ...
        if (
          tcnt > 0 &&
          $dlg.find(".modal-footer .ew-close-btn").data("import-progress")
        ) {
          // Show progress
          ...
          showMessage(
            ew.language
              ...
              .replace("%s", scnt)
              .replace("%e", fcnt),
            fcnt > 0 ? "text-danger" : "text-primary"
          );
        }
      } catch (e) {}
    };
    ...
    let importComplete = function (e) {
      ...
      if (result.success) {
        ...
        if (
          ew.IMPORT_MAX_FAILURES > 0 &&
          result.failCount >= ew.IMPORT_MAX_FAILURES
        )
          msg.push(
            ew.language
              .phrase("ImportMaxFailuresExceeded")
              .replace("%s", ew.IMPORT_MAX_FAILURES)
          );
      ...
      let $btn = $dlg
        .find(".modal-footer .ew-import-btn")
        .toggleClass("d-none", !result.rollbacked || !result.success); // Show/Hide the import button
    ...
    if (!options.acceptFileTypes)
      options.acceptFileTypes = new RegExp(
        "\\.(" + ew.IMPORT_FILE_ALLOWED_EXTENSIONS.replace(/,/g, "|") + ")$",
        "i"
      );
        ...
        Object.assign(
          {
            ...
            messages: {
              ...
              maxNumberOfFiles: ew.language.phrase(
                "UploadErrorMaxNumberOfFiles"
              ),
            ...
            beforeSend: function (jqxhr, settings) {
              ...
              if (
                Array.isArray(
                  data == null ||
                    (_data$result = data.result) == null ||
                    (_data$result = _data$result.files) == null
                    ? void 0
                    : _data$result.importfiles
                )
              ) {
                let errors = [];
                data.result.files.importfiles.forEach(function (file, index) {
                  if (file.error)
                    errors.push(
                      ew.language
                        .phrase("ImportUploadError")
                        .replace("%f", file.name)
                        .replace("%s", file.error)
                    );
                });
                if (errors.length) {
            ...
            fail: function (e, data) {
              showMessage(
                ew.language
                  .phrase("ImportServerError")
                  .replace("%s", data.textStatus)
                  .replace("%t", data.errorThrown),
                "text-danger"
              );
            },
    ...
    $dlg
      .find(".modal-footer .ew-close-btn")
      .off("click.ew")
      .on("click.ew", function () {
        let $this = $$1(this);
    ...
    ew.importDialog = $dlg
      .on("show.bs.modal", function () {
        ...
        hideProgress();
        (_this$querySelector = this.querySelector(
          ".modal-footer .ew-import-btn"
        )) == null || _this$querySelector.classList.add("d-none");
  ...
  function autoFill(el) {
    ...
    let success = function (data) {
      ...
      for (let j = 0; j < dest_array.length; j++) {
        let destEl = getElements(
          dest_array[j].replace(/^x_/, "x" + rowindex + "_"),
          f
        );
        if (destEl) {
          let val = $$1.isValue(result["af" + j])
              ? String(result["af" + j])
              : "",
            args = {
              results,
      ...
      $this
        ...
        .on("show.bs.popover", function () {
          ...
          if (wd)
            ...
            $$1(bootstrap.Popover.getInstance(this)._getTipElement()).css(
              "max-width",
              parseInt(wd, 10) + "px"
            );
        });
  ...
  function initSearchFilters(e) {
    var _e$target11;
    let el =
        (_e$target11 = e == null ? void 0 : e.target) != null
          ? _e$target11
          : document,
      btn = el == null ? void 0 : el.querySelector(".ew-filter-option");
    if (
      ew.CLIENT_SEARCH_FILTER ||
      (ew.SERVER_SEARCH_FILTER &&
        ew.IS_LOGGEDIN &&
        !ew.IS_SYS_ADMIN &&
        ew.CURRENT_USER_NAME != "")
    ) {
      $$1(btn)
        .removeClass("d-none")
        .find(".ew-btn-dropdown")
        .on("show.bs.dropdown", function (e) {
          var _frm$filterList$filte, _frm$filterList;
          let formId = $$1(this)
              .find(".ew-save-filter[data-form], .ew-delete-filter[data-form]")
              .data("form"),
            frm = ew.forms.get(formId),
            filters = ew.CLIENT_SEARCH_FILTER
              ? ew.parseJson(
                  localStorage.getItem(
                    ew.PROJECT_NAME + "_" + frm.id + "_filters"
                  ) || "[]"
                ) || []
              : (_frm$filterList$filte =
                  frm == null || (_frm$filterList = frm.filterList) == null
                    ? void 0
                    : _frm$filterList.filters) != null
              ? _frm$filterList$filte
              : [];
          frm.setupFilters(
            e,
            filters.filter((val) => Array.isArray(val) && val.length == 2)
          );
        });
    } else {
      ...
      $$1("#ew-email-dialog")
        .on("shown.bs.modal", (e) =>
          setTimeout(() => {
            ...
            return (_e$target$querySelect = e.target.querySelector(
              ".modal-body .form-control"
            )) == null
              ? void 0
              : _e$target$querySelect.focus();
          }, 200)
        )
        .on("click", ".modal-footer .btn-primary", function (e) {
          var _$$closest$find$data;
          e.preventDefault();
          if (
            (_$$closest$find$data = $$1(this)
              .closest(".modal")
              .find(".modal-body form")
              .data("form")) != null &&
            _$$closest$find$data.submit()
          )
            $dlg.modal("hide");
        });
    if (!$dlg[0]) {
      ...
      frm = new FormBuilder()
        ...
        .addFields([
          [
            "sender",
            [
              ew.Validators.required(ew.language.phrase("Sender")),
              ew.Validators.email,
            ],
          ],
          [
            "recipient",
            [
              ew.Validators.required(ew.language.phrase("Recipient")),
              ew.Validators.emails(
                ew.MAX_EMAIL_RECIPIENT,
                ew.language.phrase("EnterProperRecipientEmail")
              ),
            ],
          ],
          [
            ...
            ew.Validators.emails(
              ew.MAX_EMAIL_RECIPIENT,
              ew.language.phrase("EnterProperCcEmail")
            ),
          ],
          [
            "bcc",
            ew.Validators.emails(
              ew.MAX_EMAIL_RECIPIENT,
              ew.language.phrase("EnterProperBccEmail")
            ),
          ],
        ...
        .setSubmit(function () {
          ...
          let data = mergeSearchParams(
            $f.serialize(),
            args.key,
            getchartParams(),
            form && args.exportSelected
              ? $$1(form)
                  .find("input[type=checkbox][name='key_m[]']:checked")
                  .serialize()
              : null
  ...
  function exportEmail(tblVar, data) {
    ...
    return swal.fire({
      ...
      willOpen: () => {
        swal.showLoading();
        _fetch(
          getApiUrl([ew.API_EXPORT_ACTION, "email", tblVar]) +
            "?rnd=" +
            random(),
          {
            // Post to Export API
          ...
          .then(async (response) => {
            ...
            if (result != null && result.success) {
              ...
              swal.update({
                html:
                  (_ref3 = "<div class='text-danger'>" + getError(result)) !=
                  null
                    ? _ref3
                    : ew.language.phrase("FailedToExport") + "</div>",
              });
              $document$1.trigger("export", [
                {
                  type: "fail",
          ...
          .catch((error) => {
            ...
            swal.update({
              html:
                (_ref4 = "<div class='text-danger'>" + error) != null
                  ? _ref4
                  : ew.language.phrase("FailedToExport") + "</div>",
            });
            $document$1.trigger("export", [
    ...
    if (!bootstrap.Popover.getInstance(obj)) {
      $obj
        .popover({
          ...
          trigger: "manual",
          template:
            '<div class="popover" role="tooltip"><h3 class="popover-header d-none" style="cursor: move;"></h3><div class="popover-body"></div></div>',
          // No .popover-arrow
        ...
        .on("shown.bs.popover", function (e) {
          ...
          $$1.ajax({
            ...
            success: function (data) {
              let $tip = $$1(
                bootstrap.Popover.getInstance(obj)._getTipElement()
              );
              if (config.title)
                $tip
                  .find(".popover-header")
              ...
              $tip
                .find(".popover-body")
                .html($$1("<div></div>").html(html).find("#ew-report")) // Insert the container table only
                .find(".ew-table")
                .each(ew.setupTable);
            ...
            error: function (o) {
              if (o.responseText) {
                ...
                if (
                  $$1.isString(o.responseText) &&
                  o.responseText.startsWith("{") &&
                  o.responseText.endsWith("}")
                ) {
                  var _result$error13, _result$error14;
                  let result = parseJson(o.responseText);
                  if (
                    result != null &&
                    (_result$error13 = result.error) != null &&
                    _result$error13.type &&
                    result != null &&
                    (_result$error14 = result.error) != null &&
                    _result$error14.description
                  ) {
                ...
                popover == null ||
                  popover.setContent({
                    ".popover-body":
                      '<p class="text-danger">' + o.responseText + "</p>",
                  });
              }
            },
          });
  ...
  function toggleLayout(el) {
    ...
    (_bootstrap$Tooltip$ge6 = bootstrap.Tooltip.getInstance(el)) == null ||
      _bootstrap$Tooltip$ge6.hide();
    document.body.style.cursor = "wait";
    fetch(sanitizeUrl(setLayout(el.dataset.url, el.dataset.layout)))
      .then((response) => response.text())
      .then((html) => {
        let $grid = $$1(".ew-multi-column-grid").html(
          $$1("<div>" + html + "</div>")
            .find(".ew-multi-column-grid")
            .html()
        );
        ew.initPage({
          target: $grid[0],
  ...
  function copyToClipboard(source) {
    ...
    const str =
      ((_source = source) == null ? void 0 : _source.value) ||
      ((_source2 = source) == null ? void 0 : _source2.innerText);
    if (str) {
  ...
  function checkRange(object_value, min_value, max_value) {
    ...
    if ($$1.isNumber(min_value) || $$1.isNumber(max_value)) {
      ...
      if (checkNumber(object_value))
        object_value = ew.parseNumber(object_value);
    }
  ...
  function checkPhone(object_value) {
    if (!object_value || object_value.length == 0) return true;
    return /^\(\d{3}\) ?\d{3}( |-)?\d{4}|^\d{3}( |-)?\d{3}( |-)?\d{4}$/.test(
      object_value.trim()
    );
  }
  ...
  function checkSsn(object_value) {
    if (!object_value || object_value.length == 0) return true;
    return /^(?!000)([0-6]\d{2}|7([0-6]\d|7[012]))([ -]?)(?!00)\d\d\3(?!0000)\d{4}$/.test(
      object_value.trim()
    );
  }
  ...
  function checkGuid(object_value) {
    if (!object_value || object_value.length == 0) return true;
    return /^(\{\w{8}-\w{4}-\w{4}-\w{4}-\w{12}\}|\w{8}-\w{4}-\w{4}-\w{4}-\w{12})$/.test(
      object_value.trim()
    );
  }
  ...
  function showMessage(arg) {
    ...
    let p =
        (_arg$target = arg == null ? void 0 : arg.target) != null
          ? _arg$target
          : doc,
  ...
  function upload(input) {
    ...
    let _done = function (e, data) {
      ...
      let name = data.result.files[0].name,
        ar = multiple
          ? $fn.val()
            ? $fn.val().split(ew.MULTIPLE_UPLOAD_SEPARATOR)
            : []
          : [];
      ar.push(name);
    ...
    let _change = function (e, data) {
      ...
      $ft.toggleClass(
        "ew-has-rows",
        ((_data$files = data.files) == null ? void 0 : _data$files.length) > 0
      );
      let ar = $fn.val() ? $fn.val().split(ew.MULTIPLE_UPLOAD_SEPARATOR) : [];
      ...
      if (
        $$1.isNumber(l) &&
        l > 0 &&
        ar.join(ew.MULTIPLE_UPLOAD_SEPARATOR).length > l
      ) {
    ...
    let _changed = function (e, data) {
      ...
      $ft.toggleClass(
        "ew-has-rows",
        ((_data$files2 = data.files) == null ? void 0 : _data$files2.length) >
          0 ||
          ((_data$result2 = data.result) == null ||
          (_data$result2 = _data$result2.files) == null
            ? void 0
            : _data$result2.length) > 0
      );
      let ar = $fn.val() ? $fn.val().split(ew.MULTIPLE_UPLOAD_SEPARATOR) : [];
    ...
    let _added = function (e, data) {
      ...
      $ft.toggleClass(
        "ew-has-rows",
        ((_data$files3 = data.files) == null ? void 0 : _data$files3.length) > 0
      );
    ...
    let uploadUrl = getApiUrl(ew.API_JQUERY_UPLOAD_ACTION),
      ...
      $form = $$1(form),
      readonly =
        $form.find("#confirm").val() == "confirm" ||
        $input.attr("readonly") ||
        $input.prop("disabled");
    if (readonly && !$input.prop("disabled")) $input.prop("disabled", true); // Disable input button if readonly
    const { acceptFileTypes, maxFileSize, maxNumberOfFiles } = $input.data();
    const formData = {
      id: id,
      table: tbl,
  ...
  function parseNumber(data) {
    ...
    if (locale.delimiters.thousands !== ew.GROUPING_SEPARATOR)
      locale.delimiters.thousands = ew.GROUPING_SEPARATOR;
    if (locale.delimiters.decimal !== ew.DECIMAL_SEPARATOR)
      locale.delimiters.decimal = ew.DECIMAL_SEPARATOR;
  ...
  function getNumberingSystem() {
    return (
      ew.NUMBERING_SYSTEM ||
      new Intl.NumberFormat(ew.LANGUAGE_ID).resolvedOptions().numberingSystem
    );
  }
  ...
  function formatTimeSpan(start, end, allDay) {
    ...
    if (allDay) {
      ...
      if (
        (interval.isValid && interval.count("days") > 1) ||
        (from.isValid && to.isValid && from.startOf("day") < to.startOf("day"))
      )
        return (
          formatDateTime(from, ew.DATE_FORMAT) +
          " - " +
          formatDateTime(to, ew.DATE_FORMAT)
        );
      else if (from.isValid && !end)
        // No end date
      ...
      if (from.isValid && to.isValid)
        return from.startOf("day") < to.startOf("day")
          ? formatDateTime(from, ew.DATE_FORMAT + " " + ew.TIME_FORMAT) +
              " - " +
              formatDateTime(to, ew.DATE_FORMAT + " " + ew.TIME_FORMAT)
          : formatDateTime(from, ew.TIME_FORMAT) +
              " - " +
              formatDateTime(to, ew.TIME_FORMAT);
      else if (from.isValid) return formatDateTime(from, ew.TIME_FORMAT);
    }
    return "";
  }
  ...
  function initPage(e) {
    var _e$target12;
    var el =
        (_e$target12 = e == null ? void 0 : e.target) != null
          ? _e$target12
          : document,
      $el = $$1(el),
    ...
    $el
      .find(".ew-column-dropdown")
      .each(function () {
        var _localStorage$getItem;
        let table = this.dataset.table;
        (_localStorage$getItem = localStorage.getItem(
          ew.PROJECT_NAME + "_" + table + "_invisible_fields"
        )) == null ||
          _localStorage$getItem.split(",").forEach((field) =>
            $$1("#tbl_" + table + "list")
              .find(
                "th[data-name='" + field + "'],td[data-name='" + field + "']"
              )
              .toggleClass("d-none", true)
          );
        $$1(this)
          .find(".ew-dropdown-checkbox")
          .on("click", function (e) {
            let input = this.querySelector(
                ".ew-dropdown-check-input[data-field]"
              ),
            ...
            if (table && field) {
              ...
              $$1("#tbl_" + table + "list")
                ...
                .toggleClass("d-none", !input.classList.contains("ew-checked"));
            }
            localStorage.setItem(
              ew.PROJECT_NAME + "_" + table + "_invisible_fields",
              Array.from(
                e.currentTarget
                  ...
                  .querySelectorAll(
                    ".ew-dropdown-check-input[data-field]:not(.ew-checked)"
                  ),
                (el) => el.dataset.field
              )
            );
      ...
      .on("show.bs.dropdown", function (e) {
        let table = e.currentTarget.dataset.table,
          inputs = e.currentTarget.querySelectorAll(
            ".ew-dropdown-check-input[data-field]"
          );
        for (let input of inputs) {
          ...
          input.classList.toggle(
            "ew-checked",
            !!$$1("#tbl_" + table + "list").find(
              "th[data-name='" + field + "']:not(.d-none)"
            )[0]
          );
        }
      });
    ...
    if ($rpt[0]) {
      $rpt
        .find(".card")
        .on("collapsed.lte.widget", function () {
    ...
    // Show message
    if (
      typeof ew.USE_JAVASCRIPT_MESSAGE != "undefined" &&
      ew.USE_JAVASCRIPT_MESSAGE
    )
      showMessage(e);
  }
  ...
  function redirect(url, f, method, data) {
    var _ew$vars;
    let urls =
      (_ew$vars = ew.vars) != null && _ew$vars.login
        ? Array.from(Object.entries(ew.vars.login))
            .filter((entry) => entry[0].endsWith("Url"))
            .map((entry) => entry[1])
        : [];
    ...
    fetch(url.toString())
      .then(async (response) => {
        let $html = $$1("<div>" + (await response.text()) + "</div>"),
          $new = $html
            .find(".ew-chart-top, .ew-chart-bottom")
            .filter("[data-chart='" + $container.data("chart") + "']"),
          $debug = $html.find(".ew-debug");
        if ($new[0]) $container.replaceWith($new);
        if ($debug[0]) $new.next(".ew-debug").replaceWith($debug);
      })
      .finally(() =>
        $container.removeClass("ew-loading").find($overlay).remove()
      );
  }
  ...
  function refresh(promise, context) {
    ...
    let target = document.querySelector(
        context +
          ", .ew-multi-column-grid, .ew-view-form, .ew-edit-form, .ew-calendar, .ew-dashboard, main.ew-no-record"
      ),
    ...
    let $overlay = $$1(ew.overlayTemplate()),
      $target = $$1(target)
        .addClass("ew-loading")
        .toggleClass("overlay-wrapper", !target.classList.contains("card"))
        .append($overlay);
    return promise
      .then(async (response) => {
        ...
        if (ct != null && ct.includes("json")) {
          ...
          if (dashboard) {
            ...
            for (let key in result) {
              $$1("#" + key)
                .find(".card-body")
                .html(""); // Clear content first
              $$1("#" + key)
                .find("[data-card-widget='card-refresh']")
                .CardRefresh("load");
            }
            ...
            for (let [key, value] of Object.entries(validation)) {
              ...
              if ($$1.isNumber(key) && $$1.isObject(value)) {
                ...
                for (let [fldvar, msg] of Object.entries(value))
                  f.addCustomError(fldvar, msg, key);
        ...
        if (
          $html.find("main.ew-no-record")[0] ||
          $target.is("main.ew-no-record")
        ) {
          ...
          $$1(selector).each((i, el) => {
            ...
            if (selector == ".ew-toolbar") {
              ...
              if ($tmpl[0]) {
                ...
                if (
                  $search.val() != $$1("#" + ew.TABLE_BASIC_SEARCH).val() ||
                  $searchType.val() !=
                    $$1("#" + ew.TABLE_BASIC_SEARCH_TYPE).val()
                )
                  $$1(".nav-item.navbar-basic-search").replaceWith($html);
              }
              $old
                .find(".ew-search-operator .form-select.ew-operator-select")
                .each((i, el) => searchOperatorChange(el));
        ...
        selectors
          .filter((selector) => selector != ".ew-message-dialog")
          .forEach((selector) => $html.find(selector).remove()); // Remove HTML used (except message)
        ...
        target.querySelectorAll(".btn-group").forEach((el) => {
          ...
          return (_bootstrap$Tooltip$ge7 = bootstrap.Tooltip.getInstance(el)) ==
            null
            ? void 0
            : _bootstrap$Tooltip$ge7.update();
  ...
  function addEventHandlers(tblVar) {
    ...
    if (fields) {
      for (const [fldVar, events] of Object.entries(fields))
        $$1("[data-table=" + tblVar + "][data-field=" + fldVar + "]").on(
          events
        );
    }
  }
  ...
  function fullCalendar(el, options, dropdown) {
    ...
    let processJson = (data) => {
      ...
      let convertValue = (key, value) => {
        ...
        else if (key == "url" && !value) return ""; // Make sure not null
        else if (key == "display" && !value)
          return "auto"; // Make sure not null
        else if (key == "classNames" && value && $$1.isString(value))
          // Make sure return as array
          return value.replace(/\s+/g, " ").split(" ");
        else if (key == "classNames" && !value)
          // Make sure return as array
          return [""];
        else if (key == "groupId" && !value)
      ...
      if (data.success) {
        ...
        for (let event of events) {
          ...
          if (action == ew.API_DELETE_ACTION) {
            ...
            (_calendar$getEventByI = calendar.getEventById(event.id)) == null ||
              _calendar$getEventByI.remove();
            ...
            for (let name in event) {
              ...
              if (key && value !== undefined) {
                ...
                if (key == "start") curEvent.setStart(value);
                else if (key == "end") curEvent.setEnd(value);
                else if (key == "allDay") curEvent.setAllDay(value);
                else if (key == "description")
                  curEvent.setExtendedProp(key, value);
                else if (key != "id" && !groupId)
    ...
    let processEvent = (info) => {
      ...
      events.forEach((event) =>
        processRequest(event, apiAction)
          .then((data) => {
            if (!data.success) {
    ...
    // Process request
    let processRequest = (event, apiAction) =>
      new Promise((resolve, reject) => {
        ...
        for (let [key, name] of Object.entries(eventFields)) {
          if (key in obj) {
            ...
            if (val instanceof Date)
              ...
              val = Array.isArray(val) ? val.join(" ") : val; // Convert array to string
            data[name] = val;
            delete obj[key];
          }
        }
        ...
        let apiUrl =
          apiAction == ew.API_ADD_ACTION
            ? ew.getApiUrl([apiAction, updateTable])
            : ew.getApiUrl([apiAction, updateTable, event.id]);
        $$1
          .post(apiUrl, data, (data) => resolve(data))
          .fail((err) => reject(err));
      });

    // Get field parameter
    let getFldParm = (name) =>
      Object.keys(currentTable.fields).find(
        (key) => currentTable.fields[key].name == name
      );
    ...
    let addEventDialogShow = (data) => {
      ...
      if (
        $$1.isObject(data) &&
        (data.startStr || data.endStr) /* || data.allDay */
      ) {
    ...
    // Delete event
    let deleteEvent = (id) =>
      ew.prompt(ew.language.phrase("DeleteEventConfirm"), (result) => {
        if (result)
          ew.modalDialogShow({
    ...
    if (addUrl) {
      fullCalendarOptions.customButtons = Object.assign(
    ...
    let getOrCreatePopover = (info) => {
      ...
      let popover = bootstrap.Popover.getOrCreateInstance(info.el, config),
        template = Array.from(
          document.querySelectorAll("script[data-name='event-popover']")
        )
          .sort((a, b) => {
      ...
      info.event.setExtendedProp(
        "timeSpan",
        ew.formatTimeSpan(info.event.start, info.event.end, info.event.allDay)
      ); // Add extended property "timeSpan"
      let content = tmpl.render(info.event, ew.jsRenderHelpers).trim();
      if (content) {
        var _Array$from$find$repl, _Array$from$find;
        let popoverClass =
          (_Array$from$find$repl =
            (_Array$from$find = Array.from(info.el.classList.values()).find(
              (name) => name.match(/^event-([\w+]+)\b/)
            )) == null
              ? void 0
              : _Array$from$find.replace(/^event-/, "popover-")) != null
            ? _Array$from$find$repl
            : "popover-default";
    ...
    if (
      ...
      (viewUrl || editUrl || (addUrl && copyUrl) || deleteUrl)
    ) {
      dropdown.dataset.bsToggle = "dropdown"; // For clearing dropdown menu
      document.addEventListener(
        "keydown",
        (event) => event.key !== "Escape" || bootstrap.Dropdown.clearMenus()
      );
      ...
      bsDropdown._menu.querySelectorAll(".dropdown-item").forEach((item) =>
        item.addEventListener("click", (e) => {
          let dataset = e.currentTarget.dataset,
            ...
            id = dataset.eventId;
          if (action && id) {
            if (action == "view" && viewUrl) viewEventDialogShow(id);
            else if (action == "edit" && editUrl) editEventDialogShow(id);
            else if (action == "copy" && addUrl && copyUrl)
              // Copy
              addEventDialogShow(id);
            else if (action == "delete" && deleteUrl) deleteEvent(id);
          }
        })
      );
    }
    ...
    let opts = ew.deepAssign(fullCalendarOptions, {
      ...
      eventDragStart: (info) => {
        ...
        return (_bootstrap$Popover$ge = bootstrap.Popover.getInstance(
          info.el
        )) == null
          ? void 0
          : _bootstrap$Popover$ge.hide();
      },
      ...
      eventClick: (info) => {
        ...
        if (info.jsEvent.button == 2) {
          // Right click
          let allowed =
            info.event.extendedProps["_view"] ||
            info.event.extendedProps["_edit"] ||
            info.event.extendedProps["_copy"] ||
            info.event.extendedProps["_delete"];
          if (bsDropdown && allowed) {
            var _bootstrap$Popover$ge2;
            (_bootstrap$Popover$ge2 = bootstrap.Popover.getInstance(info.el)) ==
              null || _bootstrap$Popover$ge2.hide();
            bsDropdown.hide();
            bsDropdown._menu
              .querySelectorAll(".dropdown-item")
              .forEach((item) => {
                ...
                item.classList.toggle(
                  "d-none",
                  !options[item.dataset.action + "Url"] ||
                    !info.event.extendedProps["_" + item.dataset.action]
                ); // Check event permission
              });
            if (
              bsDropdown._menu.querySelectorAll(".dropdown-item:not(.d-done)")
                .length
            ) {
              virtualElement.getBoundingClientRect =
                generateGetBoundingClientRect(info);
              bsDropdown.show();
            }
          }
          ...
          options.usePopover &&
          eventPopoverOptions.trigger.match(/\bclick\b/)
        ) {
          var _getOrCreatePopover;
          (_getOrCreatePopover = getOrCreatePopover(info)) == null ||
            _getOrCreatePopover.toggle();
      ...
      eventMouseEnter: (info) => {
        ...
        if (
          (_bsDropdown = bsDropdown) != null &&
          _bsDropdown._isShown(
            bsDropdown._menu
          ) /* && bsDropdown._element.dataset.eventId == info.event.id*/
        )
          return;
        if (
          options.popoverViews.includes(info.view.type) &&
          options.usePopover &&
          eventPopoverOptions.trigger.match(/\bhover\b/)
        )
          (_getOrCreatePopover2 = getOrCreatePopover(info)) == null ||
            _getOrCreatePopover2.show();
      },
      eventMouseLeave: (info) => {
        var _bsDropdown2, _bootstrap$Popover$ge3;
        if (
          (_bsDropdown2 = bsDropdown) != null &&
          _bsDropdown2._isShown(
            bsDropdown._menu
          ) /* && bsDropdown._element.dataset.eventId == info.event.id*/
        )
          return;
        if (
          ...
          (_bootstrap$Popover$ge3 = bootstrap.Popover.getInstance(info.el)) ==
            null || _bootstrap$Popover$ge3.hide();
      },
    });
  ...
  async function pushNotificationDialogShow(args) {
    ...
    if (!$form.find("input[name='key_m[]']:checked")[0]) {
      // No keys selected
      let result = await ew$1.prompt(
        ew$1.language.phrase("SendPushNotificationsToAll"),
        (bool) => bool
      ); // Callback returns result as boolean
      $f.data("all", result);
      if (!result)
        // Cancelled
        return;
    ...
    if (!frm) {
      frm = new ew$1.FormBuilder()
        .setId($f.attr("id"))
        .addFields([
          [
            "title",
            ew$1.Validators.required(
              ew$1.language.phrase("PushNotificationFormTitle")
            ),
          ],
          [
            "body",
            ew$1.Validators.required(
              ew$1.language.phrase("PushNotificationFormBody")
            ),
          ],
        ])
        .setValidate(function () {
        ...
        .setSubmit(function () {
          ...
          let data = [
            ...
            $f.data("all")
              ? ""
              : $form.find("input[name='key_m[]']:checked").serialize(),
          ].join("&");
          $$1.post(args.url, data, (result) => {
            var _result$error;
            if (Array.isArray(result)) {
              let successes = result.reduce(
              ...
              if (successes > 0 && failures == 0)
                ew$1.alert(
                  ew$1.language
                    .phrase("PushNotificationSuccess")
                    .replace("%s", successes),
                  "success"
                );
                ...
                ew$1.alert(
                  ew$1.language
                    .phrase("PushNotificationFailure")
                    .replace("%f", failures)
                );
              else if (successes == 0 && failures == 0)
                ew$1.alert(
                  ew$1.language
                    .phrase("NoSubscriptions")
                    .replace("%f", failures),
                  "primary"
                );
              else
    ...
    ew$1.pushDialog = $dlg
      .modal("hide")
      .find(".modal-title")
      .html(args.hdr || target.dataset.caption)
      .end()
      .modal("show");
    return false;
  }
  ...
  async function createSubscription() {
    var _result$error2;
    let url = ew$1.getApiUrl([
        ew$1.API_PUSH_NOTIFICATION_ACTION,
        ew$1.API_PUSH_NOTIFICATION_SUBSCRIBE,
      ]),
      serviceWorkerReg = await navigator.serviceWorker.ready,
      subscription = await serviceWorkerReg.pushManager.subscribe({
        userVisibleOnly: true,
        applicationServerKey: urlBase64ToUint8Array(
          ew$1.PUSH_SERVER_PUBLIC_KEY
        ),
      }),
  ...
  function setSubscription(subscription) {
    let $btn = $$1("#subscribe-notification")
      .toggleClass("ew-enable-notification", !subscription)
      .removeClass("disabled");
    $btn
      .find("i")
      .toggleClass("fa-bell", !subscription)
      .toggleClass("fa-bell-slash", !!subscription);
    let inst = bootstrap.Tooltip.getInstance($btn[0]);
    if (inst)
      inst._config.title = ew$1.language.phrase(
        subscription ? "DisableNotifications" : "EnableNotifications"
      );
  }
    ...
    if (Notification.permission !== "denied" && !ew$1.IS_SYS_ADMIN) {
      ...
      try {
        if (Notification.permission === "granted") {
          ...
          setSubscription(
            (_await$getSubscriptio = await getSubscription()) != null
              ? _await$getSubscriptio
          ...
          if (
            !(
              ...
              _document$getElementB.classList.contains("ew-enable-notification")
            )
          )
            ew$1.showToast(
              ew$1.language.phrase("NotificationsEnabled"),
              "success"
            );
        } else if (
          ...
          if (
            !(
              ...
              _document$getElementB2.classList.contains(
                "ew-enable-notification"
              )
            )
          )
            ew$1.showToast(
    ...
    ew$1.prompt(
      ...
      async (res) => {
        ...
        try {
          ...
          let url = ew$1.getApiUrl([
              ew$1.API_PUSH_NOTIFICATION_ACTION,
              ew$1.API_PUSH_NOTIFICATION_DELETE,
            ]),
          ...
          if (result.success) {
            ...
            ew$1.showToast(
              ew$1.language.phrase("NotificationsDisabled"),
              "success"
            );
          }
        } catch (e) {
  ...
  loadjs.ready("foot", () => {
    if (ew$1.PUSH_SERVER_PUBLIC_KEY && isPushNotificationSupported()) {
      navigator.serviceWorker
        .register(ew$1.PATH_BASE + ew$1.SERVICE_WORKER, {
          scope: ew$1.PATH_BASE,
        })
        .then(() => {
          checkSubscription();
          $$1("#subscribe-notification").on("click", function () {
            this.classList.contains("ew-enable-notification")
  ...
  function show(data) {
    ...
    if (useSingleMap) {
      ...
      if (!$div[0]) {
        $div = $$1("<div></div>")
          .attr("id", id)
          .addClass("ew-single-map")
          .height(data.singleMapHeight); // Create new $div for single map
        if (data.singleMapWidth) $div.width(data.singleMapWidth);
        $$1(".ew-grid, .ew-multi-column-grid")
          .first()
          [data.showMapOnTop ? "before" : "after"]($div); // Insert before/after table
        ...
        $div.data(
          "markerClusterer",
          useMarkerClusterer ? ext.createMarkerClusterer(data) : null
        );
      }
      ...
      .map(function (i) {
        ...
        if (data.address) {
          ...
          if (latitude && !isNaN(latitude) && longitude && !isNaN(longitude))
            data.latlng = ew.maps[data.ext].createLatLng(latitude, longitude);
          return show(data);
        }
      })
      .get();
  ...
  loadjs.ready("load", function () {
    ...
    let hash = ew.currentUrl.searchParams.get("hash"),
      scrollTop =
        (_$$offset = $$1("#" + hash).offset()) == null ? void 0 : _$$offset.top;
    if (scrollTop)
      $$1("html, body").animate(
        {
          scrollTop,
  ...
  $document
    .on("click", "[data-ew-action]:not([data-ew-action=''])", function (e) {
      ...
      if (!action) {
        ...
        let panel = document.getElementById(
          this.dataset.form + "_search_panel"
        );
        if (panel) {
        ...
        return ew.sendOTP(
          (_document$getElementB = document.getElementById(data.account)) ==
            null
            ? void 0
    ...
    .on("change", "[data-ew-action]:not([data-ew-action=''])", function (e) {
      ...
      for (let action of actions) {
        if (action == "submit-form") {
          ...
          let pageSize = ew.sameText(this.value, "ALL")
            ? "ALL"
            : ew.parseNumber(this.value);
          if (ew.convertToBool(data.ajax)) {
            let url =
              ew.setLayout(this.dataset.url, false) +
              "&" +
              this.name +
              "=" +
              pageSize;
      ...
      if (action == "change-page") {
        ...
        if (e.key == "Enter") {
          if (ew.convertToBool(data.ajax)) {
            ...
            ew.currentUrl.searchParams.set(
              this.name,
              ew.parseNumber(this.value)
            );
  ...
  $$1.widget("blueimp.fileupload", $$1.blueimp.fileupload, {
    processActions: {
      cropImage: function (data, options) {
        ...
        if (
          options.disabled ||
          ($$1.isNumber(options.maxFileSize) &&
          ...
          $$1("#ew-cropper-dialog")
            .on("shown.bs.modal", function () {
              $dlg.data(
                "cropper",
                new Cropper(
                  this.querySelector("#ew-crop-image"),
            ...
            .on("hidden.bs.modal", function () {
              var _$dlg$data;
              (_$dlg$data = $dlg.data("cropper")) == null ||
                _$dlg$data.destroy();
          ...
          .on("click", function () {
            if ($dlg.data("cropper")) {
              try {
                let canvas = $dlg
                  .data("cropper")
                  .getCroppedCanvas(options.cropperCanvasOptions);
                canvas.toBlob((blob) => {
  ...
  $$1.extend({
    ...
    isDate: function (o) {
      return (
        this.type(o) === "date" && o.toString() !== "Invalid Date" && !isNaN(o)
      );
    },
  ...
  $$1.fn.fields = function (fldvar) {
    ...
    let selector =
      ...
      (fldvar ? "=x_" + fldvar : "") +
      "]" +
      ($$1.isValue(infix) ? "[name^=x" + infix + "]" : "") +
      ":not(.ew-custom-option)";
    if (f && selector) {
      $$1(f)
        .find(selector)
        .each(function () {
  ...
  $$1.fn.extend({
    ...
    row: function () {
      ...
      let id =
          (_this$data$substring =
            (_this$data = this.data("field")) == null
              ? void 0
              : _this$data.substring(2)) != null
            ? _this$data$substring
            : "",
    ...
    visible: function (v) {
      ...
      let id =
          (_this$data$substring2 =
            (_this$data2 = this.data("field")) == null
              ? void 0
              : _this$data2.substring(2)) != null
            ? _this$data$substring2
            : "",
    ...
    toDate: function () {
      ...
      let data = this.data(),
        ...
        format =
          (_ew$vars$tables = ew.vars.tables) == null ||
          (_ew$vars$tables = _ew$vars$tables[table]) == null ||
          (_ew$vars$tables = _ew$vars$tables.fields) == null ||
          (_ew$vars$tables = _ew$vars$tables[field]) == null
            ? void 0
            : _ew$vars$tables.clientFormatPattern;
  ...
  bootstrap.Dropdown.prototype.toggle = function () {
    if (!this._element.classList.contains("ew-dropdown-toggle")) {
      if (this._element.closest("[data-widget]"))
        $$1(this._element).on("click.ew.dropdown", (e) => {
      ...
      if (this._menu.closest(".table-responsive")) {
        let container = this._menu.closest(
          ".popover-body, .modal-body, .offcanvas-body, body"
        );
          ...
          mutationList.forEach((mutation) => {
            switch (mutation.type) {
              case "attributes":
                ...
                else menu.classList.remove("d-block");
                break;
            }
          });
        }
  ...
  loadjs.ready(["wrapper", "head"], function () {
    ...
    $chatCard.on("removed.lte.cardwidget", () =>
      $chatCheck.prop("checked", false).triggerHandler("click")
    );
    $chatCard
      .on("maximized.lte.cardwidget", () => {
        ...
        $chatCard.delay(150).queue(function () {
          ...
          $chatIframe
            .addClass("w-100")
            .height(h - parseInt($chatCard.css("border-radius"), 10)); // Minus the radius of card
          $chatCard.dequeue();
        });
  ...
  const multiplyByPowerOfTen = (number, powerOfTenExponent) => {
    if (!isFiniteNumber$1(number)) return NaN;
    ...
    if (indexOfE === -1) {
      ...
      return +(
        numAsString.slice(0, indexOfE) +
        "e" +
        (+numAsString.slice(indexOfE + 1) + powerOfTenExponent)
      );
    }
  };
  ...
  const toBase = (value, valueUnit, unitScale) => {
    if (!isFiniteNumber$1(value) || valueUnit === unitScale.base) return value;
  ...
  const convertUnit = (value, originUnit, targetUnit, unitScale) => {
    if (!isFiniteNumber$1(value) || originUnit === targetUnit) return value;
    ...
    if (
      isNaN(valueAsBase) ||
      !(originUnit in resolvedScale) ||
      !(targetUnit in resolvedScale)
    )
      return NaN;
  ...
  const toBest = (value, originUnit, unitScale, options) => {
    ...
    Object.keys(scale)
      ...
      .forEach((scaleUnit) => {
        ...
        if (
          !best ||
          (absoluteResult >= resolvedOptions.cutOffNumber &&
            absoluteResult < Math.abs(best[0]))
        ) {
  ...
  const createUnitScaleFromLocaleAbbreviations = memoize$1(
    baseCreateUnitScaleFromLocaleAbbreviations
  );

  var createUnitScaleFromLocaleAbbreviations$1 =
    createUnitScaleFromLocaleAbbreviations;

  const replaceNumeralSystemWithLatinNumbers = (
    numericStringWithExtraInfo,
    numeralSystemMap
  ) => {
    ...
    const numeralSystemToLatinSystemMap = toObject$1(
      numeralSystemMap,
      (digit, digitIndex) => [digit.replace(/\u200e/g, ""), "" + digitIndex]
    );
    let output = "";
    for (
      let numericStringIndex = 0;
      numericStringIndex < numericStringLength;
      numericStringIndex++
    ) {
  ...
  const getScalingFactorFromAbbreviations = (stringOriginal, options) => {
    var _a;
    const scale = createUnitScaleFromLocaleAbbreviations$1(
      (_a = options.locale) === null || _a === void 0
        ? void 0
        : _a.abbreviations
    );
    const abbreviationsSortedByLengthDesc = Object.keys(
      scale.scaleDefinition.scale
    ...
    for (const abbreviation of abbreviationsSortedByLengthDesc) {
      ...
      const regexp = new RegExp(
        "[^a-zA-Z]" +
          `(${scapedAbbreviationForRegex})|(${scapedAbbreviationForRegex.replace(
            /\u200e/g,
            ""
          )})` +
          "(?:\\)(?:\\))?)?$"
      ...
      if (stringOriginal.match(regexp)) {
        abbreviationScalingFactor = scale.toBase(
          abbreviationScalingFactor,
          abbreviation
        );
        break;
      }
  ...
  const formattedStringToNumber = (inputString, options) => {
    ...
    const stringWithLatinDigits = replaceNumeralSystemWithLatinNumbers(
      inputString,
      (_a = options.locale) === null || _a === void 0
        ? void 0
        : _a.numeralSystem
    ...
    if (options.zeroFormat && stringWithLatinDigits === options.zeroFormat) {
      ...
      (options.nullFormat && stringWithLatinDigits === options.nullFormat) ||
      !stringWithLatinDigits.replace(/[^0-9]+/g, "").length
    ) {
      value = null;
    } else {
      // Replaces the locale decimal delimiter with a dot (.)
      const decimalDelimiterFromLocale = locale.delimiters.decimal;
      const stringWithDotDecimalDelimiter =
        decimalDelimiterFromLocale === "."
          ? stringWithLatinDigits
          : stringWithLatinDigits
              .replace(/\./g, "")
              .replace(decimalDelimiterFromLocale, ".");
      // Determines the scaling factor from the abbreviations (if has abbreviations)
      const abbreviationScalingFactor = getScalingFactorFromAbbreviations(
        stringOriginal,
        options
      );
      ...
      const negativeFactor =
        (stringWithDotDecimalDelimiter.split("-").length +
          Math.min(
            stringWithDotDecimalDelimiter.split("(").length - 1,
            stringWithDotDecimalDelimiter.split(")").length - 1
          )) %
        2
          ? 1
          : -1;
      // Remove non numbers
      const numberAsString = stringWithDotDecimalDelimiter.replace(
        /[^0-9.]+/g,
        ""
      );
      value =
        negativeFactor *
        multiplyByPowerOfTen$1(
          +numberAsString,
          log10$1(abbreviationScalingFactor)
        );
    }
    return value;
  };
  ...
  const patternReplace = (patternMask, searchValue, replaceValue) => {
    return getPatternParts(patternMask)
      .map((e) =>
        e.escaped
          ? `'${e.value}'`
          : e.value.replace(searchValue, (_) => replaceValue)
      )
      .join("");
  };
  ...
  const signRule = (pattern) => {
    ...
    const negativeParentheses =
      ...
      stringIncludes$1(patternWithoutEscapedText, ")");
    const forceSign =
      !negativeParentheses && stringIncludes$1(patternWithoutEscapedText, "+");
  ...
  const abbreviationRule = (patternMask) => {
    ...
    if (patternIncludes(patternMask, "a")) {
      ...
      const abbreviationRegExpResult =
        patternWithoutEscapedText.match(/a(k|m|b|t)?/);
      compactUnit = !!abbreviationRegExpResult
        ? abbreviationRegExpResult[1]
        : null;
    }
    let outputPatternMask = patternMask;
    outputPatternMask = patternReplace(
      outputPatternMask,
      /a(k|m|b|t)?/,
      `'ɵa'`
    );
    return [
      outputPatternMask,
      { compact, compactUnit, compactAuto: compact && !compactUnit },
    ];
  };
  ...
  const splitStringInTwoParts = (str, separator) => {
    ...
    if (indexOfSearchChar === -1) {
      ...
      return [
        str.slice(0, indexOfSearchChar),
        str.slice(indexOfSearchChar + 1),
      ];
    }
  };
  ...
  const countChars = (string, char) => {
    return !string
      ? 0
      : string.split("").filter((stringChar) => stringChar === char).length;
  };
  /**
  ...
  const decimalPlacesRule = (patternMask) => {
    const patternWithoutEscapedText = patternRemoveEscapedText(patternMask);
    const patternPrecisionPart = splitStringInTwoParts$1(
    ...
    if (!!patternPrecisionPart) {
      ...
      if (stringIncludes$1(trimmedPatternPrecisionPart, "[")) {
        ...
        const patternPrecisionPartWithoutClosingBracket =
          trimmedPatternPrecisionPart.replace("]", "");
        // Isolates forced (left) vs optional (right) decimals
        const precisionSplitted = splitStringInTwoParts$1(
          patternPrecisionPartWithoutClosingBracket,
          "["
        );
        minimumFractionDigits = countChars(precisionSplitted[0], "0");
        maximumFractionDigits =
          minimumFractionDigits + countChars(precisionSplitted[1], "0");
      } else if (stringIncludes$1(trimmedPatternPrecisionPart, "#")) {
        // If it contains optional fraction digits marked with '#'
        minimumFractionDigits = countChars(
          trimmedPatternPrecisionPart.split("#")[0],
          "0"
        );
        ...
        minimumFractionDigits = countChars(
          trimmedPatternPrecisionPart.split("X")[0],
          "0"
        );
        maximumFractionDigits = 500;
      } else {
        const fractionDigits = countChars(
          trimmedPatternPrecisionPart.split(" ")[0],
          "0"
        );
  ...
  const minimumIntegerDigitsRule = (patternMask) => {
    ...
    const patternMaskIntegerPart = patternMaskWithoutEscapedText
      .split(".")[0]
      .split(",")[0];
  ...
  const addSignPositionIfItDoesNotExists = (patternMask) => {
    if (
      stringIncludes$1(patternMask, `'ɵs'`) ||
      stringIncludes$1(patternMask, `'ɵnps'`)
    )
      return patternMask;
    return patternMask.replace(`'ɵn'`, (_) => `'ɵs''ɵn'`);
  };
  const baseParsePattern = (inputPattern) => {
    const resolvedInputPattern =
      (isString$1(inputPattern) && inputPattern) || "0,0.##########";
    const [patternMaskAfterSignRule, signRules] =
      signRule$1(resolvedInputPattern);
    const [patternMaskAfterAbbreviationRule, abbreviationRules] =
      abbreviationRule$1(patternMaskAfterSignRule);
    const [
      patternMaskAfterOptionalDecimalPlacesRule,
      optionalDecimalPlacesRules,
    ] = optionalDecimalPlacesRule$1(patternMaskAfterAbbreviationRule);
    const outputPatternMask = patternMaskAfterOptionalDecimalPlacesRule;
    const outputPatternMaskWithoutEscapedText =
      patternRemoveEscapedText(outputPatternMask);
    ...
    const patternMaskAfterHandlingNumberPosition =
      numberPositionRule(outputPatternMask);
    const patternMaskWithEnsuredSignPosition = addSignPositionIfItDoesNotExists(
      patternMaskAfterHandlingNumberPosition
    );
    const patternMask = patternMaskWithEnsuredSignPosition;
    return Object.assign(
      Object.assign(
        Object.assign(
          Object.assign(Object.assign({}, signRules), abbreviationRules),
          optionalDecimalPlacesRules
        ),
        decimalPlacesRules
      ),
      { grouping, minimumIntegerDigits, patternMask }
    );
  };
  ...
  const rescaleRoundedValue = (
    ...
    const [newScaledValue, newScaledValueUnit] = scale.toBest(
      value,
      currentAbbreviationScale || ""
    );
  ...
  const resolveForcedAbbreviationUnit = (
    forcedAbbreviationUnit,
    abbreviationsFromLocale,
    value
  ) => {
    ...
    const scaleDefinitionFromLocale =
      (abbreviationsFromLocale === null || abbreviationsFromLocale === void 0
        ? void 0
        : abbreviationsFromLocale.split("|")) || [];
    let closestPowerOfTenWithAvailableAbbreviation = null;
    for (
      let distanceFromTarget = 0;
      distanceFromTarget < scaleDefinitionFromLocale.length;
      distanceFromTarget++
    ) {
      if (
        !scaleDefinitionFromLocale[
          targetPowerOfTenExponent - distanceFromTarget
        ]
      )
        continue;
      closestPowerOfTenWithAvailableAbbreviation =
        targetPowerOfTenExponent - distanceFromTarget;
      break;
    }
  ...
  const roundNumber = (number, precision, roundingFunction) => {
    ...
    const scaledValueForRounding = multiplyByPowerOfTen$1(
      number,
      resolvedPrecision
    );
    const roundedScaledValue = resolvedRoundingFunction(scaledValueForRounding);
    const roundedValue = multiplyByPowerOfTen$1(
      roundedScaledValue,
      -resolvedPrecision
    );
    return roundedValue;
  ...
  const formatPositiveExponentResult = (valueAsString) => {
    ...
    const [integerPartOfSignificand, fractionalPartOfSignificand] =
      splitStringInTwoParts$1(significand, ".");
    const numberOfZerosToAdd =
      exponentAsNumber - fractionalPartOfSignificand.length;
    return `${integerPartOfSignificand}${fractionalPartOfSignificand}${stringRepeat$1(
      "0",
      numberOfZerosToAdd
    )}`;
  };
   ...
   */
  const formatNegativeExponentResult = (
    value,
    exponentAsNumber,
    significandAsString
  ) => {
    ...
    const [integerPartOfSignificand, fractionalPartOfSignificand] =
      splitStringInTwoParts$1(significandAsString, ".");
    const absoluteIntegerPartOfSignificand =
      integerPartOfSignificand[0] === "-"
        ? integerPartOfSignificand.slice(1)
    ...
    for (let i = 0; i < negativeExponentAbsoluteValue; i += 1) {
      ...
      outputIntegerPartOfSignificand = outputIntegerPartOfSignificand.slice(
        0,
        outputIntegerPartOfSignificand.length - 1
      );
      outputFractionalPartOfSignificand =
        (firstCharInIntegerPart || "0") + outputFractionalPartOfSignificand;
    }
    return `${value < 0 ? "-" : ""}${
      outputIntegerPartOfSignificand || 0
    }.${outputFractionalPartOfSignificand}`;
  };
  /**
  ...
  const addTrailingZerosInFractionalPart = (
    valueAsString,
    minimumFractionDigits
  ) => {
    const [integerPart, fractionalPart] = splitStringInTwoParts$1(
      valueAsString,
      "."
    );
    return `${integerPart}.${
      fractionalPart +
      stringRepeat$1("0", minimumFractionDigits - fractionalPart.length)
    }`;
  };
  /**
  ...
  const numberToFixed = (
    ...
    fractionDigits,
    roundingFunction,
    optionalFractionDigits
  ) => {
    const valueAsString = numberToNonExponentialString$1(finiteNumber);
    const minimumFractionDigits =
      fractionDigits - (optionalFractionDigits || 0);
    const fractionalPartOfValueAsString = splitStringInTwoParts$1(
      valueAsString,
      "."
    )[1];
    const targetFractionDigitsAmount = !!fractionalPartOfValueAsString
      ? Math.min(
          Math.max(fractionalPartOfValueAsString.length, minimumFractionDigits),
          fractionDigits
        )
      : minimumFractionDigits;
    const roundedValue = roundNumber$1(
      finiteNumber,
      targetFractionDigitsAmount,
      roundingFunction
    );
  ...
  const roundValueAndAddTrailingZeros = (value, patternRules, options) => {
    ...
    const shouldIncludeDecimalPlaces =
      minimumFractionDigits > 0 || maximumFractionDigits > 0;
    if (shouldIncludeDecimalPlaces) {
      const optionalDecimalDigitsCount =
        maximumFractionDigits - minimumFractionDigits;
      return numberToFixed$1(
        value,
        maximumFractionDigits,
        resolvedRoundingFunction,
        optionalDecimalDigitsCount
      );
    } else {
   ...
   */
  const addOrRemoveLeadingZerosToValue = (
    ...
    return valueIntegerPartWithoutSign.length >= minimumIntegerDigits
      ...
      : `${stringRepeat$1(
          "0",
          minimumIntegerDigits - valueIntegerPartWithoutSign.length
        )}${valueIntegerPartWithoutSign}`;
  };
  ...
  const addSignInfoToFullFormattedNumber = (
    ...
    if (negativeParentheses && isValueNegative) {
      output = output.replace(/'ɵ(nps|npe)'/g, (match) =>
        match === `'ɵnps'` ? "(" : ")"
      );
    } else if (forceSign) {
  ...
  const splitNumberIntegerAndDecimalParts = (valueAsString, patternRules) => {
    ...
    const [integerPart, decimalPart] = splitStringInTwoParts$1(
      valueAsString,
      "."
    );
  ...
  const addThousandsSeparatorToValueIntegerPart = (
    valueIntegerPartWithLeadingZerosAndWithoutSign,
    patternRules,
    options
  ) => {
    ...
    const digitGrouping = !!(digitGroupingStyle === null ||
    digitGroupingStyle === void 0
      ? void 0
      : digitGroupingStyle.length)
      ? digitGroupingStyle
      : [3];
  ...
  var addThousandsSeparatorToValueIntegerPart$1 =
    addThousandsSeparatorToValueIntegerPart;
  ...
  const applyAbbreviationLocalizedUnitToPatternMask = (
    ...
    if (abbreviationLocalizedUnit) {
       ...
       */
      return patternMask.replace(
        `'ɵa'`,
        (_) => `'${abbreviationLocalizedUnit.replace(/'/g, (_) => "\\'")}'`
      );
    } else {
  ...
  var applyAbbreviationLocalizedUnitToPatternMask$1 =
    applyAbbreviationLocalizedUnitToPatternMask;
  ...
  const scaleAndRoundValue = (number, patternRules, options) => {
    ...
    if (!patternRules.compact) {
      const roundedValueAsString = roundValueAndAddTrailingZeros$1(
        number,
        patternRules,
        options
      );
    ...
    const [scaledValue, scaledValueLocalizedUnit] =
      scaleValueWithAbbreviation$1(number, patternRules, options);
    const roundedScaledValue = +roundValueAndAddTrailingZeros$1(
      scaledValue,
      patternRules,
      options
    );
    const [rescaledValue, rescaledValueLocalizedUnit] = rescaleRoundedValue$1(
      +roundedScaledValue,
      scaledValueLocalizedUnit,
      patternRules,
      options
    );
    const roundedRescaledValueAsStringWithTrailingZeros =
      roundValueAndAddTrailingZeros$1(rescaledValue, patternRules, options);
    return [
      roundedRescaledValueAsStringWithTrailingZeros,
      rescaledValueLocalizedUnit,
    ];
  };
  const numberToFormattedNumber = (number, pattern, options) => {
    ...
    const [valueAsString, localizedAbbreviationUnit] = scaleAndRoundValue(
      resolvedValue,
      patternRules,
      options
    );
    ...
    const isValueNegative = options.signedZero
      ? number < 0
      : +valueAsString < 0;
    const isValueZero = options.signedZero
      ? number === 0
      : +valueAsString === 0;
    const valueAsStringWithoutSign = removeSignIfExists$1(valueAsString);
    const [integerPart, decimalPart] = splitNumberIntegerAndDecimalParts$1(
      valueAsStringWithoutSign,
      patternRules
    );
    const valueIntegerPartWithLeadingZeros = addOrRemoveLeadingZerosToValue$1(
      integerPart,
      patternRules
    );
    const valueIntegerPartWithThousandsSeparator =
      addThousandsSeparatorToValueIntegerPart$1(
        valueIntegerPartWithLeadingZeros,
        patternRules,
        options
      );
    const numeralSystemFromLocale = options.locale.numeralSystem;
    const integerPartWithNumeralSystem = replaceDigitsWithNumeralSystem$1(
      valueIntegerPartWithThousandsSeparator,
      numeralSystemFromLocale
    );
    const decimalPartWithNumeralSystem = replaceDigitsWithNumeralSystem$1(
      decimalPart,
      numeralSystemFromLocale
    );
    const fullNumberWithNumeralSystem =
      integerPartWithNumeralSystem +
      (!!decimalPartWithNumeralSystem
        ? (((_a = options.locale.delimiters) === null || _a === void 0
            ? void 0
            : _a.decimal) || ".") + decimalPartWithNumeralSystem
        : "");
    // Assembling
    const patternMaskWithAbbreviation =
      applyAbbreviationLocalizedUnitToPatternMask$1(
        patternRules.patternMask,
        localizedAbbreviationUnit,
        patternRules.compact
      );
    const patternMaskWithNumber = patternMaskWithAbbreviation.replace(
      `'ɵn'`,
      (_) => `'${fullNumberWithNumeralSystem.replace(/'/g, "\\'")}'`
    );
    const patternMaskWithSignInfo = addSignInfoToFullFormattedNumber$1(
      patternMaskWithNumber,
      isValueNegative,
      isValueZero,
      patternRules
    );
    const cleanPatternMask = patternMaskWithSignInfo.replace(
      /'ɵ(nps|npe|s|a|n)'/g,
      ""
    );
    const fullFormattedValueWithNormalizedText =
      patternStripAndNormalizeEscapedText(cleanPatternMask);
  ...
  const bpsFormatter = {
    ...
    format: (number, pattern, options) => {
      ...
      const formatResult = numberToFormattedNumber$1(
        scaledValue,
        patternWithEscapedBPS,
        options
      );
      return formatResult.replace("ɵBPSɵ", "BPS");
    },
    unformat: (string, options) => {
      const number = formattedStringToNumber$1(
        string.replace(/\s?BPS/, ""),
        options
      );
      return isFiniteNumber$1(number)
        ? multiplyByPowerOfTen$1(number, -4)
        : number;
    },
  };
  ...
  const timeFormatter = {
    ...
    format: (number) => {
      ...
      const seconds = truncateNumber$1(
        absoluteValue - hours * 3600 - minutes * 60
      );
  ...
  const bytesDecimalScale = unitScale({
    base: "B",
    scale: toObject$1(decimalSuffixes, (unit, unitIndex) => [
      unit,
      Math.pow(1000, unitIndex),
    ]),
  });
  const bytesBinaryScale = unitScale({
    base: "B",
    scale: toObject$1(binarySuffixes, (unit, unitIndex) => [
      unit,
  ...
  const bytesFormatter = {
    ...
    regexps: {
      ...
      unformat: (string, options) =>
        options.type === "bytes"
          ? new RegExp(unformatRegex).test(string)
          : false,
    },
    format: (number, pattern, options) => {
      const scale = patternIncludes(pattern, "bb")
        ? bytesBinaryScale
        : bytesDecimalScale;
      const [scaledValue, scaledValueUnit] = scale.toBest(number, "B");
      const patternWithEscapedBytes = patternReplace(
        pattern,
        /b[bd]/,
        `'ɵbytesɵ'`
      );
    ...
    unformat: (string, options) => {
      var _a;
      const number = formattedStringToNumber$1(
        string.replace(new RegExp(unformatRegex), ""),
        options
      );
      const suffix =
        ((_a = string.match(unformatRegex)) === null || _a === void 0
          ? void 0
          : _a[0]) || "";
      const scale = !!bytesBinaryScale.scaleDefinition.scale[suffix]
        ? bytesBinaryScale
        : bytesDecimalScale;
  ...
  const ordinalFormatter = {
    ...
    format: (number, pattern, options) => {
      var _a, _b;
      const localizedOrdinal =
        ((_b = (_a = options.locale).ordinal) === null || _b === void 0
          ? void 0
          : _b.call(_a, number)) || "";
      const patternWithEscapedOrdinal = patternReplace(pattern, /o/, `'ɵordɵ'`);
      const formatResult = numberToFormattedNumber$1(
        number,
        patternWithEscapedOrdinal,
        options
      );
  ...
  const currencyFormatter = {
    ...
    format: (number, pattern, options) => {
      ...
      const currencyFromOptions =
        (_a = options.currency) === null || _a === void 0
          ? void 0
          : _a.toUpperCase();
      const localizedCurrencySymbol =
        currencySymbolsMap[currencyFromOptions] || currencyFromOptions || "";
      const patternWithEscapedCurrencySymbol = patternReplace(
        pattern,
        /\$/,
        `'ɵcurrencyɵ'`
      );
      const formatResult = numberToFormattedNumber$1(
        number,
        patternWithEscapedCurrencySymbol,
        options
      );
  ...
  const percentageFormatter = {
    ...
    format: (number, pattern, options) => {
      const hasNotScalePercentageSymbolInPattern = patternIncludes(
        pattern,
        "%!"
      );
      const scaledValue =
        options.scalePercentage && !hasNotScalePercentageSymbolInPattern
          ? multiplyByPowerOfTen$1(number, 2)
          : number;
      const patternWithEscapedPercentage = patternReplace(
        pattern,
        /%!?/,
        `'ɵ%ɵ'`
      );
    ...
    unformat: (string, options) => {
      ...
      return number && options.scalePercentage
        ? multiplyByPowerOfTen$1(number, -2)
        : number;
    },
  };
  ...
  const exponentialFormatter = {
    ...
    format: (number, pattern, options) => {
      const exponential =
        typeof number === "number" && !isNaNNumber$1(number)
          ? number.toExponential()
          : "0e+0";
      const parts = splitStringInTwoParts$1(exponential, "e");
      const patternWithoutExponential = patternReplace(
        pattern,
        /e[+|-]{1}0/i,
        ""
      );
    ...
    unformat: (string, options) => {
      var _a;
      const value = formattedStringToNumber$1(
        string.replace(/e[+-]{1}[0-9]{1,3}/i, ""),
        options
      );
      const powerOfTenExponent = +(
        ((_a = string.match(/e([+-]{1}[0-9]{1,3})/i)) === null || _a === void 0
          ? void 0
          : _a[1]) || "0"
      );
      return isFiniteNumber$1(value)
        ? multiplyByPowerOfTen$1(value, powerOfTenExponent)
        : value;
    },
  };
  ...
  const areDelimitersValid = (delimiters) => {
    return (
      !!(delimiters === null || delimiters === void 0
        ? void 0
        : delimiters.decimal) &&
      isString$1(
        delimiters === null || delimiters === void 0
  ...
  const resolveOptionsLocale = (optionsLocale) => {
    ...
    if (!isObject$1(optionsLocale)) return defaultLocale;
    return merge(optionsLocale, {
      delimiters: areDelimitersValid(optionsLocale.delimiters)
        ? optionsLocale.delimiters
        : defaultLocale.delimiters,
  ...
  const resolveRoundingOption = (roundingOption) => {
    switch (roundingOption) {
      ...
      case "floor":
        return Math.floor;
      case "truncate":
        return truncateNumber$1;
      case "half-up":
        return Math.round;
      case "half-away-from-zero":
        return roundHalfAwayFromZero$1;
      default:
        return isFunction$1(roundingOption)
          ? roundingOption
          : roundHalfAwayFromZero$1;
    }
  };
  const resolveOptionsFormatters = (optionsFormatters) => {
    if (!optionsFormatters) return BUILT_IN_FORMATTERS$1;
  ...
  const getUnformatFunctionIfMatch = (input, resolvedOptions) => {
    for (const formatter of resolvedOptions.formatters) {
      ...
      if (!matcher) continue;
      const matcherResult = isFunction$1(matcher)
        ? matcher(input, resolvedOptions)
        : !!input.match(matcher);
  ...
  const parse$2 = (input, options) => {
    ...
    if (isNil$1(input) || isNaNNumber$1(input)) {
      ...
      if (resolvedOptions.zeroFormat && input === resolvedOptions.zeroFormat) {
        ...
        resolvedOptions.nullFormat &&
        input === resolvedOptions.nullFormat
      ) {
        value = null;
      } else {
        // Removes non-breaking spaces if they exists
        const inputStringWithNormalSpaces = input.replace(/\u00A0/, " ");
        const unformatFunctionFromFormatters = getUnformatFunctionIfMatch(
          inputStringWithNormalSpaces,
          resolvedOptions
        );
        const unformatFunction =
          unformatFunctionFromFormatters || formattedStringToNumber$1;
  ...
  const getFormatFunctionIfMatch = (pattern, resolvedOptions) => {
    ...
    for (const formatter of resolvedOptions.formatters) {
      ...
      if (!matcher) continue;
      const matcherResult = isFunction$1(matcher)
        ? matcher(pattern, resolvedOptions)
        : !!patternWithoutEscapedText.match(matcher);
    ...
    try {
      ...
      if (resolvedValue === Infinity || resolvedValue === -Infinity) {
        ...
        return isString$1(resolvedOptions.nanFormat)
          ...
          : isString$1(resolvedOptions.nullFormat)
          ? resolvedOptions.nullFormat
          : "";
      } else if (isNil$1(resolvedValue)) {
        output = isString$1(resolvedOptions.nullFormat)
          ? resolvedOptions.nullFormat
          : "";
      } else if (
        resolvedValue === 0 &&
        isString$1(resolvedOptions.zeroFormat)
      ) {
        output = resolvedOptions.zeroFormat;
      } else {
        // <!> Here value should always be a number
        const resolvedValueAsNumber = resolvedValue || 0;
        const formatFunctionFromFormatters = getFormatFunctionIfMatch(
          resolvedPattern,
          resolvedOptions
        );
        const resolvedFormatFunction =
          formatFunctionFromFormatters || numberToFormattedNumber$1;
        output = resolvedFormatFunction(
          resolvedValueAsNumber,
          resolvedPattern,
          resolvedOptions
        );
      }
      ...
      return (
        ((_a = options) === null || _a === void 0 ? void 0 : _a._errorFormat) ||
        ""
      );
    }
  };
  ...
  const createFormatFunction = (options) => {
    ...
    return (value, arg2, arg3) => {
      const pattern = isString$1(arg2) ? arg2 : null;
      const optionsFromArguments = isObject$1(arg2)
        ? arg2
        : isObject$1(arg3)
        ? arg3
        : {};
      return format$1$1(
        value,
        pattern,
        merge(baseOptions, optionsFromArguments)
      );
    };
  };
  ...
  const rtlCharsRanges =
    "\u0591-\u07FF\u200F\u202B\u202E\uFB1D-\uFDFD\uFE70-\uFEFC";
  const rtlDirCheck = new RegExp(
    "^[^" + rtlCharsRanges + "]*?[" + rtlCharsRanges + "]"
  );
  const isRTL = (string) => rtlDirCheck.test(string);
  const appendLeftToRightMarkIfIsRTL = (string) =>
    isRTL(string) ? string + leftToRightMark : string;
  ...
  const getNumeralSystemDigits = (languageTag) => {
    try {
      const localizedNumber = (1234567890).toLocaleString(languageTag, {
        useGrouping: false,
      });
      const lookupObject = {};
      ...
      const digitsWithoutGroupingDelimiters = repeatedChar
        ? localizedNumber.replace(
            new RegExp(escapeRegexString$1(repeatedChar || ""), "g"),
            ""
          )
        : localizedNumber;
      const digitsAsArray = digitsWithoutGroupingDelimiters.split("");
      const sortedDigits = [
        digitsAsArray[digitsAsArray.length - 1],
        ...digitsAsArray.slice(0, -1),
      ];
  ...
  const getGroupingAndFractionDelimiters = (languageTag, digits) => {
    try {
      ...
      const localizedNumberWithoutDigits = localizedNumber.replace(
        new RegExp(`[${escapeRegexString$1(digits)}]`, "g"),
        ""
      );
      const [groupingDelimiter = ",", fractionDelimiter = "."] = unique$1(
        localizedNumberWithoutDigits.split("")
      );
  ...
  const getGroupingStyle = (languageTag, groupingDelimiter) => {
    ...
    if (
      languagesWith4DigitsGroupingStyle.some(
        (language) => languageTag.indexOf(language) === 0
      )
    )
      return [4];
    try {
      ...
      (100000000000)
        ...
        .split("")
        .reverse()
        .forEach((digitOrGroupingDelimiter) => {
  ...
  const getAbbreviations = (languageTag, digits, type) => {
    try {
      ...
      let abbreviations = "";
      for (let i = 1; i < 50; i++) {
        const abbreviationResultForOne = (+(
          1 + stringRepeat$1("0", i)
        )).toLocaleString(languageTag, intlFormatOptions);
        if (
          new RegExp(`^${digitOfOne}[^${digitOfZero}]+$`).test(
            abbreviationResultForOne
          )
        ) {
          if (type === "long") {
            const abbreviationResultForTwo = (+(
              2 + stringRepeat$1("0", i)
            )).toLocaleString(languageTag, intlFormatOptions);
            const abbreviationOne = abbreviationResultForOne
              .replace(new RegExp(`${digitOfOne}`, "g"), "")
              .trim();
            const abbreviationTwo = abbreviationResultForTwo
              .replace(new RegExp(`${digitOfTwo}`, "g"), "")
              .trim();
            abbreviations +=
              "|" +
              appendLeftToRightMarkIfIsRTL(abbreviationOne) +
              ":::" +
              appendLeftToRightMarkIfIsRTL(abbreviationTwo);
          } else {
            const abbreviation = abbreviationResultForOne
              .replace(new RegExp(`${digitOfOne}`, "g"), "")
              .trim();
  ...
  const baseGetLocaleFromPlatform = (languageTag) => {
    ...
    const delimiters = getGroupingAndFractionDelimiters(
      resolvedLanguageTag,
      resolvedDigits
    );
    const [groupingDelimiter, fractionDelimiter] =
      !!delimiters && delimiters.length >= 2 ? delimiters : [",", "."];
    const groupingStyle = getGroupingStyle(
      resolvedLanguageTag,
      groupingDelimiter
    );
    const shortAbbreviations = getAbbreviations(
      resolvedLanguageTag,
      resolvedDigits,
      "short"
    );
    const longAbbreviations = getAbbreviations(
      resolvedLanguageTag,
      resolvedDigits,
      "long"
    );
    return {
      ...
      digitGroupingStyle: !!(groupingStyle === null || groupingStyle === void 0
        ? void 0
        : groupingStyle.length)
        ? groupingStyle
        : undefined,
      numeralSystem:
        digits !== "0123456789"
          ? digits === null || digits === void 0
            ? void 0
            : digits.split("").map(appendLeftToRightMarkIfIsRTL)
          : undefined,
