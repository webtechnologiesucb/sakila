/**
 * Create Date/Time Picker (for Sakila 2024)
...
ew.createDateTimePicker = function (formid, id, options) {
  if (id.includes("$rowindex$")) return;
  let $ = jQuery,
  ...
  if (!el || $input.data(dataKey) || $input.parent().data(dataKey)) return;
  ...
  if (args.inputGroup !== false) {
    ...
    $btn = $(
      '<button class="btn btn-default" type="button"><i class="fa-regular fa-calendar"></i></button>'
    ).on("click." + dataKey, function () {
      $textbox.removeClass("is-invalid");
    });
    $input
      .addClass(namespace + "-input")
      .attr("data-target", "#" + id)
      .wrap(
        `<div class="input-group${
          isInvalid ? " is-invalid" : ""
        }" id="${id}" data-target-input="nearest" data-td-target-toggle="nearest"></div>`
      )
      .after(
        $btn
          .attr("data-target", "#" + id)
          .attr(`data-td-toggle`, "datetimepicker")
      )
  ...
  document.addEventListener("changetheme", (e) =>
    td.updateOptions({ display: { theme: e.detail } })
  );
  return td;
};
