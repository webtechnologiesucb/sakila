...
class Address extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->district->Nullable = false; // NOT NULL field
        $this->district->Required = true; // Required field
        $this->district->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->city_id->Lookup = new Lookup($this->city_id, 'city', false, 'city_id', ["city","","",""], '', '', [], [], [], [], [], [], false, '`city` ASC', '', "`city`");
                break;
            default:
                $this->city_id->Lookup = new Lookup($this->city_id, 'city', false, 'city_id', ["city","","",""], '', '', [], [], [], [], [], [], false, '`city` ASC', '', "`city`");
                break;
        ...
        // postal_code
        $this->postal_code = new DbField(
            $this, // Table
            'x_postal_code', // Variable name
            'postal_code', // Name
            '`postal_code`', // Expression
            '`postal_code`', // Basic search expression
            200, // Type
            10, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`postal_code`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->postal_code->InputTextType = "text";
        $this->postal_code->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        ...
        $this->location->CustomDataType = new GeometryType();
        $this->location->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['address_id']) && !EmptyValue($this->address_id->CurrentValue)) {
                $rs['address_id'] = $this->address_id->CurrentValue;
            }
        }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->address_id->OldValue) && !$current ? $this->address_id->OldValue : $this->address_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "addressview" => $Language->phrase("View"),
            "addressedit" => $Language->phrase("Edit"),
            "addressadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AddressView",
            Config("API_ADD_ACTION") => "AddressAdd",
            Config("API_EDIT_ACTION") => "AddressEdit",
            Config("API_DELETE_ACTION") => "AddressDelete",
            Config("API_LIST_ACTION") => "AddressList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("addressview", $parm);
        } else {
            $url = $this->keyUrl("addressview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("addressedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("addresslist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("addressadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("addresslist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("addressdelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"address_id\":" . VarToJson($this->address_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'address');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'address', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'address', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'address', $fldname, $key, $oldvalue);
            }
        }
    }
