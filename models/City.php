...
class City extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // city_id
        $this->city_id = new DbField(
            $this, // Table
            'x_city_id', // Variable name
            'city_id', // Name
            '`city_id`', // Expression
            '`city_id`', // Basic search expression
            18, // Type
            5, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`city_id`', // Virtual expression
            false, // Is virtual
        ...
        $this->city_id->InputTextType = "text";
        $this->city_id->Raw = true;
        $this->city_id->IsAutoIncrement = true; // Autoincrement field
        $this->city_id->IsPrimaryKey = true; // Primary key field
        $this->city_id->Nullable = false; // NOT NULL field
        $this->city_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->city_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->country_id->Raw = true;
        $this->country_id->Nullable = false; // NOT NULL field
        $this->country_id->Required = true; // Required field
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->country_id->Lookup = new Lookup($this->country_id, 'country', false, 'country_id', ["country","","",""], '', '', [], [], [], [], [], [], false, '`country` ASC', '', "`country`");
                break;
            default:
                $this->country_id->Lookup = new Lookup($this->country_id, 'country', false, 'country_id', ["country","","",""], '', '', [], [], [], [], [], [], false, '`country` ASC', '', "`country`");
                break;
        ...
        $this->country_id->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "city";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->city_id->setDbValue($conn->lastInsertId());
            $rs['city_id'] = $this->city_id->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['city_id']) && !EmptyValue($this->city_id->CurrentValue)) {
                $rs['city_id'] = $this->city_id->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('city_id', $rs)) {
                AddFilter($where, QuotedName('city_id', $this->Dbid) . '=' . QuotedValue($rs['city_id'], $this->city_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->city_id->CurrentValue : $this->city_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->city_id->CurrentValue = $keys[0];
            } else {
                $this->city_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('city_id', $row) ? $row['city_id'] : null;
        } else {
            $val = !EmptyValue($this->city_id->OldValue) && !$current ? $this->city_id->OldValue : $this->city_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@city_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("citylist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "cityview" => $Language->phrase("View"),
            "cityedit" => $Language->phrase("Edit"),
            "cityadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "CityView",
            Config("API_ADD_ACTION") => "CityAdd",
            Config("API_EDIT_ACTION") => "CityEdit",
            Config("API_DELETE_ACTION") => "CityDelete",
            Config("API_LIST_ACTION") => "CityList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("cityview", $parm);
        } else {
            $url = $this->keyUrl("cityview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("citylist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("citylist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("citydelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"city_id\":" . VarToJson($this->city_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->city_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->city_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("city_id") ?? Route("city_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->city_id->CurrentValue = $key;
            } else {
                $this->city_id->OldValue = $key;
            }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'city');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'city', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'city', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'city', $fldname, $key, $oldvalue);
            }
        }
    }
