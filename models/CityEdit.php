...
class CityEdit extends City
{
    ...
    // CSS class/style
    public $CurrentPageName = "cityedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("city_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->city_id->setQueryStringValue($keyValue);
                $this->city_id->setOldValue($this->city_id->QueryStringValue);
            } elseif (Post("city_id") !== null) {
                $this->city_id->setFormValue(Post("city_id"));
                $this->city_id->setOldValue($this->city_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("city_id") ?? Route("city_id")) !== null) {
                    $this->city_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("citylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "citylist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "citylist") {
                            ...
                            $returnUrl = "citylist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if ($curVal != "") {
                ...
                $this->country_id->ViewValue = $this->country_id->Lookup !== null && is_array($this->country_id->lookupOptions()) && count($this->country_id->lookupOptions()) > 0 ? $curVal : null;
            }
            if ($this->country_id->ViewValue !== null) { // Load from cache
                $this->country_id->EditValue = array_values($this->country_id->lookupOptions());
                if ($this->country_id->ViewValue == "") {
                ...
                if ($curVal == "") {
                    ...
                    $filterWrk = SearchFilter($this->country_id->Lookup->getTable()->Fields["country_id"]->searchExpression(), "=", $this->country_id->CurrentValue, $this->country_id->Lookup->getTable()->Fields["country_id"]->searchDataType(), "");
                }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->country_id->Visible && $this->country_id->Required) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // country_id
        $this->country_id->setDbValueDef($rsnew, $this->country_id->CurrentValue, $this->country_id->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['country_id'])) { // country_id
            $this->country_id->CurrentValue = $row['country_id'];
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("citylist"), "", $this->TableVar, true);
        $pageId = "edit";
