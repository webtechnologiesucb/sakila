...
class Country extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->country_id->IsAutoIncrement = true; // Autoincrement field
        $this->country_id->IsPrimaryKey = true; // Primary key field
        $this->country_id->Nullable = false; // NOT NULL field
        $this->country_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->country_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "country";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->country_id->setDbValue($conn->lastInsertId());
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['country_id']) && !EmptyValue($this->country_id->CurrentValue)) {
                $rs['country_id'] = $this->country_id->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('country_id', $rs)) {
                AddFilter($where, QuotedName('country_id', $this->Dbid) . '=' . QuotedValue($rs['country_id'], $this->country_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->country_id->CurrentValue : $this->country_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->country_id->CurrentValue = $keys[0];
            } else {
                $this->country_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('country_id', $row) ? $row['country_id'] : null;
        } else {
            $val = !EmptyValue($this->country_id->OldValue) && !$current ? $this->country_id->OldValue : $this->country_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@country_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("countrylist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "countryview" => $Language->phrase("View"),
            "countryedit" => $Language->phrase("Edit"),
            "countryadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "CountryView",
            Config("API_ADD_ACTION") => "CountryAdd",
            Config("API_EDIT_ACTION") => "CountryEdit",
            Config("API_DELETE_ACTION") => "CountryDelete",
            Config("API_LIST_ACTION") => "CountryList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("countryview", $parm);
        } else {
            $url = $this->keyUrl("countryview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("countryedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("countrylist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("countryadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("countrylist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("countrydelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"country_id\":" . VarToJson($this->country_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->country_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->country_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("country_id") ?? Route("country_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'country');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'country', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'country', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'country', $fldname, $key, $oldvalue);
            }
        }
    }
