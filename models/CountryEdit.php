 ...
 * Page class
 */
class CountryEdit extends Country
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "CountryEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "countryedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("country_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->country_id->setQueryStringValue($keyValue);
                $this->country_id->setOldValue($this->country_id->QueryStringValue);
            } elseif (Post("country_id") !== null) {
                $this->country_id->setFormValue(Post("country_id"));
                $this->country_id->setOldValue($this->country_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("country_id") ?? Route("country_id")) !== null) {
                    $this->country_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("countrylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "countrylist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "countrylist") {
                            ...
                            $returnUrl = "countrylist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("countrylist"), "", $this->TableVar, true);
        $pageId = "edit";
