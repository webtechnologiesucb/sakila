...
class Customer extends DbTable
{
    ...
    public function __construct()
    {
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->address_id->Lookup = new Lookup($this->address_id, 'address', false, 'address_id', ["address","address2","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`address`, ''),'" . ValueSeparator(1, $this->address_id) . "',COALESCE(`address2`,''))");
                break;
            default:
                $this->address_id->Lookup = new Lookup($this->address_id, 'address', false, 'address_id', ["address","address2","",""], '', '', [], [], [], [], [], [], false, '', '', "CONCAT(COALESCE(`address`, ''),'" . ValueSeparator(1, $this->address_id) . "',COALESCE(`address2`,''))");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->active->Lookup = new Lookup($this->active, 'customer', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->active->Lookup = new Lookup($this->active, 'customer', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
        ...
        $this->create_date = new DbField(
            $this, // Table
            'x_create_date', // Variable name
            'create_date', // Name
            '`create_date`', // Expression
            CastDateFieldForLike("`create_date`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
            '`create_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->create_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['customer_id']) && !EmptyValue($this->customer_id->CurrentValue)) {
                $rs['customer_id'] = $this->customer_id->CurrentValue;
            }
        }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->customer_id->OldValue) && !$current ? $this->customer_id->OldValue : $this->customer_id->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "CustomerView",
            Config("API_ADD_ACTION") => "CustomerAdd",
            Config("API_EDIT_ACTION") => "CustomerEdit",
            Config("API_DELETE_ACTION") => "CustomerDelete",
            Config("API_LIST_ACTION") => "CustomerList",
            default => ""
        };
    }
    ...
    {
        $json = "";
        $json .= "\"customer_id\":" . VarToJson($this->customer_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'customer');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'customer', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'customer', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'customer', $fldname, $key, $oldvalue);
            }
        }
    }
