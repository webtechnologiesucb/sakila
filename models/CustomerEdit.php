...
class CustomerEdit extends Customer
{
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("customer_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->customer_id->setQueryStringValue($keyValue);
                $this->customer_id->setOldValue($this->customer_id->QueryStringValue);
            } elseif (Post("customer_id") !== null) {
                $this->customer_id->setFormValue(Post("customer_id"));
                $this->customer_id->setOldValue($this->customer_id->FormValue);
            } else {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // create_date
        $this->create_date->setDbValueDef($rsnew, UnFormatDateTime($this->create_date->CurrentValue, $this->create_date->formatPattern()), $this->create_date->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['create_date'])) { // create_date
            $this->create_date->CurrentValue = $row['create_date'];
        }
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['create_date']); // create_date
