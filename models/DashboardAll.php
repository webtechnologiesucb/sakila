 ...
 * Page class
 */
class DashboardAll extends ReportTable
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DashboardAll";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = 'DashboardAll';
        $this->TableName = 'DashboardAll';
        ...
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'DashboardAll');
        }
    ...
    public function renderItem($view, $id)
    {
        ...
        try {
            if ($id == 1) {
                $Page = Container("SalesByFilmCategoryList");
                $Page->run();
                $Page->SalesByCategory->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->SalesByCategory->render("ew-dashboard-chart", 400, 400) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 2) {
                $Page = Container("SalesByStoreList");
                $Page->run();
                $Page->SalesByStore->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->SalesByStore->render("ew-dashboard-chart", 400, 400) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 3) {
                $Page = Container("SalesByStoreList");
                $Page->run();
                $Page->SalesByManager->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->SalesByManager->render("ew-dashboard-chart", 400, 400) .
                    GetDebugMessage(); // Return chart content
            }
            if ($id == 4) {
                $Page = Container("SalesByFilmCategoryList");
            ...
            if ($id == 5) {
                $Page = Container("StaffList2List");
                $Page->run();
                $Page->StaffByCity->DrillDownUrl = ""; // No drill down for dashboard
                $content = IsApi() && !EmptyValue($this->Export) ? "" : // Disable chart content for API export
                    $Page->StaffByCity->render("ew-dashboard-chart", 400, 400) .
                    GetDebugMessage(); // Return chart content
            }
        } catch (\Exception $e) {
            $GLOBALS["Page"] = $this;
