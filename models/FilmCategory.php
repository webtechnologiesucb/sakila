...
/**
 * Table class for film_category
 */
class FilmCategory extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "film_category";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->film_id->Required = true; // Required field
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->film_id->Lookup = new Lookup($this->film_id, 'film', false, 'film_id', ["title","","",""], '', '', [], [], [], [], [], [], false, '', '', "`title`");
                break;
            default:
                $this->film_id->Lookup = new Lookup($this->film_id, 'film', false, 'film_id', ["title","","",""], '', '', [], [], [], [], [], [], false, '', '', "`title`");
                break;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->category_id->Lookup = new Lookup($this->category_id, 'category', false, 'category_id', ["name","","",""], '', '', [], [], [], [], [], [], false, '`name` ASC', '', "`name`");
                break;
            default:
                $this->category_id->Lookup = new Lookup($this->category_id, 'category', false, 'category_id', ["name","","",""], '', '', [], [], [], [], [], [], false, '`name` ASC', '', "`name`");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "film_category";
    }
    ...
    {
        return "`film_id` = @film_id@ AND `category_id` = @category_id@";
    }
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 2) {
            ...
            if ($current) {
                $this->category_id->CurrentValue = $keys[1];
            } else {
                $this->category_id->OldValue = $keys[1];
            }
        }
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("filmcategorylist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "filmcategoryview" => $Language->phrase("View"),
            "filmcategoryedit" => $Language->phrase("Edit"),
            "filmcategoryadd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "FilmCategoryView",
            Config("API_ADD_ACTION") => "FilmCategoryAdd",
            Config("API_EDIT_ACTION") => "FilmCategoryEdit",
            Config("API_DELETE_ACTION") => "FilmCategoryDelete",
            Config("API_LIST_ACTION") => "FilmCategoryList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("filmcategoryview", $parm);
        } else {
            $url = $this->keyUrl("filmcategoryview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmcategoryedit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmcategorylist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmcategoryadd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmcategorylist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("filmcategorydelete", $parm);
        }
    }
    ...
    public function keyUrl($url, $parm = "")
    {
        ...
        if ($this->category_id->CurrentValue !== null) {
            $url .= $this->RouteCompositeKeySeparator . $this->encodeKeyValue($this->category_id->CurrentValue);
        } else {
            return "javascript:ew.alert(ew.language.phrase('InvalidRecord'));";
        }
        if ($parm != "") {
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->film_id->CurrentValue = $key[0];
            } else {
                $this->film_id->OldValue = $key[0];
            }
            if ($setCurrent) {
                $this->category_id->CurrentValue = $key[1];
            } else {
                $this->category_id->OldValue = $key[1];
            }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'film_category');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'film_category', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'film_category', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'film_category', $fldname, $key, $oldvalue);
            }
        }
    }
