...
/**
 * Table class for film_list
 */
class FilmList2 extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // FID
        $this->FID = new DbField(
            $this, // Table
            'x_FID', // Variable name
            'FID', // Name
            '`FID`', // Expression
            '`FID`', // Basic search expression
            18, // Type
            5, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`FID`', // Virtual expression
            false, // Is virtual
        ...
        $this->FID->addMethod("getDefault", fn() => 0);
        $this->FID->InputTextType = "text";
        $this->FID->Raw = true;
        $this->FID->Nullable = false; // NOT NULL field
        $this->FID->Required = true; // Required field
        $this->FID->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->FID->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->price->addMethod("getDefault", fn() => 4.99);
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->rating->Lookup = new Lookup($this->rating, 'film_list2', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->rating->Lookup = new Lookup($this->rating, 'film_list2', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
        ...
        // actors
        $this->actors = new DbField(
            $this, // Table
            'x_actors', // Variable name
            'actors', // Name
            '`actors`', // Expression
            '`actors`', // Basic search expression
            200, // Type
            4096, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`actors`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->actors->InputTextType = "text";
        $this->actors->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
        $this->Fields['actors'] = &$this->actors;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "film_list";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->actors->DbValue = $row['actors'];
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("filmlist2list");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "filmlist2view" => $Language->phrase("View"),
            "filmlist2edit" => $Language->phrase("Edit"),
            "filmlist2add" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "FilmList2View",
            Config("API_ADD_ACTION") => "FilmList2Add",
            Config("API_EDIT_ACTION") => "FilmList2Edit",
            Config("API_DELETE_ACTION") => "FilmList2Delete",
            Config("API_LIST_ACTION") => "FilmList2List",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("filmlist2view", $parm);
        } else {
            $url = $this->keyUrl("filmlist2view", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmlist2edit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmlist2list", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmlist2add", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("filmlist2list", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("filmlist2delete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // FID
        $this->FID->setupEditAttributes();
        $this->FID->EditValue = $this->FID->CurrentValue;
        $this->FID->PlaceHolder = RemoveHtml($this->FID->caption());
        if (strval($this->FID->EditValue) != "" && is_numeric($this->FID->EditValue)) {
            $this->FID->EditValue = FormatNumber($this->FID->EditValue, null);
        }
        ...
        // actors
        $this->actors->setupEditAttributes();
        if (!$this->actors->Raw) {
            $this->actors->CurrentValue = HtmlDecode($this->actors->CurrentValue);
        }
        $this->actors->EditValue = $this->actors->CurrentValue;
        $this->actors->PlaceHolder = RemoveHtml($this->actors->caption());
