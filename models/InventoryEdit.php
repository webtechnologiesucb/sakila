 ...
 * Page class
 */
class InventoryEdit extends Inventory
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "InventoryEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "inventoryedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("inventory_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->inventory_id->setQueryStringValue($keyValue);
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("inventory_id") ?? Route("inventory_id")) !== null) {
                    $this->inventory_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("inventorylist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "inventorylist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "inventorylist") {
                            ...
                            $returnUrl = "inventorylist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("inventorylist"), "", $this->TableVar, true);
        $pageId = "edit";
