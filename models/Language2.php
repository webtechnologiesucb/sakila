 ...
 */
class Language2 extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->language_id->IsAutoIncrement = true; // Autoincrement field
        $this->language_id->IsPrimaryKey = true; // Primary key field
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "language";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->language_id->setDbValue($conn->lastInsertId());
            $rs['language_id'] = $this->language_id->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['language_id']) && !EmptyValue($this->language_id->CurrentValue)) {
                $rs['language_id'] = $this->language_id->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('language_id', $rs)) {
                AddFilter($where, QuotedName('language_id', $this->Dbid) . '=' . QuotedValue($rs['language_id'], $this->language_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    {
        $keys = [];
        $val = $current ? $this->language_id->CurrentValue : $this->language_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->language_id->CurrentValue = $keys[0];
            } else {
                $this->language_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('language_id', $row) ? $row['language_id'] : null;
        } else {
            $val = !EmptyValue($this->language_id->OldValue) && !$current ? $this->language_id->OldValue : $this->language_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@language_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("language2list");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "language2view" => $Language->phrase("View"),
            "language2edit" => $Language->phrase("Edit"),
            "language2add" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "Language2View",
            Config("API_ADD_ACTION") => "Language2Add",
            Config("API_EDIT_ACTION") => "Language2Edit",
            Config("API_DELETE_ACTION") => "Language2Delete",
            Config("API_LIST_ACTION") => "Language2List",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("language2view", $parm);
        } else {
            $url = $this->keyUrl("language2view", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("language2edit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("language2list", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("language2add", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("language2list", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("language2delete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"language_id\":" . VarToJson($this->language_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->language_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->language_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("language_id") ?? Route("language_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->language_id->CurrentValue = $key;
            } else {
                $this->language_id->OldValue = $key;
            }
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'language');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'language', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'language', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'language', $fldname, $key, $oldvalue);
            }
        }
    }
