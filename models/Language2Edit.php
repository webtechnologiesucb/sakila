 ...
 * Page class
 */
class Language2Edit extends Language2
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "Language2Edit";
    ...
    // CSS class/style
    public $CurrentPageName = "language2edit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("language_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->language_id->setQueryStringValue($keyValue);
                $this->language_id->setOldValue($this->language_id->QueryStringValue);
            } elseif (Post("language_id") !== null) {
                $this->language_id->setFormValue(Post("language_id"));
                $this->language_id->setOldValue($this->language_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("language_id") ?? Route("language_id")) !== null) {
                    $this->language_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("language2list"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "language2list") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "language2list") {
                            ...
                            $returnUrl = "language2list"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->language_id->IsDetailKey) {
            $this->language_id->setFormValue($val);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("language2list"), "", $this->TableVar, true);
        $pageId = "edit";
