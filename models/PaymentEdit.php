 ...
 * Page class
 */
class PaymentEdit extends Payment
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "PaymentEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "paymentedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("payment_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->payment_id->setQueryStringValue($keyValue);
                $this->payment_id->setOldValue($this->payment_id->QueryStringValue);
            } elseif (Post("payment_id") !== null) {
                $this->payment_id->setFormValue(Post("payment_id"));
                $this->payment_id->setOldValue($this->payment_id->FormValue);
            } else {
    ...
    protected function loadFormValues()
    {
        ...
        if (!$this->payment_id->IsDetailKey) {
            $this->payment_id->setFormValue($val);
        }
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->payment_id->Visible && $this->payment_id->Required) {
                if (!$this->payment_id->IsDetailKey && EmptyValue($this->payment_id->FormValue)) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        // rental_id
        $this->rental_id->setDbValueDef($rsnew, $this->rental_id->CurrentValue, $this->rental_id->ReadOnly);

        // amount
        $this->amount->setDbValueDef($rsnew, $this->amount->CurrentValue, $this->amount->ReadOnly);

        // payment_date
        $this->payment_date->setDbValueDef($rsnew, UnFormatDateTime($this->payment_date->CurrentValue, $this->payment_date->formatPattern()), $this->payment_date->ReadOnly);
    ...
    public function getRowHash($row)
    {
        ...
        $hash .= GetFieldHash($row['rental_id']); // rental_id
