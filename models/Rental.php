...
/**
 * Table class for rental
 */
class Rental extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->rental_id->IsAutoIncrement = true; // Autoincrement field
        $this->rental_id->IsPrimaryKey = true; // Primary key field
        $this->rental_id->Nullable = false; // NOT NULL field
        $this->rental_id->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->rental_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['rental_id'] = &$this->rental_id;

        // rental_date
        $this->rental_date = new DbField(
            $this, // Table
            'x_rental_date', // Variable name
            'rental_date', // Name
            '`rental_date`', // Expression
            CastDateFieldForLike("`rental_date`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
            '`rental_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->rental_date->InputTextType = "text";
        $this->rental_date->Raw = true;
        $this->rental_date->Nullable = false; // NOT NULL field
        $this->rental_date->Required = true; // Required field
        $this->rental_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->rental_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['rental_date'] = &$this->rental_date;

        // inventory_id
        $this->inventory_id = new DbField(
            $this, // Table
            'x_inventory_id', // Variable name
            'inventory_id', // Name
            '`inventory_id`', // Expression
            '`inventory_id`', // Basic search expression
            19, // Type
            8, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`inventory_id`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->inventory_id->InputTextType = "text";
        $this->inventory_id->Raw = true;
        $this->inventory_id->Nullable = false; // NOT NULL field
        $this->inventory_id->Required = true; // Required field
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->inventory_id->Lookup = new Lookup($this->inventory_id, 'inventory', false, 'inventory_id', ["inventory_id","","",""], '', '', [], [], [], [], [], [], false, '', '', "`inventory_id`");
                break;
            default:
                $this->inventory_id->Lookup = new Lookup($this->inventory_id, 'inventory', false, 'inventory_id', ["inventory_id","","",""], '', '', [], [], [], [], [], [], false, '', '', "`inventory_id`");
                break;
        ...
        $this->inventory_id->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->customer_id->Lookup = new Lookup($this->customer_id, 'customer', false, 'customer_id', ["last_name","","",""], '', '', [], [], [], [], [], [], false, '`last_name` ASC', '', "`last_name`");
                break;
            default:
                $this->customer_id->Lookup = new Lookup($this->customer_id, 'customer', false, 'customer_id', ["last_name","","",""], '', '', [], [], [], [], [], [], false, '`last_name` ASC', '', "`last_name`");
                break;
        ...
        // return_date
        $this->return_date = new DbField(
            $this, // Table
            'x_return_date', // Variable name
            'return_date', // Name
            '`return_date`', // Expression
            CastDateFieldForLike("`return_date`", 0, "DB"), // Basic search expression
            135, // Type
            19, // Size
            0, // Date/Time format
            false, // Is upload field
            '`return_date`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->return_date->InputTextType = "text";
        $this->return_date->Raw = true;
        $this->return_date->DefaultErrorMessage = str_replace("%s", $GLOBALS["DATE_FORMAT"], $Language->phrase("IncorrectDate"));
        $this->return_date->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "rental";
    }
    ...
    public function insert(&$rs)
    {
        ...
        if ($result) {
            $this->rental_id->setDbValue($conn->lastInsertId());
            $rs['rental_id'] = $this->rental_id->DbValue;
    ...
    public function update(&$rs, $where = "", $rsold = null, $curfilter = true)
    {
        ...
        if ($success) {
            if (!isset($rs['rental_id']) && !EmptyValue($this->rental_id->CurrentValue)) {
                $rs['rental_id'] = $this->rental_id->CurrentValue;
            }
        }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('rental_id', $rs)) {
                AddFilter($where, QuotedName('rental_id', $this->Dbid) . '=' . QuotedValue($rs['rental_id'], $this->rental_id->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->rental_date->DbValue = $row['rental_date'];
    ...
    {
        $keys = [];
        $val = $current ? $this->rental_id->CurrentValue : $this->rental_id->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->rental_id->CurrentValue = $keys[0];
            } else {
                $this->rental_id->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('rental_id', $row) ? $row['rental_id'] : null;
        } else {
            $val = !EmptyValue($this->rental_id->OldValue) && !$current ? $this->rental_id->OldValue : $this->rental_id->CurrentValue;
        }
        if (!is_numeric($val)) {
        ...
        if ($val === null) {
            ...
            $keyFilter = str_replace("@rental_id@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("rentallist");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "rentalview" => $Language->phrase("View"),
            "rentaledit" => $Language->phrase("Edit"),
            "rentaladd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "RentalView",
            Config("API_ADD_ACTION") => "RentalAdd",
            Config("API_EDIT_ACTION") => "RentalEdit",
            Config("API_DELETE_ACTION") => "RentalDelete",
            Config("API_LIST_ACTION") => "RentalList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("rentalview", $parm);
        } else {
            $url = $this->keyUrl("rentalview", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("rentaledit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("rentallist", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("rentaladd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("rentallist", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("rentaldelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"rental_id\":" . VarToJson($this->rental_id->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    {
        if ($this->rental_id->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->rental_id->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("rental_id") ?? Route("rental_id")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->rental_id->CurrentValue = $key;
            } else {
                $this->rental_id->OldValue = $key;
            }
    ...
    public function renderEditRow()
    {
        ...
        $this->rental_date->EditValue = FormatDateTime($this->rental_date->CurrentValue, $this->rental_date->formatPattern());
        ...
        $this->return_date->EditValue = FormatDateTime($this->return_date->CurrentValue, $this->return_date->formatPattern());
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->rental_date);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->rental_date);
    ...
    {
        WriteAuditLog(CurrentUserIdentifier(), $typ, 'rental');
    }
    ...
    public function writeAuditTrailOnAdd(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "A", 'rental', $fldname, $key, "", $newvalue);
            }
        }
    }
        ...
        foreach (array_keys($rsnew) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && array_key_exists($fldname, $rsold) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                if ($modified) {
                    ...
                    WriteAuditLog($usr, "U", 'rental', $fldname, $key, $oldvalue, $newvalue);
                }
            }
        }
    }
    ...
    public function writeAuditTrailOnDelete(&$rs)
    {
        ...
        foreach (array_keys($rs) as $fldname) {
            if (array_key_exists($fldname, $this->Fields) && $this->Fields[$fldname]->DataType != DataType::BLOB) { // Ignore BLOB fields
                ...
                WriteAuditLog($usr, "D", 'rental', $fldname, $key, $oldvalue);
            }
        }
    }
