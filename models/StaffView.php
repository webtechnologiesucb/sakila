...
class StaffView extends Staff
{
    ...
    // CSS class/style
    public $CurrentPageName = "staffview";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("staff_id") ?? Route("staff_id")) !== null) {
            ...
            $this->RecKey["staff_id"] = $this->staff_id->QueryStringValue;
        } elseif (Post("staff_id") !== null) {
            $this->staff_id->setFormValue(Post("staff_id"));
            $this->RecKey["staff_id"] = $this->staff_id->FormValue;
        } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
            $this->staff_id->setQueryStringValue($keyValue);
            $this->RecKey["staff_id"] = $this->staff_id->QueryStringValue;
        } elseif (!$loadCurrentRecord) {
            $returnUrl = "stafflist"; // Return to list
        }
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "stafflist"; // No matching record, return to list
                    }
                break;
        }
        if ($returnUrl != "") {
