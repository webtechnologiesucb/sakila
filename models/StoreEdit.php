...
class StoreEdit extends Store
{
    ...
    // CSS class/style
    public $CurrentPageName = "storeedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("store_id") ?? Key(0) ?? Route(2)) !== null) {
                $this->store_id->setQueryStringValue($keyValue);
                $this->store_id->setOldValue($this->store_id->QueryStringValue);
            } elseif (Post("store_id") !== null) {
                $this->store_id->setFormValue(Post("store_id"));
                $this->store_id->setOldValue($this->store_id->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("store_id") ?? Route("store_id")) !== null) {
                    $this->store_id->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("storelist"); // No matching record, return to list
                        return;
                    }
                break;
            case "update": // Update
                $returnUrl = $this->getReturnUrl();
                if (GetPageName($returnUrl) == "storelist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "storelist") {
                            ...
                            $returnUrl = "storelist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'manager_staff_id' first before field var 'x_manager_staff_id'
        $val = $CurrentForm->hasValue("manager_staff_id") ? $CurrentForm->getValue("manager_staff_id") : $CurrentForm->getValue("x_manager_staff_id");
        if (!$this->manager_staff_id->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->manager_staff_id->Visible = false; // Disable update for API request
            } else {
                $this->manager_staff_id->setFormValue($val, true, $validate);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->manager_staff_id->CurrentValue = $this->manager_staff_id->FormValue;
    ...
    public function renderRow()
    {
        ...
        // manager_staff_id
        $this->manager_staff_id->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // manager_staff_id
            $this->manager_staff_id->setupEditAttributes();
            $this->manager_staff_id->EditValue = $this->manager_staff_id->CurrentValue;
            $curVal = strval($this->manager_staff_id->CurrentValue);
            if ($curVal != "") {
                $this->manager_staff_id->EditValue = $this->manager_staff_id->lookupCacheOption($curVal);
                if ($this->manager_staff_id->EditValue === null) { // Lookup from database
                    $filterWrk = SearchFilter($this->manager_staff_id->Lookup->getTable()->Fields["staff_id"]->searchExpression(), "=", $curVal, $this->manager_staff_id->Lookup->getTable()->Fields["staff_id"]->searchDataType(), "");
                    $sqlWrk = $this->manager_staff_id->Lookup->getSql(false, $filterWrk, '', $this, true, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    ...
                    if ($ari > 0) { // Lookup values found
                        $arwrk = $this->manager_staff_id->Lookup->renderViewRow($rswrk[0]);
                        $this->manager_staff_id->EditValue = $this->manager_staff_id->displayValue($arwrk);
                    } else {
                        $this->manager_staff_id->EditValue = HtmlEncode(FormatNumber($this->manager_staff_id->CurrentValue, $this->manager_staff_id->formatPattern()));
                    }
                }
            } else {
                $this->manager_staff_id->EditValue = null;
            }
            $this->manager_staff_id->PlaceHolder = RemoveHtml($this->manager_staff_id->caption());
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->manager_staff_id->Visible && $this->manager_staff_id->Required) {
                if (!$this->manager_staff_id->IsDetailKey && EmptyValue($this->manager_staff_id->FormValue)) {
                    $this->manager_staff_id->addErrorMessage(str_replace("%s", $this->manager_staff_id->caption(), $this->manager_staff_id->RequiredErrorMessage));
                }
            }
            if (!CheckInteger($this->manager_staff_id->FormValue)) {
                $this->manager_staff_id->addErrorMessage($this->manager_staff_id->getErrorMessage(false));
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (manager_staff_id)
        if ($this->manager_staff_id->CurrentValue != "") {
            $filterChk = "(`manager_staff_id` = " . AdjustSql($this->manager_staff_id->CurrentValue, $this->Dbid) . ")";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->manager_staff_id->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->manager_staff_id->CurrentValue, $idxErrMsg);
    ...
    protected function getEditRow($rsold)
    {
        ...
        // manager_staff_id
        $this->manager_staff_id->setDbValueDef($rsnew, $this->manager_staff_id->CurrentValue, $this->manager_staff_id->ReadOnly);
    ...
    {
        if (isset($row['manager_staff_id'])) { // manager_staff_id
            $this->manager_staff_id->CurrentValue = $row['manager_staff_id'];
        }
    ...
    public function getRowHash($row)
    {
        ...
        $hash = "";
        $hash .= GetFieldHash($row['manager_staff_id']); // manager_staff_id
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("storelist"), "", $this->TableVar, true);
        $pageId = "edit";
