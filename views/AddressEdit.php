...
?>
<main class="edit">
<form name="faddressedit" id="faddressedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_address_address_id">
<span<?= $Page->address_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->address_id->getDisplayValue($Page->address_id->ViewValue))) ?>"></span>
<input type="hidden" data-table="address" data-field="x_address_id" data-hidden="1" name="x_address_id" id="x_address_id" value="<?= HtmlEncode($Page->address_id->FormValue) ?>">
</span>
<?php } ?>
</div></div>
    </div>
...
<span id="el_address_city_id">
    <select
        ...
        data-select2-id="faddressedit_x_city_id"
        data-table="address"
...
loadjs.ready("faddressedit", function() {
    var options = { name: "x_city_id", selectId: "faddressedit_x_city_id" };
    if (faddressedit.lists.city_id?.lookupOptions.length) {
        options.data = { id: "x_city_id", form: "faddressedit" };
    } else {
        options.ajax = { id: "x_city_id", form: "faddressedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["faddressedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("faddressedit", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="faddressedit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="faddressedit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="faddressedit" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="faddressedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="faddressedit"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="faddressedit" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } ?>
    </div><!-- /buttons offset -->
