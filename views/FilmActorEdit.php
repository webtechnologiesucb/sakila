...
?>
<main class="edit">
<form name="ffilm_actoredit" id="ffilm_actoredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<span id="el_film_actor_actor_id">
    <select
        ...
        data-select2-id="ffilm_actoredit_x_actor_id"
...
<script>
loadjs.ready("ffilm_actoredit", function() {
    var options = { name: "x_actor_id", selectId: "ffilm_actoredit_x_actor_id" };
    if (ffilm_actoredit.lists.actor_id?.lookupOptions.length) {
        options.data = { id: "x_actor_id", form: "ffilm_actoredit" };
    } else {
        options.ajax = { id: "x_actor_id", form: "ffilm_actoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options = Object.assign({}, ew.modalLookupOptions, options, ew.vars.tables.film_actor.fields.actor_id.modalLookupOptions);
    ew.createModalLookup(options);
});
</script>
<input type="hidden" data-table="film_actor" data-field="x_actor_id" data-hidden="1" data-old name="o_actor_id" id="o_actor_id" value="<?= HtmlEncode($Page->actor_id->OldValue ?? $Page->actor_id->CurrentValue) ?>">
</span>
<?php } else { ?>
<span id="el_film_actor_actor_id">
...
<span id="el_film_actor_film_id">
    <select
        ...
        data-select2-id="ffilm_actoredit_x_film_id"
        data-table="film_actor"
...
<script>
loadjs.ready("ffilm_actoredit", function() {
    var options = { name: "x_film_id", selectId: "ffilm_actoredit_x_film_id" };
    if (ffilm_actoredit.lists.film_id?.lookupOptions.length) {
        options.data = { id: "x_film_id", form: "ffilm_actoredit" };
    } else {
        options.ajax = { id: "x_film_id", form: "ffilm_actoredit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options = Object.assign({}, ew.modalLookupOptions, options, ew.vars.tables.film_actor.fields.film_id.modalLookupOptions);
    ew.createModalLookup(options);
});
</script>
<input type="hidden" data-table="film_actor" data-field="x_film_id" data-hidden="1" data-old name="o_film_id" id="o_film_id" value="<?= HtmlEncode($Page->film_id->OldValue ?? $Page->film_id->CurrentValue) ?>">
</span>
<?php } else { ?>
<span id="el_film_actor_film_id">
...
<script>
loadjs.ready(["ffilm_actoredit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffilm_actoredit", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_actoredit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="ffilm_actoredit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_actoredit" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ffilm_actoredit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_actoredit"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="ffilm_actoredit" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } ?>
    </div><!-- /buttons offset -->
