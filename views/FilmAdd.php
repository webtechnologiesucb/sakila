...
$Page->showMessage();
?>
<form name="ffilmadd" id="ffilmadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_film_language_id">
    <select
        ...
        data-select2-id="ffilmadd_x_language_id"
        data-table="film"
...
<script>
loadjs.ready("ffilmadd", function() {
    var options = { name: "x_language_id", selectId: "ffilmadd_x_language_id" };
    if (ffilmadd.lists.language_id?.lookupOptions.length) {
        options.data = { id: "x_language_id", form: "ffilmadd" };
    } else {
        options.ajax = { id: "x_language_id", form: "ffilmadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_film_original_language_id">
    <select
        ...
        data-select2-id="ffilmadd_x_original_language_id"
        data-table="film"
...
<script>
loadjs.ready("ffilmadd", function() {
    var options = { name: "x_original_language_id", selectId: "ffilmadd_x_original_language_id" };
    if (ffilmadd.lists.original_language_id?.lookupOptions.length) {
        options.data = { id: "x_original_language_id", form: "ffilmadd" };
    } else {
        options.ajax = { id: "x_original_language_id", form: "ffilmadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["ffilmadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffilmadd", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilmadd" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ffilmadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilmadd"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="ffilmadd" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
