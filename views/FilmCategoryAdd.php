...
$Page->showMessage();
?>
<form name="ffilm_categoryadd" id="ffilm_categoryadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_film_category_film_id">
    <select
        ...
        data-select2-id="ffilm_categoryadd_x_film_id"
...
<script>
loadjs.ready("ffilm_categoryadd", function() {
    var options = { name: "x_film_id", selectId: "ffilm_categoryadd_x_film_id" };
    if (ffilm_categoryadd.lists.film_id?.lookupOptions.length) {
        options.data = { id: "x_film_id", form: "ffilm_categoryadd" };
    } else {
        options.ajax = { id: "x_film_id", form: "ffilm_categoryadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_film_category_category_id">
    <select
        ...
        data-select2-id="ffilm_categoryadd_x_category_id"
        data-table="film_category"
...
<script>
loadjs.ready("ffilm_categoryadd", function() {
    var options = { name: "x_category_id", selectId: "ffilm_categoryadd_x_category_id" };
    if (ffilm_categoryadd.lists.category_id?.lookupOptions.length) {
        options.data = { id: "x_category_id", form: "ffilm_categoryadd" };
    } else {
        options.ajax = { id: "x_category_id", form: "ffilm_categoryadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["ffilm_categoryadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("ffilm_categoryadd", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_categoryadd" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ffilm_categoryadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_categoryadd"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="ffilm_categoryadd" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
