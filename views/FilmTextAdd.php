...
$Page->showMessage();
?>
<form name="ffilm_textadd" id="ffilm_textadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_film_text_film_id">
    <select
        ...
        data-select2-id="ffilm_textadd_x_film_id"
...
<script>
loadjs.ready("ffilm_textadd", function() {
    var options = { name: "x_film_id", selectId: "ffilm_textadd_x_film_id" };
    if (ffilm_textadd.lists.film_id?.lookupOptions.length) {
        options.data = { id: "x_film_id", form: "ffilm_textadd" };
    } else {
        options.ajax = { id: "x_film_id", form: "ffilm_textadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_textadd" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="ffilm_textadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="ffilm_textadd"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="ffilm_textadd" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
