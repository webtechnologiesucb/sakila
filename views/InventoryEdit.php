...
?>
<main class="edit">
<form name="finventoryedit" id="finventoryedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->inventory_id->Visible) { // inventory_id ?>
    <div id="r_inventory_id"<?= $Page->inventory_id->rowAttributes() ?>>
        <label id="elh_inventory_inventory_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->inventory_id->caption() ?><?= $Page->inventory_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->inventory_id->getDisplayValue($Page->inventory_id->EditValue))) ?>"></span>
<input type="hidden" data-table="inventory" data-field="x_inventory_id" data-hidden="1" name="x_inventory_id" id="x_inventory_id" value="<?= HtmlEncode($Page->inventory_id->CurrentValue) ?>">
</span>
<?php } else { ?>
<span id="el_inventory_inventory_id">
<span<?= $Page->inventory_id->viewAttributes() ?>>
...
<input type="hidden" data-table="inventory" data-field="x_inventory_id" data-hidden="1" name="x_inventory_id" id="x_inventory_id" value="<?= HtmlEncode($Page->inventory_id->FormValue) ?>">
</span>
<?php } ?>
</div></div>
    </div>
...
<span id="el_inventory_film_id">
    <select
        ...
        data-select2-id="finventoryedit_x_film_id"
...
<script>
loadjs.ready("finventoryedit", function() {
    var options = { name: "x_film_id", selectId: "finventoryedit_x_film_id" };
    if (finventoryedit.lists.film_id?.lookupOptions.length) {
        options.data = { id: "x_film_id", form: "finventoryedit" };
    } else {
        options.ajax = { id: "x_film_id", form: "finventoryedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_inventory_store_id">
    <select
        ...
        data-select2-id="finventoryedit_x_store_id"
        data-table="inventory"
...
<script>
loadjs.ready("finventoryedit", function() {
    var options = { name: "x_store_id", selectId: "finventoryedit_x_store_id" };
    if (finventoryedit.lists.store_id?.lookupOptions.length) {
        options.data = { id: "x_store_id", form: "finventoryedit" };
    } else {
        options.ajax = { id: "x_store_id", form: "finventoryedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["finventoryedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("finventoryedit", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="finventoryedit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="finventoryedit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="finventoryedit" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="finventoryedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="finventoryedit"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="finventoryedit" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } ?>
    </div><!-- /buttons offset -->
