...
?>
<main class="edit">
<form name="flanguage2edit" id="flanguage2edit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        .setId("flanguage2edit")
        .setPageId("edit")

        // Add fields
        .setFields([
            ["language_id", [fields.language_id.visible && fields.language_id.required ? ew.Validators.required(fields.language_id.caption) : null], fields.language_id.isInvalid],
...
<?php if ($Page->language_id->Visible) { // language_id ?>
    <div id="r_language_id"<?= $Page->language_id->rowAttributes() ?>>
        <label id="elh_language2_language_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->language_id->caption() ?><?= $Page->language_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->language_id->getDisplayValue($Page->language_id->EditValue))) ?>"></span>
<input type="hidden" data-table="language2" data-field="x_language_id" data-hidden="1" name="x_language_id" id="x_language_id" value="<?= HtmlEncode($Page->language_id->CurrentValue) ?>">
</span>
<?php } else { ?>
<span id="el_language2_language_id">
<span<?= $Page->language_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->language_id->getDisplayValue($Page->language_id->ViewValue))) ?>"></span>
<input type="hidden" data-table="language2" data-field="x_language_id" data-hidden="1" name="x_language_id" id="x_language_id" value="<?= HtmlEncode($Page->language_id->FormValue) ?>">
</span>
<?php } ?>
</div></div>
    </div>
...
<script>
loadjs.ready(["flanguage2edit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("flanguage2edit", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="flanguage2edit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="flanguage2edit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="flanguage2edit" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="flanguage2edit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="flanguage2edit"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="flanguage2edit" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } ?>
    </div><!-- /buttons offset -->
