...
?>
<main class="edit">
<form name="fpaymentedit" id="fpaymentedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
loadjs.ready(["wrapper", "head"], function () {
    ...
    let form = new ew.FormBuilder()
        ...
        // Add fields
        .setFields([
            ["payment_id", [fields.payment_id.visible && fields.payment_id.required ? ew.Validators.required(fields.payment_id.caption) : null], fields.payment_id.isInvalid],
...
<?php if ($Page->payment_id->Visible) { // payment_id ?>
    <div id="r_payment_id"<?= $Page->payment_id->rowAttributes() ?>>
        <label id="elh_payment_payment_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->payment_id->caption() ?><?= $Page->payment_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->payment_id->getDisplayValue($Page->payment_id->EditValue))) ?>"></span>
<input type="hidden" data-table="payment" data-field="x_payment_id" data-hidden="1" name="x_payment_id" id="x_payment_id" value="<?= HtmlEncode($Page->payment_id->CurrentValue) ?>">
</span>
<?php } else { ?>
<span id="el_payment_payment_id">
<span<?= $Page->payment_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->payment_id->getDisplayValue($Page->payment_id->ViewValue))) ?>"></span>
<input type="hidden" data-table="payment" data-field="x_payment_id" data-hidden="1" name="x_payment_id" id="x_payment_id" value="<?= HtmlEncode($Page->payment_id->FormValue) ?>">
</span>
<?php } ?>
</div></div>
    </div>
...
<span id="el_payment_customer_id">
    <select
        ...
        data-select2-id="fpaymentedit_x_customer_id"
        data-table="payment"
...
<script>
loadjs.ready("fpaymentedit", function() {
    var options = { name: "x_customer_id", selectId: "fpaymentedit_x_customer_id" };
    if (fpaymentedit.lists.customer_id?.lookupOptions.length) {
        options.data = { id: "x_customer_id", form: "fpaymentedit" };
    } else {
        options.ajax = { id: "x_customer_id", form: "fpaymentedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_payment_staff_id">
    <select
        ...
        data-select2-id="fpaymentedit_x_staff_id"
        data-table="payment"
...
<script>
loadjs.ready("fpaymentedit", function() {
    var options = { name: "x_staff_id", selectId: "fpaymentedit_x_staff_id" };
    if (fpaymentedit.lists.staff_id?.lookupOptions.length) {
        options.data = { id: "x_staff_id", form: "fpaymentedit" };
    } else {
        options.ajax = { id: "x_staff_id", form: "fpaymentedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_payment_rental_id">
    <select
        ...
        data-select2-id="fpaymentedit_x_rental_id"
        data-table="payment"
...
<script>
loadjs.ready("fpaymentedit", function() {
    var options = { name: "x_rental_id", selectId: "fpaymentedit_x_rental_id" };
    if (fpaymentedit.lists.rental_id?.lookupOptions.length) {
        options.data = { id: "x_rental_id", form: "fpaymentedit" };
    } else {
        options.ajax = { id: "x_rental_id", form: "fpaymentedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fpaymentedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpaymentedit", "x_payment_date", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
loadjs.ready(["fpaymentedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fpaymentedit", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpaymentedit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="fpaymentedit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpaymentedit" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpaymentedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpaymentedit"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="fpaymentedit" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } ?>
    </div><!-- /buttons offset -->
