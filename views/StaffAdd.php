...
$Page->showMessage();
?>
<form name="fstaffadd" id="fstaffadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<span id="el_staff_address_id">
    <select
        ...
        data-select2-id="fstaffadd_x_address_id"
        data-table="staff"
...
<script>
loadjs.ready("fstaffadd", function() {
    var options = { name: "x_address_id", selectId: "fstaffadd_x_address_id" };
    if (fstaffadd.lists.address_id?.lookupOptions.length) {
        options.data = { id: "x_address_id", form: "fstaffadd" };
    } else {
        options.ajax = { id: "x_address_id", form: "fstaffadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_staff_store_id">
    <select
        ...
        data-select2-id="fstaffadd_x_store_id"
        data-table="staff"
...
<script>
loadjs.ready("fstaffadd", function() {
    var options = { name: "x_store_id", selectId: "fstaffadd_x_store_id" };
    if (fstaffadd.lists.store_id?.lookupOptions.length) {
        options.data = { id: "x_store_id", form: "fstaffadd" };
    } else {
        options.ajax = { id: "x_store_id", form: "fstaffadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<div class="input-group">
    <input type="password" name="x__password" id="x__password" autocomplete="new-password" data-table="staff" data-field="x__password" size="30" maxlength="40" placeholder="<?= HtmlEncode($Page->_password->getPlaceHolder()) ?>"<?= $Page->_password->editAttributes() ?> aria-describedby="x__password_help">
...
loadjs.ready(["fstaffadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fstaffadd", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstaffadd" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fstaffadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstaffadd"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="fstaffadd" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
