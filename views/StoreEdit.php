...
?>
<main class="edit">
<form name="fstoreedit" id="fstoreedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<?php if ($Page->store_id->Visible) { // store_id ?>
    <div id="r_store_id"<?= $Page->store_id->rowAttributes() ?>>
        <label id="elh_store_store_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->store_id->caption() ?><?= $Page->store_id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="store" data-field="x_store_id" data-hidden="1" name="x_store_id" id="x_store_id" value="<?= HtmlEncode($Page->store_id->CurrentValue) ?>">
</span>
<?php } else { ?>
<span id="el_store_store_id">
<span<?= $Page->store_id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->store_id->getDisplayValue($Page->store_id->ViewValue))) ?>"></span>
<input type="hidden" data-table="store" data-field="x_store_id" data-hidden="1" name="x_store_id" id="x_store_id" value="<?= HtmlEncode($Page->store_id->FormValue) ?>">
</span>
<?php } ?>
</div></div>
    </div>
...
<span id="el_store_manager_staff_id">
    <select
        ...
        data-select2-id="fstoreedit_x_manager_staff_id"
        data-table="store"
...
loadjs.ready("fstoreedit", function() {
    var options = { name: "x_manager_staff_id", selectId: "fstoreedit_x_manager_staff_id" };
    if (fstoreedit.lists.manager_staff_id?.lookupOptions.length) {
        options.data = { id: "x_manager_staff_id", form: "fstoreedit" };
    } else {
        options.ajax = { id: "x_manager_staff_id", form: "fstoreedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_store_address_id">
    <select
        ...
        data-select2-id="fstoreedit_x_address_id"
        data-table="store"
...
<script>
loadjs.ready("fstoreedit", function() {
    var options = { name: "x_address_id", selectId: "fstoreedit_x_address_id" };
    if (fstoreedit.lists.address_id?.lookupOptions.length) {
        options.data = { id: "x_address_id", form: "fstoreedit" };
    } else {
        options.ajax = { id: "x_address_id", form: "fstoreedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready(["fstoreedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fstoreedit", "x_last_update", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->UpdateConflict == "U") { // Record already updated by other user ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstoreedit" data-ew-action="set-action" data-value="overwrite"><?= $Language->phrase("OverwriteBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-reload" id="btn-reload" type="submit" form="fstoreedit" data-ew-action="set-action" data-value="show"><?= $Language->phrase("ReloadBtn") ?></button>
<?php } else { ?>
<?php if (!$Page->isConfirm()) { // Confirm page ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstoreedit" data-ew-action="set-action" data-value="confirm"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fstoreedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } else { ?>
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstoreedit"><?= $Language->phrase("ConfirmBtn") ?></button>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="submit" form="fstoreedit" data-ew-action="set-action" data-value="cancel"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
<?php } ?>
    </div><!-- /buttons offset -->
